...
(function initKeyboardPOS() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    // ── Teclado numérico en modal de CANTIDAD ──
    if (document.getElementById('modalCantidad')?.classList.contains('open')) {
      if (e.key >= '0' && e.key <= '9') {
        e.preventDefault();
        if (typeof cantTecladoDigito === 'function') cantTecladoDigito(e.key);
        return;
      }
      if (e.key === 'Backspace') { e.preventDefault(); if (typeof cantTecladoBorrar === 'function') cantTecladoBorrar(); return; }
      if (e.key === 'Enter')     { e.preventDefault(); if (typeof cantTecladoConfirmar === 'function') cantTecladoConfirmar(); return; }
    }
...
function renderCorteCaja() {
  ...
  cont.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:12px;margin-bottom:20px;">
      ${_kpiCard('💰', 'Ventas del día', '$' + totalVentas.toFixed(2), '#ea580c')}
      ...
      ${_kpiCard('✨', 'Ganancia estimada', '$' + gananciaEstimada.toFixed(2), '#65a30d')}
    </div>
...
function corteActualizarDiferencia(esperado) {
  ...
  el.style.color = diff === 0 ? '#ea580c' : diff > 0 ? '#1d4ed8' : '#dc2626';
}
...
function _crearModalReportesPro() {
  ...
  <div class="modal-box modal-lg" style="max-width:700px;">
    <div class="modal-header" style="background:linear-gradient(135deg,#65a30d,#4d7c0f);">
...
function renderReportesPro(tab = 'dia') {
  ...
  if (tab === 'ganancias') {
    ...
    cont.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:20px;">
        ${_kpiCard('💰', 'Total vendido', '$' + totalVentas.toFixed(2), '#ea580c')}
        ${_kpiCard('✨', 'Ganancia neta', '$' + ganancia.toFixed(2), '#65a30d')}
      ...
      ${Object.entries(prods).sort((a,b)=>(b[1].total-b[1].compra)-(a[1].total-a[1].compra)).slice(0,15).map(([nom,d]) => {
        ...
        return `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
          ...
          <div style="font-size:10px;background:${mg>20?'#ffedd5':mg>10?'#fef3c7':'#fee2e2'};color:${mg>20?'#c2410c':mg>10?'#92400e':'#991b1b'};padding:2px 6px;border-radius:100px;font-weight:800;">${mg.toFixed(0)}%</div>
        </div>`;
      }).join('')}`;
    return;
  }
  ...
  cont.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:20px;">
      ${_kpiCard('💰', tab === 'dia' ? 'Ventas hoy' : 'Total período', '$' + totalVentas.toFixed(2), '#ea580c')}
...
function posProInyectarBotones() {
  ...
  if (navStats && !document.getElementById('btnReporteNv')) {
    ...
    btn.id = 'btnReporteNv';
    btn.className = 'stat-chip';
    btn.style.cssText = 'cursor:pointer;background:rgba(101,163,13,0.25);border-color:rgba(101,163,13,0.5);';
