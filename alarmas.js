...
let _lastScanTs = 0;
let _scannerFacingMode = 'environment'; // trasera por defecto

async function abrirScannerVenta() {
  _scannerModo = 'venta';
  _scannerFacingMode = 'environment'; // siempre abre con la trasera por defecto
  ...
  try {
    ...
    const config = esFirefox
      ? {
          ...
          disableFlip: false, // tolera cámara frontal espejada
        }
      : {
    ...
    // 1) { ideal: _scannerFacingMode } — suave, funciona en todos los browsers y SO
    // 2) Listar cámaras y elegir la trasera/frontal por label (fallback Android/Firefox)
    // 3) Cualquier cámara disponible (último recurso desktop/Firefox sin la cámara pedida)
    ...
    try {
      await _html5Qr.start({ facingMode: { ideal: _scannerFacingMode } }, config, onScan, onErr);
      iniciado = true;
    } catch (e1) { /* continuar */ }

    // Intento 2: elegir cámara trasera/frontal por ID (Firefox desktop / algunos Android)
    if (!iniciado) {
      try {
        const cameras = await Html5Qrcode.getCameras();
        if (cameras && cameras.length > 0) {
          const regexCam = _scannerFacingMode === 'environment'
            ? /back|environ|rear|poste|trasera/i
            : /front|user|frontal|selfie/i;
          const elegida = cameras.find(c => regexCam.test(c.label || ''));
          const camId = elegida ? elegida.id : cameras[0].id;
...
// Alterna entre cámara trasera y frontal sin cambiar velocidad ni formatos:
// reinicia el mismo iniciarScanner(), que ya usa _scannerFacingMode y aplica
// la misma config (fps/qrbox/formatos) y estrategia de respaldo para ambas cámaras.
async function flipScannerCamera() {
  const btn = document.getElementById('btnFlipCamScanner');
  if (btn) btn.disabled = true;
  _scannerFacingMode = _scannerFacingMode === 'environment' ? 'user' : 'environment';
  setScannerStatus(_scannerFacingMode === 'environment' ? 'Cambiando a cámara trasera…' : 'Cambiando a cámara frontal…');
  try {
    await iniciarScanner();
  } finally {
...
function renderInvAnalisis() {
  ...
  historial.forEach(v => {
    ...
    (v.items || []).forEach(it => {
      ...
      ventasPorCat[catItem].total    += (it.totalItem !== undefined ? Number(it.totalItem) : (it.cantCobrada !== undefined ? Number(it.cantCobrada)*Number(it.precio||0) : Number(it.cant||0)*Number(it.precio||0)));
      const nomKey = it.nom || '?';
      ventasPorCat[catItem].detalle[nomKey] ??= { nom: nomKey, cant: 0, total: 0 };
      ventasPorCat[catItem].detalle[nomKey].cant  += Number(it.cant || 0);
      ventasPorCat[catItem].detalle[nomKey].total += (it.totalItem !== undefined ? Number(it.totalItem) : (it.cantCobrada !== undefined ? Number(it.cantCobrada)*Number(it.precio||0) : Number(it.cant||0)*Number(it.precio||0)));
    });
  });
