      ...
      if (region) {
        region.innerHTML = `
          <div style="padding:20px;text-align:center;color:#fff;">
            ...
            <button onclick="location.reload()" style="margin-top:14px;background:#ea580c;color:#fff;border:none;border-radius:10px;padding:10px 22px;font-size:14px;font-weight:900;cursor:pointer;">🔄 Recargar página</button>
          </div>`;
      }
...
function renderListaAlarmas() {
  ...
  cont.innerHTML = alarmasPDF.map((a,i) => {
    ...
    const provLabel = a.proveedor && a.proveedor !== 'todos' ? a.proveedor : null;
    return `<div style="background:${a.activa?'var(--green-light)':'var(--surface2)'};border:1.5px solid ${a.activa?'var(--border-mid)':'var(--border)'};border-radius:var(--r-sm);padding:10px 12px;margin-bottom:8px;transition:all 0.2s;">
      ...
      <div style="font-size:11px;color:var(--text-dim);font-weight:700;display:flex;flex-wrap:wrap;gap:6px;">
        ...
        <span style="background:rgba(234,88,12,0.1);color:var(--green-dark);border-radius:4px;padding:2px 7px;">📦 ${catLabel}</span>
        ${provLabel ? `<span style="background:rgba(180,83,9,0.1);color:#92400e;border-radius:4px;padding:2px 7px;">🏭 ${provLabel}</span>` : ''}
...
function resetFormAlarma() {
  ...
  const selProvReset = document.getElementById('alarmaProveedorSel');
  if (selProvReset) selProvReset.value = 'todos';
...
function editarAlarma(i) {
  ...
  const selProvEdit = document.getElementById('alarmaProveedorSel');
  if (selProvEdit) selProvEdit.value = a.proveedor || 'todos';
...
function guardarAlarma() {
  ...
  const proveedor = document.getElementById('alarmaProveedorSel')?.value || 'todos';
  ...
  const alarma = { id: _alarmaEditId !== null ? alarmasPDF[_alarmaEditId].id : Date.now(), nombre, dias: diasActivos, hora, fechaDesde, fechaHasta, categoria, proveedor, activa: true };
...
function ejecutarDescargaAlarma(a) {
  ...
  // Llenar proveedor si el selector existe y la opción también
  const selProv = document.getElementById('pdfProveedor');
  if (selProv) {
    const optProv = [...selProv.options].find(o => o.value === (a.proveedor || 'todos'));
    selProv.value = optProv ? (a.proveedor || 'todos') : 'todos';
  }
  generarPDFRango();
}
...
function poblarCategoriasSelectorAlarma() {
  ...
  const selProv = document.getElementById('alarmaProveedorSel');
  if (selProv) {
    const provs = [...new Set(productos.map(p => p.proveedor).filter(Boolean))].sort();
    const valorActualProv = selProv.value;
    selProv.innerHTML = '<option value="todos">🏭 Todos</option>';
    provs.forEach(pr => { const o = document.createElement('option'); o.value=pr; o.textContent=pr; selProv.appendChild(o); });
    if (valorActualProv && [...selProv.options].find(o=>o.value===valorActualProv)) selProv.value = valorActualProv;
  }
}
...
function poblarInvAnCat() {
  ...
  const selP = document.getElementById('invAnProv');
  if (selP) {
    const provs = [...new Set(productos.map(p => p.proveedor).filter(Boolean))].sort();
    const actualP = selP.value;
    selP.innerHTML = '<option value="todos">🏭 Todos los proveedores</option>' +
      provs.map(pr => `<option value="${pr}"${pr === actualP ? ' selected' : ''}>${pr}</option>`).join('');
  }
}
...
function renderInvAnalisis() {
  ...
  const provFiltro = document.getElementById('invAnProv')?.value || 'todos';
  const agruparPor = document.getElementById('invAnAgrupar')?.value || 'cat';
  const desde      = document.getElementById('invAnDesde')?.value || '';
  const hasta      = document.getElementById('invAnHasta')?.value || '';

  // Encabezados dinámicos según agrupación elegida
  const labelGrupo = agruparPor === 'proveedor' ? 'Proveedor' : 'Categoría';
  const thStock  = document.getElementById('invAnTh1Stock');
  const thVentas = document.getElementById('invAnTh1Ventas');
  if (thStock)  thStock.textContent  = labelGrupo;
  if (thVentas) thVentas.textContent = labelGrupo;

  const grupoDe = (p) => agruparPor === 'proveedor' ? (p.proveedor || 'SIN PROVEEDOR') : (p.cat || 'SIN CATEGORÍA');
  ...
  // ── Valor inventario actual agrupado por categoría/proveedor ───────────
  const invPorCat = {};
  prods.forEach(p => {
    const cat = grupoDe(p);
  ...
  // ── Ventas del período agrupadas por categoría/proveedor ───────────────
  ...
  historial.forEach(v => {
    ...
    (v.items || []).forEach(it => {
      ...
      const catItemReal  = it.cat || (prod ? (prod.cat || 'SIN CATEGORÍA') : 'SIN CATEGORÍA');
      const provItemReal = prod ? (prod.proveedor || '') : '';
      if (catFiltro !== 'todas' && catItemReal !== catFiltro) return;
      if (provFiltro !== 'todos' && provItemReal !== provFiltro) return;
      const grupoItem = agruparPor === 'proveedor' ? (provItemReal || 'SIN PROVEEDOR') : catItemReal;
      ventasPorCat[grupoItem] ??= { cat: grupoItem, prodsSet: new Set(), unidades: 0, total: 0, detalle: {} };
      ventasPorCat[grupoItem].prodsSet.add(it.nom || '');
      ventasPorCat[grupoItem].unidades += Number(it.cant || 0);
      ventasPorCat[grupoItem].total    += (it.totalItem !== undefined ? Number(it.totalItem) : (it.cantCobrada !== undefined ? Number(it.cantCobrada)*Number(it.precio||0) : Number(it.cant||0)*Number(it.precio||0)));
      const nomKey = it.nom || '?';
      ventasPorCat[grupoItem].detalle[nomKey] ??= { nom: nomKey, cat: catItemReal, proveedor: provItemReal, cant: 0, total: 0 };
      ventasPorCat[grupoItem].detalle[nomKey].cant  += Number(it.cant || 0);
      ventasPorCat[grupoItem].detalle[nomKey].total += (it.totalItem !== undefined ? Number(it.totalItem) : (it.cantCobrada !== undefined ? Number(it.cantCobrada)*Number(it.precio||0) : Number(it.cant||0)*Number(it.precio||0)));
    });
  });
  ...
  if (infoEl) infoEl.textContent = `📆 Ventas ${periodoStr}${catFiltro !== 'todas' ? ' · Cat: ' + catFiltro : ''}${provFiltro !== 'todos' ? ' · Prov: ' + provFiltro : ''}`;
...
function invAnVerDetalle(cat) {
  ...
  tbody.innerHTML = items.map(it => `<tr>
    ...
    <td style="font-size:11px;color:var(--text-muted);">${it.cat || '—'}</td>
    <td style="font-size:11px;color:var(--text-muted);">${it.proveedor || '—'}</td>
...
function generarPDFInventario() {
  ...
  const provLabel = provFiltro === 'todos' ? 'Todos los proveedores' : provFiltro;
  const rangoLabel = desde && hasta ? `${desde} al ${hasta}` : desde ? `Desde ${desde}` : hasta ? `Hasta ${hasta}` : 'Todo el historial';
  doc.text(`Categoría: ${catLabel}   |   Proveedor: ${provLabel}`, 14, 19);
  doc.text(`Período de ventas: ${rangoLabel}   |   Generado: ${new Date().toLocaleString('es-SV')}`, 14, 25);

  let y = 34;

  // ── Sección 1: Inventario actual ──────────────────────────────────────
  doc.setFont('helvetica', 'bold'); doc.setFontSize(12); doc.setTextColor(...VERDE_DARK);
  doc.text(`INVENTARIO ACTUAL POR ${labelGrupo}`, 14, y); y += 6;
  ...
  doc.autoTable({
    head: [[labelGrupo[0] + labelGrupo.slice(1).toLowerCase(), 'Productos', 'Stock', 'Valor Compra', 'Valor Venta', 'Ganancia Est.']],
  ...
  historial.forEach(v => {
    ...
    (v.items || []).forEach(it => {
      ...
      ventasPorCat[grupoItem].total    += (it.totalItem !== undefined ? Number(it.totalItem) : (it.cantCobrada !== undefined ? Number(it.cantCobrada)*Number(it.precio||0) : Number(it.cant||0)*Number(it.precio||0)));
      const nomKey = it.nom || '?';
      ventasPorCat[grupoItem].detalle[nomKey] ??= { nom: nomKey, cant: 0, total: 0 };
      ventasPorCat[grupoItem].detalle[nomKey].cant  += Number(it.cant || 0);
      ventasPorCat[grupoItem].detalle[nomKey].total += (it.totalItem !== undefined ? Number(it.totalItem) : (it.cantCobrada !== undefined ? Number(it.cantCobrada)*Number(it.precio||0) : Number(it.cant||0)*Number(it.precio||0)));
    });
  });
  ...
  if (!ventRows.length) {
    ...
    doc.autoTable({
      head: [[labelGrupo[0] + labelGrupo.slice(1).toLowerCase(), 'Prods. Distintos', 'Unidades', 'Total Vendido']],
    ...
    // Detalle por categoría/proveedor
    ventRows.forEach(r => {
  ...
  const nombreArchivo = `Inventario_${catFiltro !== 'todas' ? catFiltro + '_' : ''}${provFiltro !== 'todos' ? provFiltro + '_' : ''}${desde || 'inicio'}_${hasta || 'hoy'}.pdf`;
  if (typeof _descargarPDF === 'function') {
    if (_descargarPDF(doc, nombreArchivo)) toast(`✓ PDF de inventario generado`);
  } else {
