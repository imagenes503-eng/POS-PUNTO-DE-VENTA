...
(function _inyectarEstilosMembresias() {
  ...
  s.textContent = `
    ...
    .memb-scroll-wrap::-webkit-scrollbar-thumb { background: #fed7aa; border-radius: 10px; }
    ...
    .memb-table tbody tr:hover { background: #fffbeb; }
...
async function _cargarYRenderTablaMemb() {
  ...
  try {
    ...
    const filas = _membActivas.map(m => {
      ...
      // Color del avatar
      const avatarColors = [
        'linear-gradient(135deg,#65a30d,#3f6212)',
      ...
      if (!vence) {
        ...
        const fillColor = pct > 40 ? '#ea580c' : pct > 15 ? '#d97706' : '#dc2626';
