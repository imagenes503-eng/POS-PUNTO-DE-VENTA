...
function _fusionarDos(local, ext) {
  ...
  // BUGFIX SYNC (Reiniciar Mes): igual que con historial, respetar tombstone de
  // borrado de pagos/gastos. Antes esto era una unión pura por id, así que un
  // teléfono que NO recibía en vivo el "Reiniciar Mes" mantenía sus gastos
  // viejos para siempre — y encima se los "contagiaba" de vuelta al otro
  // teléfono en la próxima fusión.
  const _pagosWipeLocal = local.pagosWipeTs ? Date.parse(local.pagosWipeTs) : 0;
  const _pagosWipeExt   = ext.pagosWipeTs   ? Date.parse(ext.pagosWipeTs)   : 0;
  const _pagosWipeMax   = Math.max(_pagosWipeLocal, _pagosWipeExt);
  if (_pagosWipeMax > 0) local.pagosWipeTs = new Date(_pagosWipeMax).toISOString();
  local.pagos = (local.pagos||[]).filter(g => _pagosWipeMax === 0 || (g.ts||0) >= _pagosWipeMax);
  const seenP=new Set((local.pagos||[]).map(g=>String(g.id)));
  (ext.pagos||[]).forEach(g=>{
    if(!seenP.has(String(g.id)) && (_pagosWipeMax === 0 || (g.ts||0) >= _pagosWipeMax)){
  ...
  // BUGFIX SYNC (Reiniciar Mes): antes esto SIEMPRE tomaba el mayor entre los
  // dos valores (Math.max), así que un "Reiniciar Mes" que ponía efectivoInicial
  // en 0 JAMÁS podía sobrevivir una fusión con un teléfono que todavía tenía el
  // valor viejo (ej. $50) — Math.max(0,50) siempre daba 50, resucitando el saldo
  // ya reiniciado. Ahora, si hay un reinicio de mes más reciente que el otro
  // lado, se respeta ese reinicio en vez de quedarse con el valor más alto.
  const _mesWipeLocal = local.reinicioMesTs ? Date.parse(local.reinicioMesTs) : 0;
  const _mesWipeExt   = ext.reinicioMesTs   ? Date.parse(ext.reinicioMesTs)   : 0;
  if (_mesWipeLocal > 0 || _mesWipeExt > 0) {
    if (_mesWipeExt > _mesWipeLocal) {
      // El otro dispositivo reinició el mes más recientemente → adoptar su valor tal cual
      local.efectivoInicial = parseFloat(ext.efectivoInicial || 0);
      local.reinicioMesTs   = ext.reinicioMesTs;
    } else {
      // El reinicio local es igual o más reciente → conservar el valor local tal cual
      local.efectivoInicial = parseFloat(local.efectivoInicial || 0);
    }
  } else {
    local.efectivoInicial=Math.max(parseFloat(local.efectivoInicial||0),parseFloat(ext.efectivoInicial||0));
  }
  // BUGFIX (Reiniciar Semana): fusionar el mapa de cortes por categoría,
  // quedándose con el timestamp más reciente por categoría entre ambos lados.
  try {
    const _mapaLocal = local.reinicioSemMapa ? JSON.parse(local.reinicioSemMapa) : {};
    const _mapaExt    = ext.reinicioSemMapa   ? JSON.parse(ext.reinicioSemMapa)   : {};
    const _mapaFinal = { ..._mapaLocal };
    for (const cat in _mapaExt) {
      const eInfo = _mapaExt[cat], lInfo = _mapaFinal[cat];
      if (!lInfo || Date.parse(eInfo.ts) > Date.parse(lInfo.ts)) _mapaFinal[cat] = eInfo;
    }
    local.reinicioSemMapa = JSON.stringify(_mapaFinal);
  } catch(e) {}
...
async function _aplicarDatos(datos) {
  ...
  // BUGFIX SYNC (Reiniciar Mes): guardar los tombstones de historial/pagos/mes
  // que vengan del snapshot fusionado, para que este dispositivo también los
  // respete en futuras fusiones (evita que un "Reiniciar Mes" se pierda al
  // volver a fusionar con un tercer teléfono que aún no se enteró).
  ['historialWipeTs','pagosWipeTs','reinicioMesTs'].forEach(k => {
    if (datos[k]) {
      const actual = localStorage.getItem('vpos_' + k) || '';
      if (datos[k] > actual) localStorage.setItem('vpos_' + k, datos[k]);
    }
  });
  if (datos.reinicioSemMapa) {
    localStorage.setItem('vpos_reinicioSemMapa', datos.reinicioSemMapa);
    if (typeof _recalcularReportesDesdeHistorial === 'function') _recalcularReportesDesdeHistorial();
  }
...
// BUGFIX: si la app arranca con el modo ya activado desde antes, mostrar el
// punto rojo de una vez en vez de esperar a que algo más dispare _dot().
if (_modoSoloLocal && typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => { if (typeof _dot === 'function') _dot('red'); });
  if (document.body && typeof _dot === 'function') _dot('red');
}
...
async function toggleModoSoloLocal() {
  ...
  if (!_modoSoloLocal) {
    ...
    if (navigator.onLine) {
      ...
      if (typeof _dot === 'function') _dot('yellow');
      await _flushOfflineQueue();
      _iniciarPolling();
      if (typeof _autoEnviarSnapshot === 'function') _autoEnviarSnapshot();
    } else {
    ...
    _actualizarUILocalMode();
    if (typeof _dot === 'function') _dot('red');
...
async function _autoEnviarSnapshot() {
  ...
  const snap = {
    ...
    datos: JSON.stringify({
      ...
      // BUGFIX SYNC (Reiniciar Mes): tombstones de pagos y de reinicio de mes completo
      pagosWipeTs:  localStorage.getItem('vpos_pagosWipeTs')  || null,
      reinicioMesTs: localStorage.getItem('vpos_reinicioMesTs') || null,
      reinicioSemMapa: localStorage.getItem('vpos_reinicioSemMapa') || null,
...
async function _autoFusionar() {
  ...
  try {
    ...
    const miSnap = {
      ...
      historial, pagos, ventasDiarias, restockLog: restockLog || [],
      // BUGFIX SYNC: estos tombstones faltaban aquí (solo se subían a Supabase,
      // pero no se usaban en la fusión local en memoria) — se agregan para que
      // esta fusión también los respete simétricamente en ambos lados.
      productosEliminados:     typeof productosEliminados     !== 'undefined' ? (productosEliminados     || []) : [],
      cobrosEliminados:        typeof cobrosEliminados         !== 'undefined' ? (cobrosEliminados         || []) : [],
      ventasDiariasEliminadas: typeof ventasDiariasEliminadas  !== 'undefined' ? (ventasDiariasEliminadas  || []) : [],
      reinicioDiaTs:   localStorage.getItem('vpos_reinicioDiaTs')   || null,
  ...
  try {
    _realtimeChannel = client
      ...
      .on('broadcast', { event: 'reinicio_sem' }, ({ payload }) => {
        ...
        idbSet('vpos_ventasSem', ventasSem).catch(()=>{});
        // BUGFIX (Reiniciar Semana): guardar el mapa de cortes para que este
        // teléfono tampoco resucite la semana vieja en el próximo recálculo.
        if (payload.reinicioSemMapa) {
          localStorage.setItem('vpos_reinicioSemMapa', JSON.stringify(payload.reinicioSemMapa));
        }
        if (typeof actualizarTodo === 'function') actualizarTodo();
        _dot('green');
      ...
      .on('broadcast', { event: 'reinicio_mes' }, ({ payload }) => {
        ...
        ventasDia = {}; ventasSem = {}; ventasMes = {};
        historial = []; pagos = []; ventasDiarias = []; restockLog = restockLog || [];
        // BUGFIX SYNC (Reiniciar Mes): guardar los tombstones de forma permanente
        // en este teléfono, para que ninguna fusión futura resucite datos viejos
        // aunque este teléfono se desconecte y reconecte después.
        ['historialWipeTs','pagosWipeTs','reinicioMesTs'].forEach(k => {
          if (payload[k]) localStorage.setItem('vpos_' + k, payload[k]);
        });
...
function _dot(color){
  ...
  if(!d){d=document.createElement('div');d.id='syncDot';d.style.cssText='position:fixed;top:8px;right:8px;z-index:9999;width:9px;height:9px;border-radius:50%;transition:all .3s;pointer-events:none;';document.body.appendChild(d);}
  // BUGFIX: mientras "Usar sin internet" está activado, el punto debe quedar
  // ROJO de forma FIJA (no naranja parpadeando ni desapareciendo) para que se
  // note claramente que la app está en modo local. Esto tiene prioridad sobre
  // cualquier otro color que un proceso interno intente poner mientras tanto.
  if (typeof _modoSoloLocal !== 'undefined' && _modoSoloLocal) {
    d.style.background = '#ef4444';
    d.style.opacity = '1';
    return;
  }
  d.style.background={yellow:'#f59e0b',green:'#16a34a',red:'#ef4444'}[color]||'#ccc';
  d.style.opacity='1';
  if(color!=='yellow')setTimeout(()=>{ if (!(typeof _modoSoloLocal !== 'undefined' && _modoSoloLocal)) d.style.opacity='0'; },color==='green'?2000:5000);
}
