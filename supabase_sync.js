...
const _SB_URL_DEFAULT = 'https://miutqtdugbgimrgtqhfy.supabase.co';  // ← pega aquí tu Project URL
const _SB_KEY_DEFAULT = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1pdXRxdGR1Z2JnaW1yZ3RxaGZ5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODUxMDQyNTEsImV4cCI6MjEwMDY4MDI1MX0.NaMXuwz_BtZwNIVnp-breDGbtVG_4T5McvJtTOFNZzw';  // ← pega aquí tu anon public key
...
// SYNC FIX: detecta la respuesta "JWT expired" (PGRST303) de Supabase.
function _esJwtExpirado(status, txt) {
  return status === 401 && /PGRST303|JWT expired/i.test(txt || '');
}

// SYNC FIX: refresca el token INMEDIATAMENTE (sin esperar el intervalo de
// 50 min, que se pausa cuando el navegador pone la pestaña en segundo
// plano). Si dos llamadas piden refrescar al mismo tiempo, comparten la
// misma promesa para no duplicar peticiones. Si el refresh falla de verdad
// (refresh_token inválido/expirado), fuerza cierre de sesión y pide re-login
// en vez de seguir fallando en silencio para siempre.
//
// BUGFIX (sesión se cerraba sola al minimizar): Supabase usa refresh
// tokens ROTATIVOS — cada vez que se usan quedan invalidados y se emite
// uno nuevo. Si tienes la app instalada Y la página en el navegador
// abiertas a la vez (comparten el mismo localStorage por ser el mismo
// origen), al volver ambas de segundo plano casi al mismo tiempo, AMBAS
// intentan renovar con el mismo refresh_token viejo: la primera tiene
// éxito y recibe uno nuevo, la segunda llega con el token ya usado y
// Supabase la rechaza (400/401) — aunque la sesión sigue perfectamente
// válida. Antes esto se trataba como "sesión realmente muerta" y se
// borraba todo. Ahora, antes de rendirnos, revisamos si el
// refresh_token guardado en localStorage ya cambió (señal de que OTRA
// pestaña/instancia lo renovó con éxito mientras esperábamos la
// respuesta) y, si es así, simplemente adoptamos ese token nuevo en vez
// de cerrar la sesión.
let _refrescandoToken = null;
const _REFRESH_COOLDOWN_MS = 20000; // no repetir un refresco forzado antes de 20s, ni siquiera entre pestañas distintas

async function _refrescarTokenForzado() {
  if (_refrescandoToken) return _refrescandoToken;

  // Enfriamiento compartido entre pestañas/instancias: si otra
  // pestaña/ventana (misma app, mismo origen) ya refrescó hace muy poco,
  // no repetir la petición — solo tomar el token vigente que dejó.
  const ultimoRefresco = parseInt(localStorage.getItem('vpos_ultimoRefrescoTs') || '0', 10);
  if (Date.now() - ultimoRefresco < _REFRESH_COOLDOWN_MS) {
    const tokenActual = localStorage.getItem('vpos_authToken');
    if (tokenActual) {
      _authToken    = tokenActual;
      _refreshToken = localStorage.getItem('vpos_refreshToken') || _refreshToken;
      return true;
    }
  }
  ...
  _refrescandoToken = (async () => {
    ...
    if (!rt) return { ok: false, invalido: false };
    try {
      ...
      if (data.access_token) {
        ...
        localStorage.setItem('vpos_ultimoRefrescoTs', String(Date.now()));
        return { ok: true, invalido: false };
      }
      // SYNC FIX (race de refresh token rotativo): antes de asumir que
      // el refresh_token es inválido, comprobar si OTRA pestaña/app ya
      // lo renovó exitosamente mientras esta petición estaba en camino.
      // Si el valor guardado en localStorage ya no es el mismo `rt` que
      // usamos, es que se rotó bien en otro lado — no es un cierre de
      // sesión real, solo adoptamos el token vigente.
      const refreshGuardadoAhora = localStorage.getItem('vpos_refreshToken');
      const tokenGuardadoAhora   = localStorage.getItem('vpos_authToken');
      if (refreshGuardadoAhora && refreshGuardadoAhora !== rt && tokenGuardadoAhora) {
        _authToken    = tokenGuardadoAhora;
        _refreshToken = refreshGuardadoAhora;
        return { ok: true, invalido: false };
      }
      // SYNC FIX: solo es un refresh_token realmente inválido si el
      // SERVIDOR respondió y lo rechazó (400/401). Cualquier otra cosa
      // (500, respuesta rara, etc.) se trata como fallo pasajero — no
      // como "tu sesión ya no sirve".
      const invalido = r.status === 400 || r.status === 401;
      return { ok: false, invalido };
    } catch (e) {
      // Fallo de RED (típico justo al reabrir la app, con el WiFi/datos
      // todavía reconectando) — NO significa que el token esté mal, solo
      // que no se pudo verificar todavía. No cerrar sesión por esto.
      return { ok: false, invalido: false };
    }
  })();
  const resultado = await _refrescandoToken;
  _refrescandoToken = null;
  if (!resultado.ok && resultado.invalido) _forzarRelogin();
  return resultado.ok;
}

function _forzarRelogin() {
  // Última verificación de seguridad: puede que OTRA pestaña/instancia
  // (app instalada + navegador abiertos a la vez) ya haya renovado la
  // sesión exitosamente justo antes de que esta decidiera cerrarla.
  // Si el localStorage ya tiene una sesión vigente, respetarla en vez
  // de borrarla — evita el falso "cierre de sesión" al minimizar.
  const tokenVigente   = localStorage.getItem('vpos_authToken');
  const refreshVigente = localStorage.getItem('vpos_refreshToken');
  const sesionVigente  = localStorage.getItem('vpos_sesionActiva') === '1';
  if (tokenVigente && refreshVigente && sesionVigente) {
    _authToken    = tokenVigente;
    _refreshToken = refreshVigente;
    _sesionActiva = true;
    console.warn('[Auth] El refresco falló en esta pestaña, pero otra ya había renovado la sesión con éxito — se mantiene activa.');
    return;
  }
  console.warn('[Auth] La sesión expiró y no se pudo renovar — se requiere iniciar sesión de nuevo.');
  _sesionActiva = false; _authToken = null; _refreshToken = null;
  localStorage.removeItem('vpos_sesionActiva');
  localStorage.removeItem('vpos_authToken');
  localStorage.removeItem('vpos_refreshToken');
  if (typeof toast === 'function') toast('⚠ Tu sesión expiró — vuelve a iniciar sesión', true);
  if (typeof abrirLogin === 'function') setTimeout(() => abrirLogin(), 500);
}

// SYNC FIX: si vuelves a la pestaña/app después de tenerla en segundo plano,
// refresca el token Y los datos de inmediato — no esperar al sondeo de 15s.
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible' && _sesionActiva) {
    _refrescarTokenForzado();
...
async function _sbGet(tabla, params) {
  ...
  if (!resp.ok) {
    ...
    // SYNC FIX: token vencido → refrescar UNA vez y reintentar antes de fallar.
    if (_esJwtExpirado(resp.status, txt) && await _refrescarTokenForzado()) {
...
async function _sbPost(tabla, body, upsert) {
  ...
  let resp = await fetch(url + '/rest/v1/' + tabla, { method: 'POST', headers: h, body: JSON.stringify(body) });
  if (!resp.ok) {
    let txt = await resp.text();
    // SYNC FIX: token vencido → refrescar UNA vez y reintentar antes de fallar.
    if (_esJwtExpirado(resp.status, txt) && await _refrescarTokenForzado()) {
      const h2 = _headers({ 'Prefer': upsert ? 'resolution=merge-duplicates,return=minimal' : 'return=minimal' });
      resp = await fetch(url + '/rest/v1/' + tabla, { method: 'POST', headers: h2, body: JSON.stringify(body) });
      if (resp.ok) return;
      txt = await resp.text();
    }
    throw new Error('HTTP ' + resp.status + ': ' + txt);
...
async function _sbPatch(tabla, filtro, body) {
  ...
  if (!resp.ok) {
    ...
    // SYNC FIX: token vencido → refrescar UNA vez y reintentar antes de fallar.
    if (_esJwtExpirado(resp.status, txt) && await _refrescarTokenForzado()) {
      const h2 = _headers({ 'Prefer': 'return=minimal' });
      resp = await fetch(url + '/rest/v1/' + tabla + qs, { method: 'PATCH', headers: h2, body: JSON.stringify(body) });
      if (resp.ok) return;
      txt = await resp.text();
    }
    throw new Error('HTTP ' + resp.status + ': ' + txt);
    ...
    if (!document.getElementById('modalCerrarSesion')) {
      ...
      m.innerHTML = `
        <div style="background:#fff;border-radius:20px;width:100%;max-width:340px;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
          <div style="background:linear-gradient(135deg,#9a3412,#ea580c);padding:22px 24px 18px;text-align:center;">
  ...
  if (!document.getElementById('loginStyles')) {
    ...
    style.textContent = `
      #modalLogin {
        ...
        background: linear-gradient(135deg, #7c2d12 0%, #9a3412 40%, #9a3412 100%);
      ...
      .login-tab-btn.active { color: #ea580c; border-bottom-color: #ea580c; background: #fffbeb; }
      ...
      .login-btn-main {
        width: 100%; padding: 15px; background: linear-gradient(135deg, #ea580c, #c2410c);
        ...
        box-shadow: 0 4px 20px rgba(234,88,12,0.4); transition: all 0.2s;
        letter-spacing: 0.3px;
      }
      .login-btn-main:hover { transform: translateY(-1px); box-shadow: 0 6px 24px rgba(234,88,12,0.5); }
      ...
      .plan-card:hover { border-color: #fdba74; background: #fffbeb; transform: translateY(-2px); }
      .plan-card.selected { border-color: #ea580c; background: #fffbeb; box-shadow: 0 0 0 3px rgba(234,88,12,0.15); }
      .plan-card.popular::before {
        content: '⭐ Popular'; position: absolute; top: -10px; left: 50%; transform: translateX(-50%);
        background: #ea580c; color: #fff; font-size: 10px; font-weight: 900; font-family: Nunito, sans-serif;
        padding: 3px 10px; border-radius: 20px; white-space: nowrap;
      }
      .plan-icono { font-size: 24px; margin-bottom: 6px; }
      .plan-nombre { font-size: 13px; font-weight: 900; color: #7c2d12; font-family: Nunito, sans-serif; margin-bottom: 4px; }
      .plan-precio { font-size: 22px; font-weight: 900; color: #ea580c; font-family: Nunito, sans-serif; }
      ...
      .pago-metodo-btn:hover { border-color: #fdba74; background: #fffbeb; }
      .pago-metodo-btn.selected { border-color: #1d4ed8; background: #eff6ff; color: #1d4ed8; }
      .membresia-footer { padding: 16px 20px 20px; }
      .membresia-resumen {
        background: #fffbeb; border: 1px solid #fed7aa; border-radius: 12px;
        padding: 12px 16px; margin-bottom: 14px; font-family: Nunito, sans-serif;
      }
      .membresia-resumen .res-row { display: flex; justify-content: space-between; align-items: center; font-size: 13px; color: #9a3412; font-weight: 700; }
      .membresia-resumen .res-row.total { font-size: 16px; font-weight: 900; color: #7c2d12; margin-top: 8px; padding-top: 8px; border-top: 1px solid #fed7aa; }
      .btn-pagar {
        width: 100%; padding: 15px; border: none; border-radius: 14px;
        font-size: 16px; font-weight: 900; font-family: Nunito, sans-serif;
        cursor: pointer; transition: all 0.2s; letter-spacing: 0.3px;
      }
      .btn-pagar.efectivo { background: linear-gradient(135deg, #65a30d, #3f6212); color: #fff; box-shadow: 0 4px 16px rgba(101,163,13,0.4); }
        ...
        <div class="login-field">
          ...
          <div style="text-align:right;margin-top:6px;">
            <button type="button" onclick="_abrirRecuperarPass()"
              style="background:none;border:none;cursor:pointer;font-size:12px;font-weight:700;color:#ea580c;font-family:Nunito,sans-serif;padding:0;text-decoration:underline;">
...
function _actualizarNombreTienda() {
  // Ahora el Super Admin también puede personalizar su nombre. Si nadie
  // ha guardado uno todavía, cada rol conserva su valor por defecto de
  // siempre (igual que antes de este cambio).
  const esSA = _esSuperAdmin();
  const porDefecto = esSA ? 'Despensa Económica' : (_getTiendaId() || 'Mi Tienda');
  const nombre = _usuarioActual?.tienda_nombre || porDefecto;
...
function _aplicarNombreTiendaDOM(nombre) {
  ...
  if (logo) {
    logo.innerHTML = html;
    // Nombres largos: reducir tamaño para que quepan cómodo en 1-2 líneas
    logo.style.fontSize = nombre.length > 22 ? '14px' : nombre.length > 14 ? '16px' : '19px';
  }
...
// ── Editar el nombre de la tienda después de iniciar sesión ──────────
async function editarNombreTienda() {
  if (!_usuarioActual) { toast('Inicia sesión primero', true); return; }

  const actual = _usuarioActual.tienda_nombre || _getTiendaId() || 'Mi Tienda';
  const nuevo = prompt('Nombre de tu tienda (aparece en el menú, encabezado, tickets, PDF y reportes descargados):', actual);
  if (nuevo === null) return; // canceló
  const limpio = nuevo.trim().slice(0, 40);
  if (!limpio) { toast('El nombre no puede quedar vacío', true); return; }
  if (limpio === actual) return;

  // Aplicar de inmediato en toda la UI (offline-first, igual que el resto de la app)
  _usuarioActual.tienda_nombre = limpio;
  localStorage.setItem('vpos_usuarioData', JSON.stringify(_usuarioActual));
  localStorage.setItem('vpos_tiendaNombre', limpio);
  _aplicarNombreTiendaDOM(limpio);
  toast('✓ Nombre de tienda actualizado');

  // Sincronizar con Supabase para que quede igual en todos los dispositivos
  try {
    await _sbPatch('perfiles', { id: 'eq.' + _usuarioActual.id }, { tienda_nombre: limpio });
  } catch (e) {
    console.warn('[editarNombreTienda] No se pudo sincronizar aún, se reintentará al reconectar:', e.message);
  }
}
  ...
  if (!document.getElementById('modalRecuperarPass')) {
    ...
    m.innerHTML = `
      <div style="background:#fff;border-radius:20px;width:100%;max-width:400px;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
        <div style="background:linear-gradient(135deg,#ea580c,#c2410c);padding:22px 24px 18px;display:flex;align-items:center;gap:12px;">
        ...
        <div style="padding:22px 24px;">
          <div style="margin-bottom:16px;">
            ...
            <input id="recPassEmail" type="email" placeholder="tu@correo.com"
              style="width:100%;padding:12px 14px;border:1.5px solid #ecfccb;border-radius:11px;font-size:15px;font-weight:700;font-family:Nunito,sans-serif;box-sizing:border-box;outline:none;">
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
            ...
            <button id="btnEnviarRecPass" onclick="_enviarRecuperarPass()"
              style="padding:13px;background:linear-gradient(135deg,#ea580c,#c2410c);border:none;border-radius:12px;font-size:14px;font-weight:900;font-family:Nunito,sans-serif;cursor:pointer;color:#fff;box-shadow:0 4px 14px rgba(234,88,12,0.3);">
              📧 Enviar enlace
            </button>
          </div>
        </div>
  ...
  try {
    ...
    if (msg) {
      ...
      if (resp.ok || resp.status === 200) {
        msg.style.background = '#fffbeb'; msg.style.color = '#c2410c';
...
function _actualizarBadgeLogin() {
  ...
  document.querySelectorAll('.login-status').forEach(el => {
    ...
    el.style.color = activa ? '#ea580c' : '#6b7280';
  });
...
async function restaurarSesion() {
  ...
  try {
    ...
    // SYNC FIX: esto ANTES vivía solo dentro del try, así que si la consulta
    // de "perfiles" fallaba (típico justo al reconectar desde estar offline,
    // con la red aún inestable), NUNCA se arrancaba la carga de datos frescos
    // ni el polling — el teléfono se quedaba con el caché viejo para siempre,
    // hasta que otra acción (como abrir Ventas por Día) disparara otra ruta
    // que sí trajera datos. Ahora esto corre SIEMPRE, sin importar si la
    // consulta de perfiles tuvo éxito o no.
    _iniciarRefreshToken();
    _autoCargarDesdeSupa();
    _iniciarPolling();
    setTimeout(_autoFusionar, 300);
    setTimeout(() => { if (typeof renderCajaPanelMini === 'function') renderCajaPanelMini(); }, 1800);
  }
}
...
async function _autoCargarDesdeSupa() {
  ...
  try {
      ...
      const merged = sbProds.map(sp => {
        ...
        if (local) {
          ...
          if (sbTs > localTs) {
            ...
            console.log(`[Diag-STOCK] AutoCarga — prod ${sp.id}: local stock=${local.stock} _ts=${localTs} → supabase stock=${sp.stock} _ts=${sbTs} | APLICA`);
            return {
              ...sp,
    ...
    if (vd && vd.length > 0) {
      ...
      const _tid = _getTiendaId();
      console.log('[Diag-VD] Filas recibidas de Supabase:', JSON.stringify(vd));
      console.log('[Diag-VD] ventasDiarias local ANTES de mezclar:', JSON.stringify(ventasDiarias));
      vd.forEach(row => {
        ...
        if (!fechaLimpia || !/^\d{4}-\d{2}-\d{2}$/.test(fechaLimpia)) { console.log('[Diag-VD] fila descartada, fecha inválida:', row.fecha); return; }
        // BUGFIX SYNC: ignorar solo si el dato es igual o más viejo que cuando
        // se borró esa fecha localmente — una venta nueva (o una devolución)
        // hecha DESPUÉS del borrado (ej. tras Reiniciar Mes) sí debe pasar.
        if (typeof _vdEstaBloqueado === 'function' && _vdEstaBloqueado(fechaLimpia, row._ts)) { console.log('[Diag-VD] fila descartada, tombstone vigente:', fechaLimpia); return; }
        const idx = ventasDiarias.findIndex(v => v.fecha === fechaLimpia);
        if (idx >= 0) {
          // SYNC FIX: comparar por _ts (última escritura gana), NO por magnitud.
          // Antes solo se aceptaba si el monto de Supabase era MAYOR, lo que
          // funcionaba para ventas nuevas pero bloqueaba para siempre que una
          // devolución (que BAJA el monto) se propagara por este respaldo
          // automático cuando el broadcast en vivo se perdía.
          const sbTs = Number(row._ts) || 0;
          const localTs = Number(ventasDiarias[idx]._ts) || 0;
          console.log(`[Diag-VD] ${fechaLimpia}: local monto=${ventasDiarias[idx].monto} _ts=${localTs} | supabase monto=${sbMonto} _ts=${sbTs} | ${sbTs>localTs?'APLICA':'IGNORA'}`);
          if (sbTs > localTs) {
            ventasDiarias[idx].monto = sbMonto;
            ventasDiarias[idx]._ts = sbTs;
          } else if (localTs > sbTs && sbMonto !== ventasDiarias[idx].monto) {
            // SYNC FIX: el local es más reciente pero Supabase sigue con el
            // valor viejo — significa que una subida anterior falló y nunca
            // se reintentó (ej. un corte de red breve). Volver a subirla
            // aquí corrige el atasco solo, sin esperar otra venta.
            console.log(`[Diag-VD] ${fechaLimpia}: local más nuevo que Supabase pero no coincide — re-subiendo`);
          ...
          console.log(`[Diag-VD] ${fechaLimpia}: no existía local, se agrega monto=${sbMonto} _ts=${row._ts}`);
          // Fecha que no existe localmente → agregarla desde Supabase
          ventasDiarias.push({ fecha: fechaLimpia, monto: sbMonto, nota: row.nota || '', _ts: Number(row._ts) || 0 });
        }
      });
      ventasDiarias.sort((a, b) => a.fecha.localeCompare(b.fecha));
      console.log('[Diag-VD] ventasDiarias local DESPUÉS de mezclar:', JSON.stringify(ventasDiarias));
      idbSet('vpos_ventasDiarias', ventasDiarias).catch(() => {});
    } else {
      console.log('[Diag-VD] La consulta a ventas_diarias no devolvió filas (vd=' + JSON.stringify(vd) + ')');
    }
    ...
    if (cfg && cfg.length > 0) {
      cfg.forEach(row => {
        ...
        // SYNC FIX (red de respaldo para Reiniciar Mes): si el broadcast en
        // vivo 'reinicio_mes' se perdió porque el canal Realtime estaba caído
        // en ese momento, este sondeo periódico (que sí es confiable, ya
        // corre siempre como parte de AutoCarga) detecta que otro teléfono
        // hizo un reinicio más nuevo que el que tenemos guardado localmente,
        // y lo aplica reutilizando la MISMA función del botón (remoto=true).
        if (row.clave === 'reinicioMesTs' && row.valor) {
          const tsLocal = localStorage.getItem('vpos_reinicioMesTs') || '';
          if (row.valor > tsLocal) {
            localStorage.setItem('vpos_reinicioMesTs', row.valor);
            if (typeof ejecutarReiniciarMes === 'function') ejecutarReiniciarMes(true);
          }
        }
      });
    }
    ...
    // SYNC FIX: usar la MISMA ruta que ya funciona siempre al entrar a
    // "Ventas por Día" — recalcular ventasDiarias de HOY directamente desde
    // ventasDia (recién reconstruido arriba desde la tabla "ventas", la
    // fuente más confiable). Esto es un respaldo independiente de la fila de
    // "ventas_diarias" en Supabase, que puede llegar en carrera justo cuando
    // el otro teléfono acaba de vender. Solo SUBE el monto si es mayor —
    // nunca interfiere con una devolución que lo baja (esa la maneja el
    // merge de _ts de arriba).
    ...
    // SYNC FIX: si algún paso intermedio falló y saltó al catch de arriba,
    // esto garantiza que la pantalla igual se repinte con lo que sí se haya
    // alcanzado a mezclar — antes, un error a medio camino dejaba Caja (y el
    // resto del dashboard) congelados con datos viejos sin ningún aviso.
...
function _quitarRestriccionesPorRol() {
  ...
  // Ocultos permanentemente por decisión del dueño de la tienda — no
  // deben reaparecer al cerrar sesión.
  ['dniSync','btnModoLocalDrawer'].forEach(id => {
    const el = document.getElementById(id); if (el) el.style.display = 'none';
  });
  document.querySelectorAll('.nav-tab[onclick*="pgSync"]').forEach(el => { el.style.display = 'none'; });
  const btnBackup = document.querySelector('.backup-bar .btn-backup');
  if (btnBackup) btnBackup.style.display = 'none';
  document.querySelectorAll('.backup-bar .btn-restore').forEach(btn => {
    const oc = btn.getAttribute('onclick') || '';
    if (oc.includes('inputImportar') || oc.includes('inputFusionar') || oc.includes('abrirModalCodigo') || oc.includes('abrirConfigSheets')) {
      btn.style.display = 'none';
    }
  });
}
...
function _fusionarDos(local, ext) {
  ...
  // Ahora cada tombstone es {f, ts} — al unir, si la misma fecha aparece en
  // ambos lados se conserva la marca de tiempo MÁS RECIENTE. Y ya no se borra
  // ciegamente cualquier entrada con esa fecha: solo se borra si es igual o
  // más vieja que el tombstone (una venta nueva hecha después del borrado,
  // ej. tras Reiniciar Mes, debe sobrevivir la fusión).
  const _vdTomb = new Map();
  const _normVd = x => typeof x === 'string' ? { f: x, ts: 0 } : { f: x.f, ts: Number(x.ts) || 0 };
  (local.ventasDiariasEliminadas||[]).map(_normVd).forEach(t => { if (!_vdTomb.has(t.f) || t.ts > _vdTomb.get(t.f)) _vdTomb.set(t.f, t.ts); });
  (ext.ventasDiariasEliminadas||[]).map(_normVd).forEach(t => { if (!_vdTomb.has(t.f) || t.ts > _vdTomb.get(t.f)) _vdTomb.set(t.f, t.ts); });
  local.ventasDiariasEliminadas = [..._vdTomb.entries()].map(([f, ts]) => ({ f, ts }));
  // Eliminar del local solo las fechas cuyo dato es igual o más viejo que su tombstone
  local.ventasDiarias = local.ventasDiarias.filter(v => {
    const tombTs = _vdTomb.get(v.fecha);
    if (tombTs === undefined) return true;
    return Number(v._ts || 0) > tombTs;
  });
  (ext.ventasDiarias||[]).forEach(vExt=>{
    const tombTs = _vdTomb.get(vExt.fecha);
    if (tombTs !== undefined && Number(vExt._ts || 0) <= tombTs) return; // dato viejo, ya fue borrado después
    const idx=local.ventasDiarias.findIndex(vL=>vL.fecha===vExt.fecha);
    if(idx>=0){
      // Última escritura gana por _ts (permite que bajen los montos por devoluciones)
      if (Number(vExt._ts || 0) >= Number(local.ventasDiarias[idx]._ts || 0)) {
        local.ventasDiarias[idx].monto = Number(vExt.monto) || 0;
        local.ventasDiarias[idx]._ts = Number(vExt._ts) || 0;
      }
    }
  ...
  (local.productos||[]).forEach(p=>{
    ...
    if (!extProd) return;
    // SYNC FIX: la heurística anterior comparaba qué cobros existían solo en
    // un lado del historial para "adivinar" cuánto se vendió y recalcular el
    // stock desde ahí. Eso no distinguía una devolución PARCIAL (el cobro
    // sigue existiendo en ambos lados, solo con menos items) de una venta
    // real, y terminaba sumando el stock devuelto una segunda vez encima del
    // que ya se había sincronizado por la vía directa (Supabase). Ahora se
    // usa el mismo criterio de "última escritura gana" por _ts que ya se usa
    // en productos, ventas_diarias, etc. — simple y sin doble conteo.
    const tsLocal = Number(p._ts) || 0;
    const tsExt   = Number(extProd._ts) || 0;
    if (tsExt > tsLocal) {
      console.log(`[Diag-STOCK] Fusion (_aplicarDatos) — prod ${pid}: local stock=${p.stock} _ts=${tsLocal} → ext stock=${extProd.stock} _ts=${tsExt} | APLICA`);
      p.stock = Number(extProd.stock) || 0;
      p._ts   = tsExt;
    }
  });
...
async function _aplicarDatos(datos) {
  ...
  { const _vdSnap = datos.ventasDiarias || [];
    ...
    _vdSnap.forEach(vExt => {
      ...
      // SYNC FIX: comparar por _ts (última escritura gana) en vez de por
      // magnitud, para que una devolución (monto menor) también se aplique
      // correctamente al fusionar snapshots de otros dispositivos.
      const extTs = Number(vExt._ts) || 0;
      if (idx >= 0) {
        const localTs = Number(ventasDiarias[idx]._ts) || 0;
        if (extTs > localTs) {
          ventasDiarias[idx].monto = Number(vExt.monto) || 0;
          ventasDiarias[idx]._ts = extTs;
  ...
  if(typeof normalizeHistorial==='function') historial=normalizeHistorial(historial);
  if(typeof normalizePagos==='function') pagos=normalizePagos(pagos);
  // SYNC FIX: repintar Estado de Caja explícitamente aquí. Antes dependía de
  // que actualizarTodo() se llamara desde otro lado después de esta fusión;
  // si eso no pasaba a tiempo, la Caja del dashboard se quedaba mostrando el
  // valor viejo hasta que el usuario cambiaba de página manualmente.
  if (typeof renderCajaPanelMini === 'function') renderCajaPanelMini();
...
function _iniciarPolling() {
  ...
  // SYNC FIX: antes este respaldo solo corría si el canal Realtime estaba
  // caído (!_realtimeActivo). Pero el canal se reconecta seguido (CHANNEL_ERROR
  // intermitente) y un broadcast puede perderse justo en ese instante aunque
  // "activo" vuelva a true enseguida — sin quedar ningún mecanismo que lo
  // rescate. Ahora este refresco corre SIEMPRE cada 15s, sin importar el
  // estado del canal, trayendo directamente los datos reales de Supabase
  // (ventas, productos, etc.) — así cualquier venta/cambio perdido en el
  // camino se autocorrige solo en unos segundos como máximo.
  _pollingTimer = setInterval(() => {
    if (!_realtimeActivo) { _autoFusionar(); }
    if (typeof _autoCargarDesdeSupa === 'function') _autoCargarDesdeSupa();
    // SYNC FIX: cuando una subida falla por un corte de red pasajero (no un
    // "desconectado" completo que dispare el evento 'online' del navegador),
    // la operación queda en la cola offline esperando para siempre — nada la
    // reintentaba salvo recargar la página. Ahora se reintenta cada 15s junto
    // con el resto del respaldo, así una venta que no se subió por un corte
    // breve se corrige sola sin depender de una recarga manual.
    if (typeof _flushOfflineQueue === 'function') _flushOfflineQueue();
  }, 15000);
  console.log('[Sync] Realtime iniciado, respaldo de sondeo cada 15s (siempre activo)');
}
  ...
  try {
    _realtimeChannel = client
      ...
      .on('broadcast', { event: 'historial_actualizado' }, ({ payload }) => {
        ...
        if (payload.productos_devolucion && Array.isArray(payload.productos_devolucion)) {
          payload.productos_devolucion.forEach(updatedProd => {
            ...
            if (idx >= 0) {
              console.log(`[Diag-STOCK] Broadcast historial_actualizado — prod ${updatedProd.id}: local stock=${productos[idx].stock} → nuevo=${updatedProd.stock}`);
              const imgLocal = productos[idx].img;
      ...
      .on('broadcast', { event: 'venta_diaria_actualizada' }, ({ payload }) => {
        ...
        (payload.ventasDiarias || []).forEach(vExt => {
          ...
          if (idx >= 0) {
            ...
            // SYNC FIX: antes solo se estampaba _ts "if (vExt._ts)" — si el
            // payload llegaba sin _ts (o en 0) el monto cambiaba pero la
            // marca de tiempo se quedaba vieja, y el siguiente merge por _ts
            // en otro teléfono descartaba este cambio por parecer "viejo".
            // Ahora, si el monto cambió, SIEMPRE se refresca _ts.
            if (Number(vExt.monto) !== Number(ventasDiarias[idx].monto)) {
              ventasDiarias[idx].monto = Number(vExt.monto) || 0;
              ventasDiarias[idx]._ts   = Number(vExt._ts) || Date.now();
            }
            if (vExt.nota !== undefined) ventasDiarias[idx].nota = vExt.nota;
          } else {
            ventasDiarias.push({ fecha: vExt.fecha, monto: Number(vExt.monto) || 0, nota: vExt.nota || '', _ts: Number(vExt._ts) || Date.now() });
          }
        });
        ventasDiarias.sort((a, b) => a.fecha.localeCompare(b.fecha));
        idbSet('vpos_ventasDiarias', ventasDiarias).catch(()=>{});
      ...
      .on('broadcast', { event: 'ventas_dia_eliminada' }, ({ payload }) => {
        if (!payload) return;
        // BUGFIX SYNC: guardar tombstone recibido (con hora) para que _fusionarDos
        // tampoco restaure esa fecha, pero sin bloquear ventas nuevas posteriores.
        if (typeof ventasDiariasEliminadas !== 'undefined') {
          const _normVd = x => typeof x === 'string' ? { f: x, ts: 0 } : { f: x.f, ts: Number(x.ts) || 0 };
          const _lista = payload.ventasDiariasEliminadas || (payload.fecha ? [{ f: payload.fecha, ts: Number(payload.ts) || Date.now() }] : []);
          _lista.map(_normVd).forEach(t => {
            const idx = ventasDiariasEliminadas.findIndex(x => (typeof x === 'string' ? x : x.f) === t.f);
            if (idx >= 0) {
              const actual = _normVd(ventasDiariasEliminadas[idx]);
              if (t.ts > actual.ts) ventasDiariasEliminadas[idx] = t;
            } else {
              ventasDiariasEliminadas.push(t);
            }
          });
          idbSet('vpos_ventasDiariasElim', ventasDiariasEliminadas).catch(() => {});
        }
        if (!payload.fecha) { if (typeof actualizarTodo === 'function') actualizarTodo(); return; }
      ...
      // SYNC FIX: antes este listener duplicaba manualmente solo una parte de
      // la lógica de reinicio (no tocaba productos[].lotes, inventarioCosto,
      // finanzas del mes, etc.), por eso el segundo teléfono quedaba
      // desincronizado. Ahora reutiliza la MISMA función que usa el botón
      // (ejecutarReiniciarMes, definida en app.js), pasando remoto=true para
      // que no pida confirmación, no vuelva a descargar el PNG/JSON, y no
      // vuelva a emitir broadcasts (evita el bucle A→B→A→B...).
...
function _aplicarVentaRealtime(v) {
  ...
  if (typeof autoRegistrarVentaDiaria === 'function') autoRegistrarVentaDiaria();
  // SYNC FIX: repintado final incondicional — autoRegistrarVentaDiaria puede
  // saltarse su propio render en algunos casos; esto garantiza que Caja
  // siempre quede correcta tras recibir una venta de otro teléfono.
  if (typeof renderCajaPanelMini === 'function') renderCajaPanelMini();
  _dot('green');
}
...
async function _subirVentasDiarias(){
  ...
  await _sbPost('ventas_diarias',ventasDiarias.map(v=>({fecha:tid+'_'+v.fecha,tienda_id:tid,monto:parseFloat(v.monto)||0,nota:v.nota||'',_ts:Number(v._ts)||Date.now()})),true);
}
...
function actualizarBadgeSheets(){const url=_sbUrl(),sesion=_sesionActiva&&_tiendaId;document.querySelectorAll('.sheets-status').forEach(el=>{if(sesion){el.textContent='Sync: '+_tiendaId;el.style.color='#ea580c';}else{el.textContent=url?'Sin sesion':'Sync';el.style.color=url?'#d97706':'#6b7280';}});document.querySelectorAll('.login-status').forEach(el=>{if(sesion){el.textContent='Sync: '+_tiendaId;el.style.color='#ea580c';}else{el.textContent='Iniciar sesion';el.style.color='#6b7280';}});}
...
function _mostrarConfirmacionEfectivo() {
  ...
  overlay.innerHTML = `
    <div style="background:#fff;border-radius:24px;padding:36px 28px;max-width:420px;width:100%;text-align:center;box-shadow:0 32px 80px rgba(0,0,0,0.5);">
      ...
      <h2 style="font-size:20px;font-weight:900;color:#7c2d12;font-family:Nunito,sans-serif;margin:0 0 12px;">Solicitud Registrada</h2>
      <p style="font-size:14px;color:#a16207;font-weight:700;font-family:Nunito,sans-serif;line-height:1.6;margin:0 0 20px;">
      ...
      <div style="background:#fffbeb;border:1px solid #fed7aa;border-radius:12px;padding:14px;margin-bottom:20px;font-size:13px;color:#9a3412;font-weight:700;font-family:Nunito,sans-serif;text-align:left;line-height:1.8;">
      ...
      <button onclick="document.getElementById('confirmacionEfectivo').remove(); _volverAlLogin();"
        style="background:linear-gradient(135deg,#ea580c,#c2410c);color:#fff;border:none;border-radius:12px;padding:14px 28px;font-size:15px;font-weight:900;font-family:Nunito,sans-serif;cursor:pointer;width:100%;">
...
function _mostrarConfirmacionTarjeta() {
  ...
  overlay.innerHTML = `
    <div style="background:#fff;border-radius:24px;padding:36px 28px;max-width:420px;width:100%;text-align:center;box-shadow:0 32px 80px rgba(0,0,0,0.5);">
      <div style="font-size:56px;margin-bottom:16px;">💳</div>
      <h2 style="font-size:20px;font-weight:900;color:#7c2d12;font-family:Nunito,sans-serif;margin:0 0 12px;">Pago con Tarjeta</h2>
...
// Oculta el botón "Ver Código" (deshabilitado a pedido del usuario — se
// mantiene siempre oculto, sin importar el rol)
...
async function adminCargarMembresias() {
  ...
  try {
    ...
    container.innerHTML = rows.map(m => {
      ...
      const tagBg = activa ? '--mpt-bg:#ffedd5;--mpt-c:#ea580c;--mpt-b:#fed7aa'
      ...
      return `
        <div class="mem-row">
          <div class="mem-avatar" style="background:linear-gradient(135deg,${activa?'#65a30d,#3f6212':pendiente?'#d97706,#b45309':'#9ca3af,#6b7280'})">${inicial}</div>
          <div class="mem-info">
            <div class="mem-name">${m.nombre || '—'}</div>
