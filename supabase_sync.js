...
const _SB_URL_DEFAULT = 'https://nujydecjvolpddzfphzd.supabase.co';  // ← pega aquí tu Project URL
const _SB_KEY_DEFAULT = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im51anlkZWNqdm9scGRkemZwaHpkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODQ1ODU5NjEsImV4cCI6MjEwMDE2MTk2MX0.FAf8LHNkdQw-bWudYSA2TjPHJ3BU93NH08gs7CzhgRw';  // ← pega aquí tu anon public key
...
// ── Renovar el JWT ahora mismo (no esperar al intervalo de 50 min) ──────
// Deduplicado: si ya hay una renovación en curso, todos los que la pidan
// reciben la MISMA promesa en vez de disparar varios refresh en paralelo.
let _refrescandoTokenPromise = null;
async function _refrescarTokenAhora() {
  if (_refrescandoTokenPromise) return _refrescandoTokenPromise;
  _refrescandoTokenPromise = (async () => {
    ...
    try {
      ...
      console.warn('[Auth] Refresh token falló:', e.message);
      return false;
    } finally {
      _refrescandoTokenPromise = null;
    }
  })();
  return _refrescandoTokenPromise;
}

// ── BUGFIX SYNC: "Error al sincronizar productos... JWT expired" y en
// general "ya no funciona nada en tiempo real" — el JWT de sesión de
// Supabase dura ~1 hora. Antes la ÚNICA renovación era un setInterval cada
// 50 min, y los navegadores móviles retrasan o directamente pausan esos
// timers cuando la pestaña queda en segundo plano o la pantalla se
// bloquea — algo constante en un POS que no se usa todo el tiempo. Cuando
// el token vencía sin que el intervalo llegara a renovarlo, CADA llamada a
// Supabase (subir una venta, traer productos, etc.) empezaba a fallar con
// 401, y como todas esas llamadas ya tenían su error atrapado en .catch(),
// fallaban en silencio — daba la sensación de que la sincronización en
// tiempo real "se apagó sola". Ahora toda llamada REST pasa por acá: si
// Supabase responde 401, se intenta renovar el token una vez y se repite
// la MISMA petición con el token nuevo antes de darla por perdida.
async function _sbFetch(url, options) {
  let resp = await fetch(url, options);
  if (resp.status === 401) {
    const renovado = await _refrescarTokenAhora();
    if (renovado) {
      const headersNuevos = Object.assign({}, options && options.headers, { 'Authorization': 'Bearer ' + _authToken });
      resp = await fetch(url, Object.assign({}, options, { headers: headersNuevos }));
    }
  }
  return resp;
}
...
async function _sbGet(tabla, params) {
  ...
  const resp = await _sbFetch(url + '/rest/v1/' + tabla + qs, { headers: _headers({'Prefer': ''}) });
...
async function _sbRpc(nombreFuncion, params) {
  const url = _sbUrl(), key = _sbKey();
  if (!url || !key) throw new Error('Sin configuración de Supabase');
  const resp = await _sbFetch(url + '/rest/v1/rpc/' + nombreFuncion, {
    method: 'POST',
...
async function _sbPost(tabla, body, upsert) {
  ...
  const resp = await _sbFetch(url + '/rest/v1/' + tabla, { method: 'POST', headers: h, body: JSON.stringify(body) });
  if (!resp.ok) { const txt = await resp.text(); throw new Error('HTTP ' + resp.status + ': ' + txt); }
}
...
async function _sbPatch(tabla, filtro, body) {
  ...
  const resp = await _sbFetch(url + '/rest/v1/' + tabla + qs, {
...
async function _sbDeleteFiltro(tabla, filtro) {
  const url = _sbUrl(), key = _sbKey();
  if (!url || !key) return;
  await _sbFetch(url + '/rest/v1/' + tabla + '?' + new URLSearchParams(filtro).toString(), {
...
async function _finalizarLogin() {
  ...
  // FIX VELOCIDAD: 800ms -> 300ms, no hay razón para esperar tanto aquí.
...
// ── Renovación automática de JWT (cada 50 min, + al volver a primer plano) ──
// BUGFIX SYNC: un setInterval de 50 min es el único disparador que había antes.
// En un celular, cuando la pantalla se bloquea o el navegador queda en segundo
// plano, ese timer se retrasa o se pausa — así que el token podía vencer sin
// que nadie lo renovara hasta que la pantalla volviera a encenderse, y para
// entonces ya habían pasado varios "Error 401: JWT expired" silenciosos.
// Ahora, además del intervalo, cada vez que la pestaña vuelve a estar visible
// se intenta renovar de una — es barato (una sola llamada) y _refrescarTokenAhora()
// ya evita que se disparen varias renovaciones en paralelo.
function _iniciarRefreshToken() {
  clearInterval(_refreshInterval);
  _refreshInterval = setInterval(() => { _refrescarTokenAhora(); }, 50 * 60 * 1000); // 50 minutos
  if (!window._vposVisibilityRefreshInit) {
    window._vposVisibilityRefreshInit = true;
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible' && _sesionActiva) _refrescarTokenAhora();
    });
  }
}
...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    // BUGFIX SYNC: esta función pull se llama "fuente de verdad", pero el chequeo
    // ".length > 0" trataba "Supabase realmente no tiene ventas" (ej: justo después
    // de un Reiniciar Mes en otro teléfono) igual que "falló la consulta" — en ambos
    // casos no hacía nada y dejaba el historial local viejo intacto. _sbGet devuelve
    // [] (array vacío) en una consulta exitosa sin filas, y solo null si el fetch
    // falló (por el .catch(() => null) de arriba), así que alcanza con chequear que
    // la consulta haya funcionado, sin importar cuántas filas trajo.
    if (ventas) {
    ...
    // BUGFIX SYNC: mismo problema que con ventas — [] real de Supabase también debe reflejarse (pagos vacío), no solo ignorarse.
    if (pagosData) {
    ...
    // BUGFIX SYNC: idem — reflejar también cuando Supabase legítimamente ya no tiene restock log.
    if (restock) {
    ...
    if (vd && vd.length > 0) {
      ...
      vd.forEach(row => {
        ...
        if (idx >= 0) {
          // BUGFIX: comparar por _ts (misma regla que el resto de los puntos de
          // sincronización — ver _ventaDiariaGanadora) en vez de solo por monto.
          // "Solo si es mayor" impedía que una devolución (que baja el monto a
          // propósito) se reflejara acá; y compararlas sin ningún criterio corría
          // el riesgo de pisar una venta recién hecha con una fila de Supabase que
          // todavía no terminaba de escribirse.
          const ganador = _ventaDiariaGanadora(ventasDiarias[idx], { monto: sbMonto, nota: row.nota, _ts: Number(row.ts) || 0 });
          ventasDiarias[idx] = { ...ventasDiarias[idx], monto: ganador.monto, _ts: ganador._ts || ventasDiarias[idx]._ts };
          ...
          ventasDiarias.push({ fecha: fechaLimpia, monto: sbMonto, nota: row.nota || '', _ts: Number(row.ts) || 0 });
        }
      });
      ventasDiarias.sort((a, b) => a.fecha.localeCompare(b.fecha));
      idbSet('vpos_ventasDiarias', ventasDiarias).catch(() => {});
    }
    // BUGFIX SYNC: el bloque de arriba solo agrega/actualiza fechas que trae
    // Supabase, nunca quita una fecha local que ya no debería estar — por eso se
    // purga acá cualquier fecha marcada en el tombstone (ej: por un Reiniciar Mes
    // que este teléfono se perdió), corra o no el bloque de arriba.
    if (typeof ventasDiariasEliminadas !== 'undefined' && ventasDiariasEliminadas.length && ventasDiarias.length) {
      const _vdElimSet = new Set(ventasDiariasEliminadas);
      const _antesVd = ventasDiarias.length;
      ventasDiarias = ventasDiarias.filter(v => !_vdElimSet.has(v.fecha));
      if (ventasDiarias.length !== _antesVd) idbSet('vpos_ventasDiarias', ventasDiarias).catch(() => {});
    }
    ...
    if (cfg && cfg.length > 0) {
      cfg.forEach(row => {
        if (row.clave === 'efectivoInicial') {
          // BUGFIX SYNC (Estado de Caja): esto pisaba efectivoInicial con lo que
          // hubiera en la tabla config, SIN comparar contra el valor local — y
          // esto corre en CADA carga de página, justo después de aplicar el
          // snapshot. Si por lo que sea la tabla config iba un paso atrás del
          // valor real (ej: se actualizó por otra vía sin pasar por _subirConfig),
          // esta línea deshacía la protección de _aplicarDatos() unos milisegundos
          // después, en la misma secuencia de arranque. Mismo criterio en todos
          // lados ahora: tomar el mayor.
          efectivoInicial = Math.max(parseFloat(efectivoInicial) || 0, parseFloat(row.valor) || 0);
        ...
        if (row.clave === 'inventarioInicial') {
          inventarioInicial = Math.max(parseFloat(inventarioInicial) || 0, parseFloat(row.valor) || 0);
        ...
        if (row.clave === 'inventarioCosto') {
          inventarioCosto = Math.max(parseFloat(inventarioCosto) || 0, parseFloat(row.valor) || 0);
...
async function _ejecutarLimpiarSupabase() {
  ...
  try {
    ...
    for (const t of tablas) {
      await _sbFetch(_sbUrl()+'/rest/v1/'+t+'?id=neq.null',{method:'DELETE',headers:_headers({'Prefer':'return=minimal'})}).catch(()=>{});
      await _sbFetch(_sbUrl()+'/rest/v1/'+t+'?fecha=neq.null',{method:'DELETE',headers:_headers({'Prefer':'return=minimal'})}).catch(()=>{});
    }
    await _sbFetch(_sbUrl()+'/rest/v1/ventas_diarias?fecha=neq.null',{method:'DELETE',headers:_headers({'Prefer':'return=minimal'})}).catch(()=>{});
    await _sbFetch(_sbUrl()+'/rest/v1/productos?id=neq.null',{method:'DELETE',headers:_headers({'Prefer':'return=minimal'})}).catch(()=>{});

    // ── Limpiar finanzas y cierres en Supabase ──────────────────────────
    const tid = _getTiendaId();
    if (tid) {
      await _sbFetch(_sbUrl()+'/rest/v1/finanzas_mes?tienda_id=eq.'+encodeURIComponent(tid),{method:'DELETE',headers:_headers({'Prefer':'return=minimal'})}).catch(()=>{});
      await _sbFetch(_sbUrl()+'/rest/v1/cierre_diario?tienda_id=eq.'+encodeURIComponent(tid),{method:'DELETE',headers:_headers({'Prefer':'return=minimal'})}).catch(()=>{});
    }
...
// =====================================================================
//  VENTAS POR DÍA — regla única de fusión
// ---------------------------------------------------------------------
//  Antes, cada punto de sincronización de ventasDiarias (el broadcast de
//  venta_diaria_actualizada, el de historial_actualizado, _fusionarDos,
//  _aplicarDatos y la lectura de la tabla de Supabase) tenía su PROPIA
//  lógica para decidir qué monto quedarse — y no todas eran iguales:
//  algunas no comparaban nada (pisaban sin más), otras solo dejaban subir
//  el monto nunca bajarlo. Ese último criterio parecía razonable (una
//  venta nueva suma) pero rompe el caso real de una DEVOLUCIÓN, que resta
//  del total a propósito — un monto más chico ahí es tan legítimo como uno
//  más grande. Esa mezcla de reglas distintas es lo que causaba que
//  "Estado de Caja"/"Ventas por Día" aparecieran bien un instante y luego
//  se corrigieran hacia el valor equivocado según qué sincronización corriera
//  primero, o que una devolución no se propagara al otro teléfono.
//
//  Ahora hay UNA sola regla, en un solo lugar: gana quien tenga el _ts
//  (marca de tiempo de cuándo se escribió ese monto) más reciente — sin
//  importar si es mayor o menor que el otro. Si a alguno le falta _ts
//  (datos viejos, o la fila viene de la tabla de Supabase sin esa columna)
//  se prefiere el lado que sí tenga marca de tiempo; si ninguno tiene, se
//  usa el monto mayor como último respaldo (el criterio que ya existía).
// =====================================================================
function _ventaDiariaGanadora(local, ext) {
  if (!local) return ext;
  if (!ext) return local;
  const tsLocal = Number(local._ts) || 0;
  const tsExt   = Number(ext._ts)   || 0;
  if (tsLocal > 0 || tsExt > 0) return (tsExt >= tsLocal) ? ext : local;
  return (Number(ext.monto) || 0) >= (Number(local.monto) || 0) ? ext : local;
}
if (typeof window !== 'undefined') window._ventaDiariaGanadora = _ventaDiariaGanadora;

// =====================================================================
//  HISTORIAL (cobros) — misma idea que _ventaDiariaGanadora, para cuando
//  el mismo cobro (mismo id) existe en ambos lados de una fusión. Sin esto,
//  una devolución PARCIAL (edita el cobro para sacarle un ítem, no lo
//  borra del todo) no se propagaba: al fusionar, si el id ya existía del
//  lado local se descartaba la versión entrante sin comparar nada — el
//  otro teléfono se quedaba para siempre con la versión sin editar.
// =====================================================================
function _historialGanador(local, ext) {
  if (!local) return ext;
  if (!ext) return local;
  const tsLocal = Number(local._editTs) || 0;
  const tsExt   = Number(ext._editTs)   || 0;
  if (tsLocal > 0 || tsExt > 0) return (tsExt >= tsLocal) ? ext : local;
  return local; // sin ninguna marca de edición de ningún lado: no hay forma de saber cuál es más nueva, se mantiene el comportamiento anterior
}
if (typeof window !== 'undefined') window._historialGanador = _historialGanador;
...
function _fusionarDos(local, ext) {
  ...
  const idxPorIdH = new Map((local.historial||[]).map((v,i) => [String(v.id), i]));
  // Only add external entries that are not deleted and postdate the wipe timestamp;
  // when the id already exists locally, keep whichever copy is actually newer
  // (_historialGanador) instead of always discarding the external one.
  (ext.historial||[]).forEach(v=>{
    if (_todosElimCobros.has(String(v.id)) || (_wipeMax > 0 && (v.ts||0) < _wipeMax)) return;
    const idStr = String(v.id);
    if (idxPorIdH.has(idStr)) {
      const i = idxPorIdH.get(idStr);
      local.historial[i] = _historialGanador(local.historial[i], v);
    } else {
      local.historial.push(v); idxPorIdH.set(idStr, local.historial.length - 1);
    }
  });
  ...
  // BUGFIX SYNC: tombstone de "Reiniciar Mes" para pagos/gastos y restock log.
  // Antes, pagos se fusionaba con una simple unión de IDs que nunca podía "achicarse":
  // si Reiniciar Mes vaciaba pagos en un teléfono pero el otro aún tenía los viejos
  // (por ejemplo porque estaba desconectado en ese momento), la fusión los traía de
  // vuelta. Mismo mecanismo que ya protege a historial con historialWipeTs.
  const _wipeMesLocal = local.mesWipeTs ? Date.parse(local.mesWipeTs) : 0;
  const _wipeMesExt   = ext.mesWipeTs   ? Date.parse(ext.mesWipeTs)   : 0;
  const _wipeMesMax   = Math.max(_wipeMesLocal, _wipeMesExt);
  if (_wipeMesMax > 0) local.mesWipeTs = new Date(_wipeMesMax).toISOString();
  const _pagoTs = g => Number(g.ts) || Date.parse(g.fechaISO || g.fecha || '') || 0;
  // BUGFIX SYNC: respetar también pagos eliminados individualmente (borrado manual),
  // igual que ya se hace con cobrosEliminados/productosEliminados un poco más arriba.
  const _pagosElimLocal = new Set((local.pagosEliminados||[]).map(String));
  const _pagosElimExt   = new Set((ext.pagosEliminados||[]).map(String));
  const _todosPagosElim = new Set([..._pagosElimLocal, ..._pagosElimExt]);
  local.pagosEliminados = [..._todosPagosElim];
  local.pagos = (local.pagos||[]).filter(g =>
    (_wipeMesMax === 0 || _pagoTs(g) >= _wipeMesMax) && !_todosPagosElim.has(String(g.id))
  );
  const seenP=new Set((local.pagos||[]).map(g=>String(g.id)));
  (ext.pagos||[]).forEach(g=>{
    if(!seenP.has(String(g.id)) && (_wipeMesMax === 0 || _pagoTs(g) >= _wipeMesMax) && !_todosPagosElim.has(String(g.id))){
      local.pagos.push(g);seenP.add(String(g.id));
    }
  });
  ...
  (ext.ventasDiarias||[]).forEach(vExt=>{
    ...
    if(idx>=0){
      // BUGFIX SYNC (Estado de Caja): esta línea pisaba el monto local SIN
      // comparar nada — a diferencia de _aplicarDatos(), que sí compara. El
      // problema era que miSnap.ventasDiarias (el "local" que llega acá) apunta
      // a los MISMOS objetos que el array global ventasDiarias (no es una copia
      // profunda), así que mutar en el lugar corrompía el valor global al
      // instante. Ahora se usa la misma regla de _ts que el resto de los
      // puntos de sincronización (ver _ventaDiariaGanadora), y se reemplaza el
      // objeto en vez de mutarlo en el lugar para no depender de que nadie más
      // comparta esa referencia.
      local.ventasDiarias[idx] = _ventaDiariaGanadora(local.ventasDiarias[idx], vExt);
    }
  ...
  // BUGFIX SYNC: mismo tombstone de mes aplicado al restock log (antes tampoco
  // tenía ninguna protección contra resurrección).
  local.restockLog = local.restockLog.filter(r => _wipeMesMax === 0 || Number(r.ts||0) >= _wipeMesMax);
  const seenR=new Set(local.restockLog.map(r=>r.id));
  (ext.restockLog||[]).forEach(r=>{
    if(!seenR.has(r.id) && (_wipeMesMax === 0 || Number(r.ts||0) >= _wipeMesMax)){
      local.restockLog.push(r);seenR.add(r.id);
    }
  });
...
async function _aplicarDatos(datos) {
  ...
  // BUGFIX SYNC (Historial de cobros que no se actualiza en el otro teléfono):
  // esto era "historial = datos.historial || []" — un reemplazo CIEGO. Como
  // _aplicarDatos() corre en CADA carga de página (aplicando el snapshot
  // "fusionado" de Supabase), si ese snapshot todavía no reflejaba una
  // devolución reciente (o cualquier cobro nuevo hecho en este mismo
  // teléfono), recargar la página lo hacía desaparecer y volver a la
  // versión vieja. Ahora se fusiona por id — igual que ya se hace con
  // productos/ventasDiarias — respetando cobrosEliminados/historialWipeTs,
  // y usando _historialGanador para decidir cuál copia es más nueva cuando
  // el mismo id existe en ambos lados (una devolución parcial que edita el
  // cobro, no lo borra).
  {
    const _wipeTs = datos.historialWipeTs ? Date.parse(datos.historialWipeTs) : 0;
    const _cobrosElim = new Set([
      ...(typeof cobrosEliminados !== 'undefined' ? (cobrosEliminados || []) : []),
      ...((datos.cobrosEliminados || []).map(String))
    ]);
    if (typeof cobrosEliminados !== 'undefined') cobrosEliminados = [..._cobrosElim];
    const base = (historial || []).filter(v => (_wipeTs === 0 || (v.ts||0) >= _wipeTs) && !_cobrosElim.has(String(v.id)));
    const porId = new Map(base.map(v => [String(v.id), v]));
    (datos.historial || []).forEach(vExt => {
      if (_cobrosElim.has(String(vExt.id))) return;
      if (_wipeTs > 0 && (vExt.ts||0) < _wipeTs) return;
      const idStr = String(vExt.id);
      porId.set(idStr, porId.has(idStr) ? _historialGanador(porId.get(idStr), vExt) : vExt);
    });
    historial = [...porId.values()];
  }
  // BUGFIX SYNC: mismo criterio para pagos — unión por id en vez de reemplazo
  // ciego, respetando pagosEliminados. Sin esto, un gasto recién agregado en
  // este teléfono podía desaparecer al recargar si el snapshot aún no lo tenía.
  {
    const _pagosElim = new Set([
      ...(typeof pagosEliminados !== 'undefined' ? (pagosEliminados || []) : []),
      ...((datos.pagosEliminados || []).map(String))
    ]);
    if (typeof pagosEliminados !== 'undefined') pagosEliminados = [..._pagosElim];
    const basePagos = (pagos || []).filter(g => !_pagosElim.has(String(g.id)));
    const porIdPago = new Map(basePagos.map(g => [String(g.id), g]));
    (datos.pagos || []).forEach(gExt => {
      if (_pagosElim.has(String(gExt.id))) return;
      if (!porIdPago.has(String(gExt.id))) porIdPago.set(String(gExt.id), gExt);
    });
    pagos = [...porIdPago.values()];
  }
  ...
  { const _vdSnap = datos.ventasDiarias || [];
    ...
    _vdSnap.forEach(vExt => {
      ...
      if (idx >= 0) {
        // BUGFIX SYNC: "solo si es mayor" bloqueaba una devolución legítima que
        // baja el monto. Ahora usa la misma regla de _ts que el resto de los
        // puntos de sincronización de ventasDiarias (ver _ventaDiariaGanadora).
        ventasDiarias[idx] = _ventaDiariaGanadora(ventasDiarias[idx], vExt);
      } else {
  ...
  // BUGFIX SYNC (Estado de Caja): esta línea pisaba efectivoInicial con lo que
  // trajera el snapshot SIN comparar nada — ni siquiera evitaba bajar a 0. Como
  // _cargarDatosAlIniciar() aplica el snapshot "fusionado" de Supabase en CADA
  // carga de página (no solo al sincronizar con otro teléfono), si ese snapshot
  // alguna vez quedó desactualizado, cada recarga volvía a pisar el efectivo
  // inicial real con el valor viejo — y como Caja = efectivoInicial + ventas del
  // día − gastos, esto hacía "desaparecer" la Caja aunque ventasDiarias estuviera
  // perfecto. _fusionarDos ya protege este mismo campo con Math.max (línea de
  // más arriba); acá faltaba la misma protección.
  if(datos.efectivoInicial!==undefined){
    const _efIncoming = parseFloat(datos.efectivoInicial) || 0;
    efectivoInicial = Math.max(parseFloat(efectivoInicial) || 0, _efIncoming);
    idbSet('vpos_efectivoInicial',efectivoInicial).catch(()=>{});
    const el=document.getElementById('inpEfectivoInicial');if(el)el.value=efectivoInicial>0?efectivoInicial:'';
  }
  ...
  // BUGFIX: mismo criterio que efectivoInicial — tomar el mayor, no pisar sin comparar.
  if (datos.inventarioInicial !== undefined && datos.inventarioInicial > 0) {
    inventarioInicial = Math.max(parseFloat(inventarioInicial) || 0, parseFloat(datos.inventarioInicial) || 0);
    idbSet('vpos_inventarioInicial', inventarioInicial).catch(()=>{});
  ...
  if (datos.inventarioCosto !== undefined && datos.inventarioCosto > 0) {
    if (typeof inventarioCosto !== 'undefined') {
      inventarioCosto = Math.max(parseFloat(inventarioCosto) || 0, parseFloat(datos.inventarioCosto) || 0);
      idbSet('vpos_inventarioCosto', inventarioCosto).catch(()=>{});
      const elC = document.getElementById('inpInventarioCosto');
      if (elC) elC.value = inventarioCosto.toFixed(2);
    }
...
async function _autoEnviarSnapshot() {
  ...
  const snap = {
    ...
    datos: JSON.stringify({
      ...
      // BUGFIX SYNC: tombstone para borrado de pagos/restock al Reiniciar Mes
      mesWipeTs: localStorage.getItem('vpos_mesWipeTs') || null,
      ...
      // BUGFIX SYNC: idem para pagos/gastos eliminados individualmente (antes faltaba acá)
      pagosEliminados: typeof pagosEliminados !== 'undefined' ? (pagosEliminados || []) : [],
...
async function _autoFusionar() {
  ...
  try {
    ...
    const miSnap = {
      ...
      historial, pagos, ventasDiarias, restockLog: restockLog || [],
      // BUGFIX SYNC: _fusionarDos() lee estos campos de "local" (este objeto), pero
      // antes solo viajaban en el snapshot que se SUBE a Supabase (_autoEnviarSnapshot).
      // Sin esto, el lado local de una fusión no aportaba sus propios tombstones —
      // dependía por completo de que el OTRO dispositivo los trajera.
      productosEliminados:     typeof productosEliminados     !== 'undefined' ? (productosEliminados     || []) : [],
      cobrosEliminados:        typeof cobrosEliminados        !== 'undefined' ? (cobrosEliminados        || []) : [],
      pagosEliminados:         typeof pagosEliminados         !== 'undefined' ? (pagosEliminados         || []) : [],
      ventasDiariasEliminadas: typeof ventasDiariasEliminadas !== 'undefined' ? (ventasDiariasEliminadas || []) : [],
    ...
    // FIX VELOCIDAD: antes se borraba uno por uno esperando cada respuesta
    // (varias vueltas de red en serie); ahora se hacen todas en paralelo.
    await Promise.all(
      todosSnaps
        .filter(s => s.dispositivo_id !== 'fusion' && s.dispositivo_id !== _dispositivoId)
        .map(s => _sbDeleteFiltro('sync_snapshots', { id: 'eq.' + s.id }).catch(() => {}))
    );
...
// BUGFIX SYNC: antes, un mensaje que quedaba en cola (canal no listo en ese
// instante) solo se enviaba si ALGÚN OTRO _broadcast() posterior llegaba a
// correr con el canal ya activo — es decir, dependía de que pasara otra cosa
// cualquiera después. Una acción que solo llama a _broadcast() una vez (como
// una devolución) no tenía ninguna segunda oportunidad, y si el canal estaba
// reconectando justo en ese momento (algo normal después de que el celular
// estuvo con la pantalla apagada), ese aviso podía quedarse en la cola sin
// enviarse nunca. Ahora, cada vez que algo queda en cola, se programa un
// reintento propio cada 2s hasta que el canal vuelva a estar activo.
let _broadcastFlushTimer = null;
function _programarFlushBroadcast() {
  if (_broadcastFlushTimer) return;
  _broadcastFlushTimer = setTimeout(() => {
    _broadcastFlushTimer = null;
    if (!_broadcastQueue.length) return;
    if (_realtimeChannel && _realtimeActivo) {
      ...
      console.log('[Broadcast] 📤 Cola reenviada:', pending.length, 'mensaje(s)');
    } else {
      _programarFlushBroadcast(); // el canal sigue sin estar listo — seguir intentando
    }
  }, 2000);
}
...
function _iniciarPolling() {
  ...
  // FIX VELOCIDAD: red de seguridad 10s -> 5s (solo actúa si el realtime no está activo)
  _pollingTimer = setInterval(() => {
    if (!_realtimeActivo) _autoFusionar();
  }, 5000);
  console.log('[Sync] Realtime iniciado, polling seguridad 5s');
}
  ...
  try {
    _realtimeChannel = client
      ...
      .on('broadcast', { event: 'producto' }, ({ payload }) => {
        ...
        idbSet('vpos_productos', productos).catch(()=>{});
        if (typeof actualizarTodo === 'function') actualizarTodo();
        if (typeof _refrescarVentaAbierta === 'function') _refrescarVentaAbierta();
        _dot('green');
      })
      ...
      .on('broadcast', { event: 'config_actualizada' }, ({ payload }) => {
        ...
        if (payload.efectivoInicial !== undefined && typeof efectivoInicial !== 'undefined') {
          // BUGFIX SYNC (Estado de Caja): ninguno de los dos emisores de este evento
          // (actualizarInventarioInicialAuto / recalcularInventarioInicialManual)
          // representa una edición deliberada de efectivoInicial — son sobre
          // inventario, y solo incluyen efectivoInicial "de paso" con lo que sea
          // que tengan en memoria en ese momento. Si en el dispositivo que emite
          // efectivoInicial todavía no había terminado de cargar (0 momentáneo),
          // este broadcast propagaba ese 0 y pisaba un valor correcto en el otro
          // teléfono. Math.max evita eso sin bloquear ninguna edición real, porque
          // la edición real (guardarEfectivoInicial) no pasa por este evento.
          efectivoInicial = Math.max(parseFloat(efectivoInicial) || 0, Number(payload.efectivoInicial) || 0);
          idbSet('vpos_efectivoInicial', efectivoInicial).catch(() => {});
      ...
      .on('broadcast', { event: 'historial_actualizado' }, ({ payload }) => {
        ...
        historial = nuevo;
        // FIX: si el payload incluye ventasDiarias, aplicarlas — pero de forma
        // distinta según el origen del evento:
        //  • reiniciarHistorial() manda ventasDiarias:[] JUNTO con historialWipeTs
        //    → es un borrado total a propósito, se reemplaza tal cual.
        //  • guardarEdicionCobro() (una devolución) manda el array completo pero
        //    SIN historialWipeTs → antes esto también se reemplazaba a ciegas, lo
        //    cual funcionaba un instante pero no dejaba rastro de _ts, así que la
        //    próxima fusión automática (_fusionarDos/_aplicarDatos) no tenía forma
        //    de saber que este valor era el más nuevo y podía revertirlo otra vez
        //    a la cifra de antes de la devolución. Ahora se hace un merge fecha por
        //    fecha con la misma regla de _ts que usan los demás puntos de
        //    sincronización (_ventaDiariaGanadora), así el resultado queda protegido.
        if (payload.ventasDiarias !== undefined) {
          if (payload.historialWipeTs) {
            ...
            (payload.ventasDiarias || []).forEach(vExt => {
              const idx = (ventasDiarias||[]).findIndex(v => v.fecha === vExt.fecha);
              if (idx >= 0) ventasDiarias[idx] = _ventaDiariaGanadora(ventasDiarias[idx], vExt);
              else ventasDiarias.push({ ...vExt });
            });
            ventasDiarias.sort((a, b) => a.fecha.localeCompare(b.fecha));
          }
          idbSet('vpos_ventasDiarias', ventasDiarias).catch(() => {});
      ...
      .on('broadcast', { event: 'venta_diaria_actualizada' }, ({ payload }) => {
        ...
        // BUGFIX SYNC: esto pisaba el valor local sin comparar nada, y una versión
        // anterior de este arreglo solo dejaba subir el monto (nunca bajarlo) para
        // proteger una venta reciente de que la pisara un dato viejo — pero eso
        // rompía el caso de una DEVOLUCIÓN, que resta del total a propósito. Ahora
        // se usa la misma regla de _ts (quién escribió más reciente) que el resto
        // de los puntos de sincronización — ver _ventaDiariaGanadora.
        (payload.ventasDiarias || []).forEach(vExt => {
          const idx = (ventasDiarias||[]).findIndex(v => v.fecha === vExt.fecha);
          if (idx >= 0) {
            ventasDiarias[idx] = _ventaDiariaGanadora(ventasDiarias[idx], vExt);
      ...
      .on('broadcast', { event: 'reinicio_mes' }, ({ payload }) => {
        ...
        // BUGFIX SYNC: guardar los tombstones que manda el otro teléfono para que
        // ESTE teléfono tampoco reviva historial/pagos/restock en una fusión futura
        // contra un tercer dispositivo desactualizado (mismo patrón que ya usa
        // el handler de 'historial_actualizado').
        if (payload.historialWipeTs) {
          const localWipe = localStorage.getItem('vpos_historialWipeTs') || '';
          if (payload.historialWipeTs > localWipe) localStorage.setItem('vpos_historialWipeTs', payload.historialWipeTs);
        }
        if (payload.mesWipeTs) {
          const localWipeMes = localStorage.getItem('vpos_mesWipeTs') || '';
          if (payload.mesWipeTs > localWipeMes) localStorage.setItem('vpos_mesWipeTs', payload.mesWipeTs);
        }
        ventasDia = {}; ventasSem = {}; ventasMes = {};
        // BUGFIX: "restockLog = restockLog || []" nunca vaciaba el log si ya tenía
        // datos (un array con datos es "truthy", así que quedaba exactamente igual).
        ...
        // BUGFIX: el matcheo por substring guardaba {} para 'vpos_ventasDiarias'
        // porque "ventasDiarias".includes('Dia') también da true, aunque ventasDiarias
        // es un ARRAY — eso corrompía el dato en IDB (y podía romper el próximo load,
        // que espera un array y llamaría .sort/.filter sobre un objeto). Se usa un
        // mapa explícito para no depender de coincidencias de texto. También faltaba
        // persistir restockLog, que esta función sí limpia pero nunca guardaba.
        const _tiposIDB = {
          vpos_ventasDia: {}, vpos_ventasSem: {}, vpos_ventasMes: {},
          vpos_historial: [], vpos_pagos: [], vpos_ventasDiarias: [], vpos_restockLog: []
        };
        Object.keys(_tiposIDB).forEach(k => idbSet(k, _tiposIDB[k]).catch(()=>{}));
        if (typeof actualizarTodo === 'function') actualizarTodo();
        _dot('green');
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          // FIX VELOCIDAD: 600ms -> 250ms al conectar el canal en tiempo real
          setTimeout(_autoFusionar, 250);
        } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          _realtimeActivo = false;
          _realtimeChannel = null;
          console.warn('[Realtime] ⚠', status, '— polling 3s, reintento en 8s');
          if (!_pollingTimer) _pollingTimer = setInterval(_autoFusionar, 3000);
          // FIX VELOCIDAD: reintento de reconexión 8000ms -> 3000ms
          setTimeout(() => { if (_sesionActiva && !_realtimeActivo) _iniciarRealtime(); }, 3000);
        } else if (status === 'CLOSED') {
          _realtimeActivo = false;
          _realtimeChannel = null;
          // FIX VELOCIDAD: reintento de reconexión 4000ms -> 1500ms (reconexiones normales, ej. bloqueo de pantalla)
        if (_sesionActiva) setTimeout(() => { if (!_realtimeActivo) _iniciarRealtime(); }, 1500);
        }
      });
...
function _aplicarVentaRealtime(v) {
  ...
  // FIX VELOCIDAD: antes esto esperaba 500ms sin necesidad; ahora es inmediato.
  if (typeof renderCajaPanelMini   === 'function') renderCajaPanelMini();
  if (typeof renderCajaPanel       === 'function') {
    const onSub = document.getElementById('pgEstadoCaja')?.classList?.contains('active');
    if (onSub) renderCajaPanel();
...
async function _subirHistorial(){
  if(!historial||!historial.length){
    // Historial borrado localmente → eliminar las ventas de ESTA tienda en Supabase.
    // BUGFIX: el filtro era {id:'not.is.null'} (sin tienda_id) — es decir, TODAS las
    // filas de la tabla, de cualquier tienda. La política RLS "ventas_delete" del
    // esquema SQL solo exige es_admin(), no que la tienda coincida, así que en un
    // proyecto Supabase compartido entre varias tiendas esto podía borrar las ventas
    // de otras tiendas al Reiniciar Mes. Se filtra explícitamente por tienda_id (igual
    // que ya hace _subirPagos) como defensa aunque también se corrija la política RLS.
    const tid=_getTiendaId();
    try{ await _sbDeleteFiltro('ventas', tid ? {tienda_id:'eq.'+tid} : {id:'not.is.null'}); }catch(e){ console.warn('[subirHistorial] delete vacío:',e.message); }
    return;
  }
  try{
    ...
    try{
      ...
      try{
        ...
        if(typeof toast==='function') {
          const esSesionVencida = /401|JWT/i.test(e2.message || '');
          toast(esSesionVencida
            ? '⚠️ Sesión vencida — cerrá y volvé a abrir la página para reconectar'
            : 'Error al sincronizar productos: ' + e2.message, true);
        }
...
async function _subirVentasDiarias(){
  ...
  await _sbPost('ventas_diarias',ventasDiarias.map(v=>({fecha:tid+'_'+v.fecha,tienda_id:tid,monto:parseFloat(v.monto)||0,nota:v.nota||'',ts:Number(v._ts)||0})),true);
}
