  ...
  try {
    ...
    if (vd && vd.length > 0) {
      ...
      vd.forEach(row => {
        ...
        if (idx >= 0) {
          // SYNC FIX: el código anterior sobrescribía SIEMPRE con el valor de
          // Supabase, aunque el comentario decía "solo si es mayor". Esto podía
          // pisar una venta recién registrada localmente (ej. Caja mostrando $0
          // justo después de vender) si este sondeo llegaba antes de que la
          // subida a Supabase terminara. Ahora sí se compara de verdad, igual
          // que en autoRegistrarVentaDiaria().
