  ...
  try {
    ...
    if (vd && vd.length > 0) {
      ...
      vd.forEach(row => {
        ...
        if (idx >= 0) {
          // BUGFIX: el comentario ya decía "solo actualizar si Supabase trae un valor
          // mayor", pero el código pisaba el monto local sin comparar nada. Si esta
          // consulta corría justo después de una venta —antes de que el upload de esa
          // misma venta terminara de escribirse en Supabase— traía un monto viejo/menor
          // y borraba (visualmente) la venta recién hecha en "Estado de Caja".
...
function _fusionarDos(local, ext) {
  ...
  (ext.ventasDiarias||[]).forEach(vExt=>{
    ...
    if(idx>=0){
      ...
      // comparar nada — a diferencia de _aplicarDatos(), que sí compara y solo
      // toma el mayor. El problema es que miSnap.ventasDiarias (el "local" que
      // llega acá) apunta a los MISMOS objetos que el array global ventasDiarias
      // (no es una copia profunda), así que esta mutación en el lugar corrompía
      // el valor global al instante — antes de que _aplicarDatos() llegara a
      // comparar nada. Resultado: una venta recién hecha (Caja correcta) se
      // pisaba con el monto viejo de un teléfono desactualizado apenas corría
      // la próxima fusión automática. Ahora se compara igual que en los demás
      // puntos de fusión, y se reemplaza el objeto en vez de mutarlo en el
      // lugar para no depender de que nadie más comparta esa referencia.
      const localEntry = local.ventasDiarias[idx];
      const esAutoExt = vExt._auto === true;
      const puedeBajar = !esAutoExt; // edición manual (Ventas por Día): se respeta aunque sea menor
      if (puedeBajar || montoExt >= Number(localEntry.monto || 0)) {
        local.ventasDiarias[idx] = { ...localEntry, monto: montoExt, nota: vExt.nota || localEntry.nota, _auto: vExt._auto };
      }
    }
  ...
  try {
    _realtimeChannel = client
      ...
      .on('broadcast', { event: 'venta_diaria_actualizada' }, ({ payload }) => {
        ...
        // BUGFIX SYNC: el comentario decía "tomar el monto mayor para no retroceder
        // valores locales" pero el código de abajo comparaba "!==" (diferente) y
        // pisaba el valor local SIN IMPORTAR la dirección. Eso es justo lo que hacía
        // que "Estado de Caja" apareciera bien un instante después de una venta y
        // luego volviera a bajar: si en ese mismo momento llegaba (o se reordenaba)
        // un broadcast viejo con un monto menor, lo pisaba igual. Ahora: las
        // actualizaciones automáticas del POS (_auto:true) solo se aplican si el
        // monto entrante es mayor o igual al local — nunca bajan solas. Una edición
        // manual desde "Ventas por Día" (sin _auto) sí puede bajar el monto a propósito.
        (payload.ventasDiarias || []).forEach(vExt => {
          const idx = (ventasDiarias||[]).findIndex(v => v.fecha === vExt.fecha);
          const montoExt = Number(vExt.monto) || 0;
          if (idx >= 0) {
            const esAutoExt = vExt._auto === true;
            const puedeBajar = !esAutoExt; // edición manual: se respeta aunque sea menor
            if (puedeBajar || montoExt >= Number(ventasDiarias[idx].monto || 0)) {
              ventasDiarias[idx].monto = montoExt;
              if (vExt._auto !== undefined) ventasDiarias[idx]._auto = vExt._auto;
            }
            if (vExt.nota !== undefined) ventasDiarias[idx].nota = vExt.nota;
          } else {
            ventasDiarias.push({ fecha: vExt.fecha, monto: montoExt, nota: vExt.nota || '', _auto: vExt._auto });
          }
        });
        ventasDiarias.sort((a, b) => a.fecha.localeCompare(b.fecha));
        idbSet('vpos_ventasDiarias', ventasDiarias).catch(()=>{});
