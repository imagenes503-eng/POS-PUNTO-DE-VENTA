...
async function softReload() {
  ...
  // BUGFIX: esta lista estaba incompleta (le faltaban pgEstadoCaja, pgInvRegistrar,
  // pgInvCapital, pgInvAnalisis, pgInvProductos y pgAdmin) — si recargabas la app
  // estando en "Estado de Caja" (u otras de estas páginas), te mandaba de vuelta
  // al Dashboard en vez de dejarte donde estabas. Ahora coincide con la lista
  // completa de páginas válidas.
  const validas = ['pgDash','pgInventario','pgInvRegistrar','pgInvCapital','pgInvAnalisis','pgInvProductos','pgReportes','pgDestacados','pgVentasDiarias','pgSync','pgAdmin','pgFinanzasMes','pgCierreDia','pgEstadoCaja'];
...
function autoRegistrarVentaDiaria() {
  ...
  if (typeof _broadcast === 'function') _broadcast('venta_diaria_actualizada', { ventasDiarias });
  if (typeof _broadcast === 'function') _broadcast('ventas_dia_eliminada', { ventasDiariasEliminadas });
  // BUGFIX SYNC: si se desbloqueó la fecha, subir el snapshot YA MISMO — si no,
  // la próxima fusión automática de fondo (cada pocos segundos) puede traer de
  // vuelta la lista de bloqueo VIEJA desde Supabase (donde "hoy" seguía bloqueado
  // desde el último Reiniciar Mes) y borrar esta venta nueva otra vez, haciendo
  // que "Estado de Caja" muestre $0 aunque la venta ya esté hecha.
  if (_huboAutoDesbloqueo && typeof _autoEnviarSnapshot === 'function') setTimeout(_autoEnviarSnapshot, 150);
...
(async function init() {
  try {
    ...
    // BUGFIX: misma lista incompleta que en softReload() — le faltaban
    // pgEstadoCaja y otras páginas, causando que un F5/recarga real del
    // navegador te sacara de "Estado de Caja" de vuelta al Dashboard.
    const validas = ['pgDash','pgInventario','pgInvRegistrar','pgInvCapital','pgInvAnalisis','pgInvProductos','pgReportes','pgDestacados','pgVentasDiarias','pgSync','pgAdmin','pgFinanzasMes','pgCierreDia','pgEstadoCaja'];
    navTo(validas.includes(_paginaActual) ? _paginaActual : 'pgDash');
