...
// ── RED DE SEGURIDAD DE ERRORES ──────────────────────────────────────
// Muestra CUALQUIER error de JavaScript no capturado (o promesa
// rechazada) como un aviso fijo en pantalla, con el mensaje y la línea
// exactos, en vez de fallar en silencio sin que el usuario vea nada.
// Se queda visible hasta que se toque "Cerrar" (a diferencia del toast
// normal, que desaparece solo en 2.6s) para poder leerlo con calma o
// tomarle una captura de pantalla.
function _mostrarErrorFatal(msg) {
  let banner = document.getElementById('errorFatalBanner');
  if (!banner) {
    banner = document.createElement('div');
    banner.id = 'errorFatalBanner';
    banner.style.cssText = 'position:fixed;left:10px;right:10px;bottom:14px;z-index:99999;background:#7f1d1d;color:#fff;border:2px solid #fca5a5;border-radius:12px;padding:12px 14px;font-family:monospace;font-size:12px;box-shadow:0 6px 24px rgba(0,0,0,.5);max-height:40vh;overflow:auto;';
    document.body.appendChild(banner);
  }
  const linea = document.createElement('div');
  linea.style.cssText = 'border-top:1px solid rgba(255,255,255,.25);padding-top:6px;margin-top:6px;';
  linea.textContent = msg;
  banner.appendChild(linea);
  if (!document.getElementById('errorFatalCerrar')) {
    const btn = document.createElement('button');
    btn.id = 'errorFatalCerrar';
    btn.textContent = '✕ Cerrar aviso de error';
    btn.style.cssText = 'display:block;width:100%;margin-top:8px;background:#fca5a5;color:#7f1d1d;border:none;border-radius:8px;padding:6px;font-weight:900;font-family:Nunito,sans-serif;';
    btn.onclick = () => banner.remove();
    banner.appendChild(btn);
  }
}
window.addEventListener('error', (e) => {
  _mostrarErrorFatal(`${e.message}  (${(e.filename||'').split('/').pop()}:${e.lineno})`);
});
window.addEventListener('unhandledrejection', (e) => {
  _mostrarErrorFatal('Promesa rechazada: ' + (e.reason && e.reason.message ? e.reason.message : String(e.reason)));
});

/* =====================================================================
   🔒 CONTRASEÑA DE SEGURIDAD — protege acciones sensibles (devoluciones,
   reinicios, borrados, edición de inventario y de los datos base del
   mes). Una sola contraseña compartida, configurable por el dueño desde
   el botón "🔒 Contraseña" en la barra de respaldo. Si todavía no se
   configuró ninguna, las acciones se dejan pasar sin pedir nada (para
   no bloquear a nadie antes de que el dueño decida activarla).
   ===================================================================== */
const _PIN_STORAGE_KEY = 'vpos_pinSeguridad';

function _pinConfigurado() { return !!localStorage.getItem(_PIN_STORAGE_KEY); }
function _pinGuardado()    { return localStorage.getItem(_PIN_STORAGE_KEY) || ''; }

// Pide la contraseña con un diálogo propio de la app (nunca prompt()
// nativo) y ejecuta `callback` solo si coincide. Si no hay contraseña
// configurada aún, ejecuta `callback` directamente sin pedir nada.
function _requierePin(callback) {
  if (!_pinConfigurado()) { callback(); return; }
  _mostrarModalPin((valorIngresado) => {
    if (valorIngresado !== null && valorIngresado === _pinGuardado()) {
      callback();
...
function _mostrarModalPin(onSubmit) {
  let modal = document.getElementById('modalPinSeguridad');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'modalPinSeguridad';
    modal.style.cssText = 'position:fixed;inset:0;z-index:10090;background:rgba(5,46,22,0.75);backdrop-filter:blur(6px);display:none;align-items:center;justify-content:center;padding:16px;';
    modal.innerHTML = `
      <div style="background:#fff;border-radius:20px;width:100%;max-width:320px;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
        <div style="background:linear-gradient(135deg,#7c2d12,#c2410c);padding:20px 22px 16px;text-align:center;">
          <div style="font-size:30px;margin-bottom:6px;">🔒</div>
          <div style="color:#fff;font-size:15px;font-weight:900;font-family:'Nunito',sans-serif;">Acción protegida</div>
        </div>
        <div style="padding:20px 22px;">
          <div style="font-size:13px;font-weight:700;color:#374151;font-family:'Nunito',sans-serif;text-align:center;margin-bottom:14px;">
            Ingresa la contraseña de seguridad para continuar
          </div>
          <input id="inputPinSeguridad" type="password" autocomplete="off" autocapitalize="off" maxlength="40" placeholder="Contraseña"
            style="width:100%;box-sizing:border-box;text-align:center;letter-spacing:2px;font-size:18px;font-weight:900;padding:12px;border:1.5px solid #e5e7eb;border-radius:12px;margin-bottom:16px;font-family:'Nunito',sans-serif;">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
            <button id="btnCancelarPin" type="button" style="padding:13px;background:#f9fafb;border:1.5px solid #e5e7eb;border-radius:12px;font-size:14px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#374151;">✕ Cancelar</button>
            <button id="btnConfirmarPin" type="button" style="padding:13px;background:linear-gradient(135deg,#ea580c,#c2410c);border:none;border-radius:12px;font-size:14px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#fff;box-shadow:0 4px 14px rgba(234,88,12,0.3);">Confirmar</button>
          </div>
        </div>
      </div>`;
  ...
  const input = document.getElementById('inputPinSeguridad');
  input.value = '';
  setTimeout(() => input.focus(), 60);
  const cerrar = (valor) => { modal.style.display = 'none'; onSubmit(valor); };
  document.getElementById('btnCancelarPin').onclick  = () => cerrar(null);
  document.getElementById('btnConfirmarPin').onclick = () => cerrar(input.value);
  input.onkeydown = (e) => { if (e.key === 'Enter') cerrar(input.value); };
}

// Bloquea/desbloquea un <input> hasta que se ingrese el PIN correcto.
// Se usa en campos que se editan directo (como "Efectivo al Iniciar")
// donde no tiene sentido pedir el PIN en cada tecla — se pide UNA vez
// al enfocar el campo, y se vuelve a bloquear al salir de él.
function _gatearCampoConPin(input) {
  if (input.dataset.pinOk === '1') return;
  input.blur();
  if (!_pinConfigurado()) { input.dataset.pinOk = '1'; input.removeAttribute('readonly'); input.focus(); return; }
  _mostrarModalPin((valorIngresado) => {
    if (valorIngresado !== null && valorIngresado === _pinGuardado()) {
      input.dataset.pinOk = '1';
...
function _rebloquearCampoConPin(input) {
  input.setAttribute('readonly', 'readonly');
  input.dataset.pinOk = '0';
}

// ── Configurar / cambiar / quitar la contraseña de seguridad ──
function abrirConfigPin() {
  let modal = document.getElementById('modalConfigPin');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'modalConfigPin';
    modal.style.cssText = 'position:fixed;inset:0;z-index:10090;background:rgba(5,46,22,0.75);backdrop-filter:blur(6px);display:none;align-items:center;justify-content:center;padding:16px;';
    document.body.appendChild(modal);
    modal.addEventListener('click', e => { if (e.target === modal) modal.style.display = 'none'; });
  }
  const yaConfigurado = _pinConfigurado();
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px;width:100%;max-width:340px;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
      <div style="background:linear-gradient(135deg,#7c2d12,#c2410c);padding:20px 22px 16px;text-align:center;">
        <div style="font-size:30px;margin-bottom:6px;">🔒</div>
        <div style="color:#fff;font-size:15px;font-weight:900;font-family:'Nunito',sans-serif;">Contraseña de Seguridad</div>
        <div style="color:rgba(255,255,255,0.85);font-size:11px;font-weight:700;font-family:'Nunito',sans-serif;margin-top:4px;">
          ${yaConfigurado ? 'Protege devoluciones, reinicios, borrados y edición de datos sensibles.' : 'Aún no tienes una contraseña configurada — actívala para proteger las acciones sensibles.'}
        </div>
      </div>
      <div style="padding:20px 22px;">
        ${yaConfigurado ? `
        <label style="font-size:11px;font-weight:900;color:#c2410c;">Contraseña actual</label>
        <input id="cpPinActual" type="password" autocomplete="off" autocapitalize="off" placeholder="Contraseña actual" style="width:100%;box-sizing:border-box;padding:11px;border:1.5px solid #e5e7eb;border-radius:10px;margin:6px 0 14px;font-size:15px;font-family:'Nunito',sans-serif;">
        ` : ''}
        <label style="font-size:11px;font-weight:900;color:#c2410c;">${yaConfigurado ? 'Nueva contraseña' : 'Crea una contraseña'}</label>
        <input id="cpPinNuevo" type="password" autocomplete="off" autocapitalize="off" placeholder="Mínimo 4 caracteres" style="width:100%;box-sizing:border-box;padding:11px;border:1.5px solid #e5e7eb;border-radius:10px;margin:6px 0 14px;font-size:15px;font-family:'Nunito',sans-serif;">
        <label style="font-size:11px;font-weight:900;color:#c2410c;">Confirmar contraseña</label>
        <input id="cpPinConfirmar" type="password" autocomplete="off" autocapitalize="off" placeholder="Repite la contraseña" style="width:100%;box-sizing:border-box;padding:11px;border:1.5px solid #e5e7eb;border-radius:10px;margin:6px 0 18px;font-size:15px;font-family:'Nunito',sans-serif;">
        <button id="cpBtnGuardar" type="button" style="width:100%;padding:13px;background:linear-gradient(135deg,#ea580c,#c2410c);border:none;border-radius:12px;font-size:14px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#fff;box-shadow:0 4px 14px rgba(234,88,12,0.3);margin-bottom:10px;">💾 Guardar contraseña</button>
        ${yaConfigurado ? `<button id="cpBtnQuitar" type="button" style="width:100%;padding:12px;background:#fef2f2;border:1.5px solid #fecaca;border-radius:12px;font-size:13px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#dc2626;margin-bottom:10px;">🗑 Quitar protección por contraseña</button>` : ''}
        <button id="cpBtnCancelar" type="button" style="width:100%;padding:12px;background:#f9fafb;border:1.5px solid #e5e7eb;border-radius:12px;font-size:13px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#374151;">Cancelar</button>
      </div>
    </div>`;
  modal.style.display = 'flex';

  document.getElementById('cpBtnCancelar').onclick = () => { modal.style.display = 'none'; };
  document.getElementById('cpBtnGuardar').onclick = () => {
    if (yaConfigurado) {
      const actual = document.getElementById('cpPinActual').value;
      if (actual !== _pinGuardado()) { toast('❌ La contraseña actual no es correcta', true); return; }
    }
    const nuevo   = document.getElementById('cpPinNuevo').value;
    const confirmar = document.getElementById('cpPinConfirmar').value;
    if (!nuevo || nuevo.length < 4) { toast('La contraseña debe tener al menos 4 caracteres', true); return; }
    if (nuevo !== confirmar) { toast('Las contraseñas no coinciden', true); return; }
    localStorage.setItem(_PIN_STORAGE_KEY, nuevo);
    modal.style.display = 'none';
    toast('🔒 Contraseña de seguridad guardada');
  };
  const btnQuitar = document.getElementById('cpBtnQuitar');
  if (btnQuitar) {
    btnQuitar.onclick = () => {
      const actual = document.getElementById('cpPinActual').value;
      if (actual !== _pinGuardado()) { toast('❌ La contraseña actual no es correcta', true); return; }
      if (!confirm('¿Quitar la protección por contraseña? Las acciones sensibles dejarán de pedirla.')) return;
      localStorage.removeItem(_PIN_STORAGE_KEY);
      modal.style.display = 'none';
      toast('Contraseña de seguridad desactivada');
    };
  }
}
...
function _validarFechaReportes() {
  ...
  if (guardado.dia !== hoyStr) {
    ...
    // Nuevo día → borrar las "Entradas de Hoy" del día anterior
    if (typeof _limpiarRestockLogAnterior === 'function') _limpiarRestockLogAnterior();
  }
...
let ventasDiariasEliminadas = []; // {f:'YYYY-MM-DD', ts} borradas — evita que vuelvan al fusionar
// SYNC FIX: helpers para el nuevo formato de tombstone con fecha/hora.
// Antes era solo una lista plana de fechas ("2026-07-27") que bloqueaba esa
// fecha PARA SIEMPRE, incluso ventas legítimas hechas días después del borrado
// (ej. tras un Reiniciar Mes). Ahora cada tombstone recuerda CUÁNDO se borró,
// y solo bloquea datos que sean de ANTES de ese momento. Sigue aceptando el
// formato viejo (string plano) para no romper datos ya guardados.
function _vdTsTombstone(fecha) {
  const e = (ventasDiariasEliminadas || []).find(x => (typeof x === 'string' ? x : x.f) === fecha);
  if (!e) return null;
  return typeof e === 'string' ? 0 : Number(e.ts) || 0;
}
function _vdEstaBloqueado(fecha, incomingTs) {
  const tombTs = _vdTsTombstone(fecha);
  if (tombTs === null) return false; // no hay tombstone para esa fecha
  return Number(incomingTs || 0) <= tombTs; // solo bloquea si el dato es igual o más viejo que el borrado
}
...
function renderCajaPanelMini() {
  ...
  el.innerHTML = `
    <div style="font-size:11px;color:#d9f99d;font-weight:700;">Caja</div>
    <div style="font-size:16px;font-weight:900;color:#fff;font-family:'Space Mono',monospace;">$${caja.toFixed(2)}</div>
    <div style="font-size:10px;color:#bef264;font-weight:700;">Capital $${capital.toFixed(2)}</div>`;
...
function renderCajaPanel() {
  ...
  const hoyPendienteTxt = !hoyYaRegistrado && ventasHoyTotal > 0
    ...
    : (hoyYaRegistrado ? `<div style="margin-top:5px;background:#ffedd5;border:1px solid #fdba74;border-radius:7px;padding:4px 8px;font-size:11px;font-weight:800;color:#c2410c;">✅ Hoy registrado: $${ventasHoyEnCaja.toFixed(2)}</div>` : '');

  grid.innerHTML = `
    <!-- ══ 1. CAJA ══════════════════════════════════════════════ -->
    <div style="background:#fff;border:2px solid #fdba74;border-radius:14px;overflow:hidden;">
      <div style="background:linear-gradient(135deg,#3f6212,#4d7c0f);padding:11px 16px;display:flex;align-items:center;gap:10px;">
        ...
        <div style="font-size:11px;color:#d9f99d;font-weight:700;">Dinero físico disponible</div></div>
      </div>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;padding:12px 14px;">
        ...
        ${card('📈','Entró a Caja','$'+entroACaja.toFixed(2),'Ventas cobradas del mes','#c2410c','#fffbeb','#fdba74')}
        ...
        <div style="background:linear-gradient(135deg,#fffbeb,#ffedd5);border:2px solid #f97316;border-radius:12px;padding:12px 14px;">
          <div style="font-size:10px;font-weight:900;color:#c2410c;text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px;">💵 CAJA ACTUAL</div>
          <div style="font-size:22px;font-weight:900;color:#c2410c;font-family:'Space Mono',monospace;">$${cajaActual.toFixed(2)}</div>
          <div style="font-size:10px;color:#fb923c;margin-top:3px;font-weight:700;">Dinero disponible ahora</div>
          ${hoyPendienteTxt}
        </div>
      </div>
      <div style="background:#fffbeb;border-top:1px solid #fed7aa;padding:7px 14px;font-size:11px;color:#c2410c;font-weight:700;">
    ...
    <div style="background:#fff;border:2px solid #7dd3fc;border-radius:14px;overflow:hidden;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;padding:12px 14px;">
        ...
        ${card('📈','Ganancia Potencial','$'+gananciaPotencial.toFixed(2),'Valor venta − Valor costo',gananciaPotencial>=0?'#c2410c':'#dc2626',gananciaPotencial>=0?'#fffbeb':'#fef2f2',gananciaPotencial>=0?'#fdba74':'#fca5a5')}
      </div>
    ...
    <div style="background:#fff;border:2px solid ${esGN?'#fdba74':'#fca5a5'};border-radius:14px;overflow:hidden;">
      <div style="background:linear-gradient(135deg,${esGN?'#365314,#3f6212':'#7f1d1d,#991b1b'});padding:11px 16px;display:flex;align-items:center;gap:10px;">
        <div style="width:32px;height:32px;background:rgba(255,255,255,.2);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:18px;">📊</div>
        <div><div style="font-size:13px;font-weight:900;color:#fff;">RENDIMIENTO DEL MES</div>
        <div style="font-size:11px;color:${esGN?'#d9f99d':'#fca5a5'};font-weight:700;">Resumen de ventas y utilidad</div></div>
      </div>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;padding:12px 14px;">
        ${card('🛒','Ventas Totales','$'+totalVentasDiariasDelMes.toFixed(2),'Total vendido en el mes','#c2410c','#fffbeb','#fdba74')}
        ...
        ${card(gananciaBruta>=0?'✅':'🔴','Utilidad Bruta','$'+gananciaBruta.toFixed(2),'Ventas − Costo (antes de gastos)',gananciaBruta>=0?'#c2410c':'#dc2626',gananciaBruta>=0?'#fffbeb':'#fef2f2',gananciaBruta>=0?'#fdba74':'#fca5a5')}
        <div style="background:linear-gradient(135deg,${esGN?'#fffbeb,#ffedd5':'#fef2f2,#fee2e2'});border:2px solid ${esGN?'#f97316':'#ef4444'};border-radius:12px;padding:12px 14px;">
          <div style="font-size:10px;font-weight:900;color:${esGN?'#c2410c':'#dc2626'};text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px;">${esGN?'✅':'🔴'} UTILIDAD NETA</div>
          <div style="font-size:22px;font-weight:900;color:${esGN?'#c2410c':'#dc2626'};font-family:'Space Mono',monospace;">${esGN?'+':''}$${gananciaNeta.toFixed(2)}</div>
          <div style="font-size:10px;color:${esGN?'#fb923c':'#f87171'};margin-top:3px;font-weight:700;">Ventas − Costo − Gastos</div>
        </div>
        ${card('%','Margen Bruto',margenMes+'%','Utilidad Bruta ÷ Ventas × 100',margenMes>=30?'#c2410c':margenMes>=15?'#d97706':'#dc2626',margenMes>=30?'#fffbeb':margenMes>=15?'#fffbeb':'#fef2f2',margenMes>=30?'#fdba74':margenMes>=15?'#fcd34d':'#fca5a5')}
        ${card('%','Margen Neto',margenNetoMes+'%','Utilidad Neta ÷ Ventas × 100',margenNetoMes>=15?'#c2410c':margenNetoMes>=5?'#d97706':'#dc2626',margenNetoMes>=15?'#fffbeb':margenNetoMes>=5?'#fffbeb':'#fef2f2',margenNetoMes>=15?'#fdba74':margenNetoMes>=5?'#fcd34d':'#fca5a5')}
      </div>
      <div style="background:${esGN?'#fffbeb':'#fef2f2'};border-top:1px solid ${esGN?'#fed7aa':'#fca5a5'};padding:7px 14px;font-size:11px;color:${esGN?'#c2410c':'#dc2626'};font-weight:700;">
    ...
    <div style="background:#fff;border:2px solid #e5e7eb;border-radius:14px;overflow:hidden;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:12px 14px;">
        ...
        <div style="background:#fffbeb;border:2px solid #f97316;border-radius:12px;padding:12px 14px;">
          <div style="font-size:10px;font-weight:900;color:#c2410c;text-transform:uppercase;letter-spacing:.4px;margin-bottom:4px;">💰 A PRECIO VENTA</div>
          <div style="font-size:22px;font-weight:900;color:#c2410c;font-family:'Space Mono',monospace;">$${capitalTotalVenta.toFixed(2)}</div>
          <div style="font-size:10px;color:#fb923c;margin-top:3px;font-weight:700;">Caja + inv. a precio venta</div>
          <div style="margin-top:5px;font-size:9px;background:#ffedd5;color:#c2410c;padding:3px 6px;border-radius:5px;font-weight:900;">🚀 Valor máximo si vendes todo</div>
        </div>
      </div>
    </div>
    ...
    <div style="background:#fff;border:2px solid #e5e7eb;border-radius:14px;overflow:hidden;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;padding:12px 14px;">
        ...
        ${card('%','Margen de Ganancia',margenMes+'%','Utilidad ÷ Ventas × 100',margenMes>=30?'#c2410c':margenMes>=15?'#d97706':'#dc2626',margenMes>=30?'#fffbeb':margenMes>=15?'#fffbeb':'#fef2f2',margenMes>=30?'#fdba74':margenMes>=15?'#fcd34d':'#fca5a5')}
...
function navTo(pgId, pushHistory = true) {
  ...
  const invSubPgs = ['pgInvRegistrar','pgInvCapital','pgInvAnalisis','pgInvProductos','pgEntradasHoy'];
...
window.addEventListener('popstate', (e) => {
  ...
  const validas = ['pgDash','pgInventario','pgInvRegistrar','pgInvCapital','pgInvAnalisis','pgInvProductos','pgEntradasHoy','pgReportes','pgDestacados','pgVentasDiarias','pgSync','pgAdmin','pgFinanzasMes','pgCierreDia','pgEstadoCaja'];
...
function actualizarStats() {
  ...
  ['drawerStatProds', 'drawerStatVentas', 'drawerStatMes', 'drawerStatCrit'].forEach((id, i) => {
    ...
    if (i === 3) el.style.color = criticos > 0 ? '#f87171' : '#fdba74';
  });
...
function recalcularInventarioInicialManual() {
  _requierePin(() => _recalcularInventarioInicialManualReal());
}
function _recalcularInventarioInicialManualReal() {
  ...
  const snap = {};
  (productos || []).forEach(p => { snap[String(p.id)] = { nom: p.nom || '—', cat: p.cat || 'SIN CATEGORÍA', proveedor: p.proveedor || '', stock: Number(p.stock || 0) }; });
...
function renderInvTotales() {
  ...
  if (peBox && peGrid) {
    if (hayPE) {
      ...
      peGrid.innerHTML = `
        ...
        <div style="background:#fffbeb;border:2px solid #fdba74;border-radius:10px;padding:12px;">
          <div style="font-size:10px;font-weight:900;color:#c2410c;margin-bottom:4px;">💵 EFECTIVO INICIAL</div>
          <div style="font-size:20px;font-weight:900;color:#c2410c;">$${efectivoInicial.toFixed(2)}</div>
          <div style="font-size:10px;color:#fb923c;margin-top:3px;font-weight:700;">Dinero en caja al inicio</div>
        </div>
        ...
        <div style="background:#fffbeb;border:2px solid #f97316;border-radius:10px;padding:12px;">
          <div style="font-size:10px;font-weight:900;color:#c2410c;margin-bottom:4px;">📈 CAPITAL INICIAL A VENTA</div>
          <div style="font-size:20px;font-weight:900;color:#c2410c;">$${capIniVenta.toFixed(2)}</div>
          <div style="font-size:10px;color:#fb923c;margin-top:3px;font-weight:700;">Efectivo + Inv. a precio venta</div>
          <div style="margin-top:5px;font-size:9px;background:#ffedd5;color:#c2410c;padding:3px 6px;border-radius:5px;font-weight:900;">📊 Valor potencial si vendes todo</div>
        </div>
        ...
        <div style="grid-column:1/-1;background:${gananciaVentas>=0?'#fffbeb':'rgba(220,38,38,0.05)'};border:2px solid ${gananciaVentas>=0?'#fdba74':'rgba(220,38,38,0.3)'};border-radius:10px;padding:12px;">
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:8px;">
            <div style="text-align:center;">
              <div style="font-size:9px;font-weight:900;color:#6b7280;">💰 VENTAS</div>
              <div style="font-size:16px;font-weight:900;color:#c2410c;">$${ventasConfirmadas.toFixed(2)}</div>
            </div>
            <div style="text-align:center;">
          ...
          <div style="border-top:1px solid ${gananciaVentas>=0?'#fdba74':'rgba(220,38,38,0.3)'};padding-top:8px;display:flex;justify-content:space-between;align-items:center;">
            <div style="font-size:10px;font-weight:900;color:#6b7280;">Ventas − Costo − Gastos</div>
            <div style="font-size:20px;font-weight:900;color:${gananciaVentas>=0?'#c2410c':'#dc2626'};">${gananciaVentas>=0?'+':''}$${gananciaVentas.toFixed(2)}</div>
          </div>
          ...
          <div style="border-top:1px solid ${gananciaVentas>=0?'#fdba74':'rgba(220,38,38,0.3)'};margin-top:8px;padding-top:8px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
            <div style="text-align:center;">
              <div style="font-size:9px;font-weight:900;color:#6b7280;">📈 UTILIDAD BRUTA</div>
              <div style="font-size:15px;font-weight:900;color:${utilidadBrutaInv>=0?'#c2410c':'#dc2626'};">${utilidadBrutaInv>=0?'+':''}$${utilidadBrutaInv.toFixed(2)}</div>
            ...
            <div style="text-align:center;">
              ...
              <div style="font-size:15px;font-weight:900;color:${margenBrutoInv>=30?'#c2410c':margenBrutoInv>=15?'#d97706':'#dc2626'};">${margenBrutoInv}%</div>
            ...
            <div style="text-align:center;">
              ...
              <div style="font-size:15px;font-weight:900;color:${margenNetoInv>=15?'#c2410c':margenNetoInv>=5?'#d97706':'#dc2626'};">${margenNetoInv}%</div>
  ...
  panel.innerHTML = `
    ...
    <div style="background:#fffbeb;border:2px solid #fdba74;border-radius:10px;padding:11px;">
      <div style="font-size:10px;font-weight:900;color:#c2410c;margin-bottom:4px;">💰 VALOR A PRECIO VENTA</div>
      <div style="font-size:20px;font-weight:900;color:#c2410c;">$${totalInvVenta.toFixed(2)}</div>
      <div style="font-size:10px;color:#fb923c;margin-top:3px;font-weight:700;">Potencial si vendes todo · Margen ${margenActual}%</div>
    </div>
    ...
    <div style="background:#fffbeb;border:2px solid #f97316;border-radius:10px;padding:11px;">
      <div style="font-size:10px;font-weight:900;color:#c2410c;margin-bottom:4px;">📈 CAPITAL A PRECIO VENTA</div>
      <div style="font-size:22px;font-weight:900;color:#c2410c;">$${totalCapVenta.toFixed(2)}</div>
      <div style="font-size:10px;color:#fb923c;margin-top:3px;font-weight:700;">Caja + inv. a venta</div>
      <div style="margin-top:5px;font-size:9px;background:#ffedd5;color:#c2410c;padding:3px 6px;border-radius:5px;font-weight:900;">🚀 Valor máximo si vendes todo el stock</div>
    </div>
  `;
}
...
function guardarProducto(e) {
  ...
  const newProveedor = (document.getElementById('inpProveedor')?.value || '').trim();
  ...
  if (id) {
    ...
    productos = productos.map(x => String(x.id) === String(id)
      ? { id: Number(id), cod: newCod, nom: newNom, cat: newCat, abrev: newAbrev, proveedor: newProveedor, compra: newCompra, venta: newVenta, stock: newStock, min: newMin, costoTotal: newCostoTotal, ventaTotal: newVentaTotal, costoUnidadEntrado, ventaUnidadEntrada, lotes: lotesActuales, paquetes: newPaquetes, img: newImg, _ts: Date.now() }
      : x);
  } else {
    productos.push({ id: Date.now(), cod: newCod, nom: newNom, cat: newCat, abrev: newAbrev, proveedor: newProveedor, compra: newCompra, venta: newVenta, stock: newStock, min: newMin, costoTotal: newCostoTotal, ventaTotal: newVentaTotal, costoUnidadEntrado, ventaUnidadEntrada, lotes: [], paquetes: newPaquetes, img: newImg, _ts: Date.now() });
  }
...
function renderInv() {
  ...
  const lista  = productos.filter(p => !filtro || (p.nom||'').includes(filtro) || (p.cod||'').includes(filtro) || (p.cat||'').includes(filtro) || (p.proveedor||'').toUpperCase().includes(filtro));
    ...
    return `<div class="inv-prod-card${critico?' critico':''}">
      <div class="inv-card-top">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div class="inv-card-cat">${p.cat||''}${p.proveedor ? ` · 🏭 ${p.proveedor}` : ''}</div>
        </div>
      </div>
  ...
  if (totEl) {
    ...
    totEl.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
        ...
        <div style="background:#fffbeb;border:1px solid #fed7aa;border-radius:10px;padding:10px;text-align:center;">
        ...
        <div style="background:linear-gradient(135deg,#fffbeb,#ffedd5);border:1.5px solid var(--green);border-radius:10px;padding:10px;text-align:center;">
...
function editarProd(id) {
  _requierePin(() => _editarProdReal(id));
}
function _editarProdReal(id) {
  const p = productos.find(x => String(x.id) === String(id)); if (!p) return;
  ['editId','inpCod','inpAbrev','inpNom','inpCat','inpProveedor','inpCompra','inpVenta','inpStock','inpMin'].forEach((fid, i) => {
    document.getElementById(fid).value = [p.id, p.cod||'', p.abrev||'', p.nom||'', p.cat||'', p.proveedor||'', p.compra||0, p.venta||0, p.stock||0, p.min||0][i];
  });
...
function _restockRenderLineas(p) {
  ...
  function _pkgOpts(ln) {
    const existentes = pkgs.map(pk =>
      `<div class="rs-pkg-opt" onclick="restockSelPkg('${ln.id}','${pk.id}')" style="padding:9px 13px;cursor:pointer;font-weight:800;font-size:13px;border-bottom:1px solid #f0f0f0;display:flex;justify-content:space-between;align-items:center;" onmouseover="this.style.background='#fffbeb'" onmouseout="this.style.background=''">
        <span>📦 ${pk.label||('Paquete '+pk.cant+' uds')}</span>
        <span style="font-size:11px;color:var(--text-muted);">${pk.cant} uds · $${fmtP(pk.precio)}</span>
      </div>`
    ).join('');
    const unidadSel = ln.pkgId==='unidad' ? 'background:#fffbeb;' : '';
    return existentes +
      `<div class="rs-pkg-opt" onclick="restockSelPkg('${ln.id}','unidad')" style="padding:9px 13px;cursor:pointer;font-weight:800;font-size:13px;border-bottom:1px solid #f0f0f0;${unidadSel}" onmouseover="this.style.background='#fffbeb'" onmouseout="this.style.background='${unidadSel}'">🔹 Unidad</div>` +
      `<div class="rs-pkg-opt" onclick="restockSelPkg('${ln.id}','nuevo')" style="padding:9px 13px;cursor:pointer;font-weight:900;font-size:13px;color:var(--green-dark);" onmouseover="this.style.background='#fffbeb'" onmouseout="this.style.background=''">➕ Nuevo paquete</div>`;
  }
...
// =====================================================================
//  📥 ENTRADAS DE INVENTARIO DE HOY — vista, filtro y descarga en PDF
//  del restockLog registrado en el día en curso. Se limpia sola cada
//  día (ver _limpiarRestockLogAnterior, enganchado a _validarFechaReportes).
// =====================================================================
function _entradasDeHoy() {
  const inicioHoy = new Date(); inicioHoy.setHours(0, 0, 0, 0);
  const inicioHoyTs = inicioHoy.getTime();
  return (restockLog || [])
    .filter(r => (r.ts || 0) >= inicioHoyTs)
    .map(r => {
      const p = productos.find(x => String(x.id) === String(r.prodId));
      return {
        id: r.id, ts: r.ts, fechaStr: r.fechaStr,
        nom: p ? p.nom : '(producto eliminado)',
        cat: p ? (p.cat || 'SIN CATEGORÍA') : 'SIN CATEGORÍA',
        proveedor: p ? (p.proveedor || '') : '',
        cant: r.cant || 0,
        costoUd: r.precioCompra || 0,
        total: (r.cant || 0) * (r.precioCompra || 0)
      };
    })
    .sort((a, b) => (b.ts || 0) - (a.ts || 0));
}

function _poblarFiltrosEntradasHoy() {
  const entradas = _entradasDeHoy();
  const selCat = document.getElementById('entHoyCat');
  if (selCat) {
    const cats = [...new Set(entradas.map(e => e.cat))].sort();
    const val = selCat.value;
    selCat.innerHTML = '<option value="todas">📦 Todas</option>' + cats.map(c => `<option value="${c}">${c}</option>`).join('');
    selCat.value = cats.includes(val) ? val : 'todas';
  }
  const selProv = document.getElementById('entHoyProv');
  if (selProv) {
    const provs = [...new Set(entradas.map(e => e.proveedor).filter(Boolean))].sort();
    const val = selProv.value;
    selProv.innerHTML = '<option value="todos">🏭 Todos</option>' + provs.map(pr => `<option value="${pr}">${pr}</option>`).join('');
    selProv.value = provs.includes(val) ? val : 'todos';
  }
}

function _entradasHoyFiltradas() {
  const cat    = document.getElementById('entHoyCat')?.value || 'todas';
  const prov   = document.getElementById('entHoyProv')?.value || 'todos';
  const buscar = (document.getElementById('entHoyBuscar')?.value || '').trim().toUpperCase();
  return _entradasDeHoy().filter(e =>
    (cat === 'todas' || e.cat === cat) &&
    (prov === 'todos' || e.proveedor === prov) &&
    (!buscar || e.nom.toUpperCase().includes(buscar))
  );
}

function renderEntradasHoy() {
  _poblarFiltrosEntradasHoy();
  const rows = _entradasHoyFiltradas();
  const tbody = document.getElementById('tbodyEntradasHoy');
  const resumen = document.getElementById('entHoyResumen');
  if (!tbody) return;

  if (!rows.length) {
    tbody.innerHTML = `<tr><td colspan="7"><div class="empty"><span class="empty-icon">📥</span>Sin entradas de inventario registradas hoy</div></td></tr>`;
    if (resumen) resumen.textContent = '';
    return;
  }
  const totalUds = rows.reduce((s, r) => s + r.cant, 0);
  const totalInv = rows.reduce((s, r) => s + r.total, 0);
  if (resumen) resumen.textContent = `${rows.length} entrada${rows.length!==1?'s':''} · ${totalUds} unidades · Invertido: $${totalInv.toFixed(2)}`;

  tbody.innerHTML = rows.map(r => `<tr>
    <td class="td-bold">${r.nom}</td>
    <td><span class="badge badge-green" style="font-size:11px;">${r.cat}</span></td>
    <td style="font-size:11px;color:var(--text-muted);">${r.proveedor || '—'}</td>
    <td class="mono" style="text-align:center;">${r.cant}</td>
    <td class="mono" style="text-align:right;">$${r.costoUd.toFixed(2)}</td>
    <td class="mono td-green" style="text-align:right;">$${r.total.toFixed(2)}</td>
    <td style="font-size:11px;color:var(--text-muted);">${(r.fechaStr||'').split(', ')[1] || r.fechaStr || ''}</td>
  </tr>`).join('');
}

function descargarPDFEntradasHoy() {
  if (!window.jspdf) { toast('jsPDF no disponible', true); return; }
  const rows = _entradasHoyFiltradas();
  if (!rows.length) { toast('No hay entradas para descargar con este filtro', true); return; }
  ...
  const prov = document.getElementById('entHoyProv')?.value || 'todos';
  const hoy  = new Date();
  const filtroLabel = (cat === 'todas' ? 'Todas las categorías' : cat) + (prov !== 'todos' ? ' · Proveedor: ' + prov : '');

  doc.setFillColor(22, 163, 74);
  doc.rect(0, 0, 210, 26, 'F');
  doc.setFont('helvetica', 'bold'); doc.setFontSize(15); doc.setTextColor(255,255,255);
  doc.text('ENTRADAS DE INVENTARIO — HOY', 105, 12, { align: 'center' });
  doc.setFontSize(9); doc.setFont('helvetica', 'normal');
  doc.text(`${hoy.toLocaleDateString('es-SV')}  ·  ${filtroLabel}`, 105, 19, { align: 'center' });
  ...
  doc.autoTable({
    startY: 32,
    head: [['Producto', 'Categoría', 'Proveedor', 'Cant.', 'Costo Ud.', 'Total', 'Hora']],
    body: rows.map(r => [r.nom, r.cat, r.proveedor || '—', String(r.cant), `$${r.costoUd.toFixed(2)}`, `$${r.total.toFixed(2)}`, (r.fechaStr||'').split(', ')[1] || r.fechaStr || '']),
    foot: [['TOTAL', '', '', String(totalUds), '', `$${totalInv.toFixed(2)}`, '']],
    styles: { fontSize: 8, cellPadding: 3 },
    headStyles: { fillColor: [22,163,74], textColor: [255,255,255], fontStyle: 'bold' },
    footStyles: { fillColor: [240,253,244], textColor: [21,128,61], fontStyle: 'bold' },
    alternateRowStyles: { fillColor: [248,250,252] },
    columnStyles: { 3:{halign:'center'}, 4:{halign:'right'}, 5:{halign:'right',fontStyle:'bold',textColor:[22,163,74]} },
    margin: { left: 10, right: 10 }
  });

  const nombreArchivo = `Entradas_${hoy.toISOString().split('T')[0]}${cat!=='todas'?'_'+cat:''}${prov!=='todos'?'_'+prov:''}.pdf`;
  if (_descargarPDF(doc, nombreArchivo)) toast('✓ PDF de entradas descargado');
}

// ── Limpieza automática: las entradas de inventario de días anteriores
// se eliminan solas apenas la app detecta que empezó un nuevo día
// (se engancha a _validarFechaReportes, que ya corre al abrir la app y
// después de cada venta). Nota honesta: esto se ejecuta la próxima vez
// que abras o uses la app después de medianoche — un navegador no puede
// ejecutar código exactamente a las 12:01 am si está cerrado.
function _limpiarRestockLogAnterior() {
  try {
    const inicioHoy = new Date(); inicioHoy.setHours(0, 0, 0, 0);
    const inicioHoyTs = inicioHoy.getTime();
    const viejos = (restockLog || []).filter(r => (r.ts || 0) < inicioHoyTs);
    if (!viejos.length) return;
    const idsViejos = viejos.map(r => r.id);
    restockLog = (restockLog || []).filter(r => (r.ts || 0) >= inicioHoyTs);
    idbSet('vpos_restockLog', restockLog).catch(() => {});
    if (typeof _sbDeleteFiltro === 'function' && typeof _sesionActiva !== 'undefined' && _sesionActiva) {
      const tid = typeof _getTiendaId === 'function' ? _getTiendaId() : '';
      if (tid && idsViejos.length) {
        const lista = idsViejos.map(id => `"${String(id).replace(/"/g, '')}"`).join(',');
        _sbDeleteFiltro('restock_log', { id: `in.(${lista})`, tienda_id: `eq.${tid}` }).catch(() => {});
      }
    }
    console.log(`[EntradasHoy] Se limpiaron ${viejos.length} entradas de días anteriores`);
  } catch (e) { console.warn('[_limpiarRestockLogAnterior]', e); }
}
...
function borrarProd(id) {
  _requierePin(() => _borrarProdReal(id));
}
function _borrarProdReal(id) {
...
function abrirModalVenta() {
  ...
  _actualizarBotonEspera();
}

// ── Atender otro cliente sin perder la venta grande ──────────────────
// En vez de abrir otra pestaña (no funciona bien en la app instalada como
// standalone — Android la trata como una ventana aparte, no una pestaña),
// esto PAUSA el carrito actual guardándolo en memoria, deja el carrito
// vacío para la venta rápida, y con un botón se retoma justo como estaba.
let carritoEnEspera = null;
try {
  const _cee = localStorage.getItem('vpos_carritoEnEspera');
  if (_cee) carritoEnEspera = JSON.parse(_cee);
} catch (e) {}

function pausarVentaActual() {
  try {
    if (!carrito.length) { toast('El carrito está vacío — no hay nada que poner en espera', true); return; }
    if (carritoEnEspera) { toast('⚠ Ya tienes una venta en espera — reanúdala antes de pausar otra', true); return; }
    carritoEnEspera = carrito.map(i => ({ ...i }));
    try { localStorage.setItem('vpos_carritoEnEspera', JSON.stringify(carritoEnEspera)); } catch (e) { console.warn('[pausarVentaActual] localStorage:', e); }
    carrito = [];
    renderCarrito();
    _actualizarBotonEspera();
    toast('⏸ Venta en espera guardada — atiende al otro cliente y luego reanúdala');
  } catch (e) {
    console.error('[pausarVentaActual]', e);
    toast('⚠ No se pudo pausar la venta — intenta de nuevo', true);
  }
}

// Reanuda la venta en espera. Esta función es 100% local (arreglos en
// memoria + localStorage) — NUNCA toca el login, el token, ni Supabase.
// Si al probar esto la sesión se cierra, NO es por esta función; revisa
// si coincide con el refresco automático del token (cada ~50 min) o con
// minimizar la app / usar el escáner de cámara.
function reanudarVentaEnEspera() {
  try {
    if (!carritoEnEspera) return;
    if (carrito.length) {
      // En vez de confirm() nativo (poco fiable en apps instaladas/PWA
      // standalone en Android — a veces no responde bien al toque),
      // usamos un diálogo propio de la app.
      _confirmarDescartarCarrito(() => _reanudarVentaEnEsperaConfirmado());
      return;
    }
    _reanudarVentaEnEsperaConfirmado();
  } catch (e) {
    console.error('[reanudarVentaEnEspera]', e);
    toast('⚠ No se pudo reanudar la venta — intenta de nuevo', true);
  }
}

function _reanudarVentaEnEsperaConfirmado() {
  try {
    // Igual que al cerrar una venta: devolver el stock reservado por el carrito que se descarta
    carrito.forEach(i => {
      const p = productos.find(x => x.id === i.id);
      if (p) p.stock += i.cant * (i.stockPorCant || 1);
    });
    carrito = carritoEnEspera;
    carritoEnEspera = null;
    try { localStorage.removeItem('vpos_carritoEnEspera'); } catch (e) { console.warn('[reanudarVentaEnEspera] localStorage:', e); }
    renderCarrito();
    _actualizarBotonEspera();
    toast('▶ Venta en espera reanudada');
  } catch (e) {
    console.error('[_reanudarVentaEnEsperaConfirmado]', e);
    toast('⚠ No se pudo reanudar la venta — intenta de nuevo', true);
  }
}

// Diálogo propio (no nativo) para confirmar que se descarta el carrito
// actual sin cobrar, antes de reanudar la venta en espera.
function _confirmarDescartarCarrito(onConfirm) {
  let modal = document.getElementById('modalDescartarCarrito');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'modalDescartarCarrito';
    modal.style.cssText = 'position:fixed;inset:0;z-index:10080;background:rgba(5,46,22,0.75);backdrop-filter:blur(6px);display:none;align-items:center;justify-content:center;padding:16px;';
    modal.innerHTML = `
      <div style="background:#fff;border-radius:20px;width:100%;max-width:360px;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
        <div style="background:linear-gradient(135deg,#9a3412,#ea580c);padding:20px 22px 16px;text-align:center;">
          <div style="font-size:30px;margin-bottom:6px;">⚠️</div>
          <div style="color:#fff;font-size:15px;font-weight:900;font-family:'Nunito',sans-serif;">Carrito actual sin cobrar</div>
        </div>
        <div style="padding:20px 22px;">
          <div style="font-size:14px;font-weight:700;color:#374151;font-family:'Nunito',sans-serif;text-align:center;margin-bottom:20px;line-height:1.5;">
            Tienes productos sin cobrar en el carrito actual. ¿Deseas descartarlos para reanudar la venta en espera?
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
            <button id="btnCancelarDescartarCarrito" style="padding:13px;background:#f9fafb;border:1.5px solid #e5e7eb;border-radius:12px;font-size:14px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#374151;">✕ Cancelar</button>
            <button id="btnConfirmarDescartarCarrito" style="padding:13px;background:linear-gradient(135deg,#ea580c,#c2410c);border:none;border-radius:12px;font-size:14px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#fff;box-shadow:0 4px 14px rgba(234,88,12,0.3);">Descartar y reanudar</button>
          </div>
        </div>
      </div>`;
    modal.addEventListener('click', e => { if (e.target === modal) modal.style.display = 'none'; });
  ...
  modal.style.display = 'flex';
  document.getElementById('btnCancelarDescartarCarrito').onclick = () => { modal.style.display = 'none'; };
  document.getElementById('btnConfirmarDescartarCarrito').onclick = () => { modal.style.display = 'none'; onConfirm(); };
}

function _actualizarBotonEspera() {
  const btn = document.getElementById('btnVentaEspera');
  if (!btn) return;
  if (carritoEnEspera && carritoEnEspera.length) {
    const total = carritoEnEspera.reduce((s, i) => s + (Number(i.venta) || 0) * (i.cant || 0), 0);
    btn.style.display = 'flex';
    btn.innerHTML = `▶ Reanudar ($${total.toFixed(2)})`;
    btn.onclick = reanudarVentaEnEspera;
    btn.title = 'Reanudar la venta que dejaste en espera';
  } else if (carrito.length) {
    btn.style.display = 'flex';
    btn.innerHTML = '⏸ Pausar';
    btn.onclick = pausarVentaActual;
    btn.title = 'Pausar esta venta para atender a otro cliente';
  } else {
    btn.style.display = 'none';
  }
}
window._actualizarBotonEspera = _actualizarBotonEspera;
...
function buscarV() {
  ...
  m.forEach(p => {
    ...
    const stockColor = sinStock ? 'color:#dc2626' : stockBajo ? 'color:#d97706' : 'color:#a16207';
...
function renderCarrito() {
  if (typeof _actualizarBotonEspera === 'function') _actualizarBotonEspera();
  ...
  resEl.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;padding:8px 0 2px;border-top:2px dashed #e5e7eb;margin-top:6px;text-align:center;">
      <div>
        <div style="font-size:9px;font-weight:900;color:#c2410c;text-transform:uppercase;">💰 Ingreso</div>
        <div style="font-size:13px;font-weight:900;color:#c2410c;">$${total.toFixed(2)}</div>
      </div>
      <div>
      ...
      <div style="background:${utilPos ? '#fffbeb' : '#fef2f2'};border-radius:8px;padding:2px 4px;">
        <div style="font-size:9px;font-weight:900;color:${utilPos ? '#65a30d' : '#dc2626'};text-transform:uppercase;">${utilPos ? '✅ Utilidad' : '🔴 Pérdida'}</div>
        <div style="font-size:13px;font-weight:900;color:${utilPos ? '#65a30d' : '#dc2626'};">${utilPos ? '+' : ''}$${utilCarrito.toFixed(2)}</div>
      </div>
    </div>`;
}
...
function abrirPickerPaquetes(p) {
  ...
  cont.innerHTML = opciones.map((op, i) => {
    ...
    const dispTxt  = op.disponibles > 0
      ? `<span style="font-size:10px;background:${isBase?'#f3f4f6':'#ffedd5'};color:${isBase?'#6b7280':'#c2410c'};border-radius:6px;padding:1px 6px;font-weight:800;">quedan ${op.disponibles}</span>`
      : '';
...
function finalizarVenta() {
  ...
  salvar(); renderCarrito(); cerrarModal('modalCobro');
...
function renderFacturaPreview(venta) {
  ...
  items.forEach(it => {
    ...
    rows += `<tr>
      ...
      <td style="padding:5px 8px;border-bottom:1px solid #e5e7eb;text-align:right;font-weight:900;color:#ea580c;">$${sub}</td>
    </tr>`;
  });
  document.getElementById('factPreviewBody').innerHTML = rows;
  document.getElementById('factPreviewTotal').textContent = '$' + venta.total;
  const hdrNombre = document.getElementById('factPreviewNombre');
  if (hdrNombre) hdrNombre.textContent = _getNombreTienda() || 'Despensa Económica';
}
...
function _construirPDFVenta(venta) {
  ...
  doc.text((_getNombreTienda() || 'Despensa Económica'), 14, 12);
...
function generarFacturaPDF() {
  ...
  if (_descargarPDF(doc, _nombreArchivoComprobante(cliente))) toast('✓ Registro descargado');
...
function actualizarCats() {
  ...
  const provs = [...new Set(productos.map(p => p.proveedor).filter(Boolean))].sort();
  ...
  if (selProv) {
    ...
    provs.forEach(pr => selProv.innerHTML += `<option value="${pr}">${pr}</option>`);
    selProv.value = provs.includes(valProv) ? valProv : 'todos';
  }

  // También poblar el selector de categoría/proveedor para PDF por rango
  ...
  const selPdfProv = document.getElementById('pdfProveedor');
  if (selPdfProv) {
    const valPdfProv = selPdfProv.value;
    selPdfProv.innerHTML = '<option value="todos">🏭 Todos los proveedores</option>';
    provs.forEach(pr => selPdfProv.innerHTML += `<option value="${pr}">${pr}</option>`);
    selPdfProv.value = provs.includes(valPdfProv) ? valPdfProv : 'todos';
  }
}
function renderVentas() {
  const f     = document.getElementById('selCat')?.value || 'todas';
  const fProv = document.getElementById('selProv')?.value || 'todos';
  const tbody = document.getElementById('tbodyVentas'); if (!tbody) return;
  // Enriquecer categoria/proveedor desde el array de productos si el reporte no la trae
  const rows = Object.values(ventasDia || {}).map(v => {
    const prod = productos.find(p => String(p.id) === String(v.id));
    return { ...v, cat: (prod && prod.cat) ? prod.cat : (v.cat || ''), proveedor: prod ? (prod.proveedor || '') : '' };
  }).filter(v => (f === 'todas' || (v.cat || '') === f) && (fProv === 'todos' || (v.proveedor || '') === fProv))
    .sort((a, b) => (b.total || 0) - (a.total || 0));
  if (!rows.length) { tbody.innerHTML = `<tr><td colspan="5"><div class="empty"><span class="empty-icon">📊</span>Sin ventas registradas</div></td></tr>`; return; }
  tbody.innerHTML = rows.map(v => {
    const catLabel = v.cat || '—';
    return `<tr><td class="td-bold">${v.nom||'—'}</td><td><span class="badge badge-green" style="font-size:11px;">${catLabel}</span></td><td style="font-size:11px;color:var(--text-muted);">${v.proveedor || '—'}</td><td class="mono">${Number(v.cant||0)}</td><td class="mono td-green">$${Number(v.total||0).toFixed(2)}</td></tr>`;
  }).join('');
}
  ...
  div.innerHTML = filtrado.map((v) => {
      ...
      return `
        <div style="display:flex;justify-content:space-between;align-items:flex-start;padding:5px 0;border-bottom:1px dashed #e5e7eb;gap:6px;">
          ...
          <div style="text-align:right;flex-shrink:0;">
            <div style="font-size:12px;font-weight:900;color:#c2410c;">$${itIngreso.toFixed(2)}</div>
            <div style="font-size:10px;color:#dc2626;">-$${itCosto.toFixed(2)}</div>
            <div style="font-size:11px;font-weight:900;color:${itPos ? '#65a30d' : '#dc2626'};">${itPos ? '+' : ''}$${itUtilidad.toFixed(2)}</div>
          </div>
        </div>`;
    }).join('');
    ...
    <div class="hist-item" style="padding:0;overflow:hidden;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;background:#f9fafb;border-top:2px solid #e5e7eb;padding:8px 12px;gap:4px;text-align:center;">
        <div>
          <div style="font-size:9px;font-weight:900;color:#c2410c;text-transform:uppercase;">💰 Ingreso</div>
          <div style="font-size:14px;font-weight:900;color:#c2410c;">$${ventaIngreso.toFixed(2)}</div>
        </div>
        <div>
          <div style="font-size:9px;font-weight:900;color:#dc2626;text-transform:uppercase;">📦 Costo</div>
          <div style="font-size:14px;font-weight:900;color:#dc2626;">$${ventaCosto.toFixed(2)}</div>
        </div>
        <div style="background:${esUtil ? '#fffbeb' : '#fef2f2'};border-radius:8px;padding:2px 4px;">
          <div style="font-size:9px;font-weight:900;color:${esUtil ? '#65a30d' : '#dc2626'};text-transform:uppercase;">${esUtil ? '✅ Utilidad' : '🔴 Pérdida'}</div>
          <div style="font-size:14px;font-weight:900;color:${esUtil ? '#65a30d' : '#dc2626'};">${esUtil ? '+' : ''}$${ventaUtilidad.toFixed(2)}</div>
        </div>
      </div>
...
function borrarGasto(id) {
  _requierePin(() => _borrarGastoReal(id));
}
function _borrarGastoReal(id) {
...
// ── Descarga robusta de un Blob: crea el link DENTRO del DOM antes del
// click (algunos navegadores móviles ignoran el click si el <a> no está
// insertado), usa una URL de blob (más confiable que toDataURL/base64 en
// archivos grandes) y libera la memoria después. Devuelve true/false para
// que quien llama solo muestre "descargado" si realmente se disparó.
function _descargarBlob(blob, filename) {
  ...
  try {
    ...
    console.error('[_descargarBlob]', e);
    return false;
  }
}

// ── Descarga un documento jsPDF usando el mismo mecanismo robusto de
// arriba, en vez de doc.save() (que en algunos Android/Chrome termina sin
// error pero sin descargar realmente el archivo).
function _descargarPDF(doc, filename) {
  try {
    const blob = doc.output('blob');
    const ok = _descargarBlob(blob, filename);
    if (!ok) toast('⚠ No se pudo descargar el PDF', true);
    return ok;
  } catch (e) {
    console.error('[_descargarPDF]', e);
    toast('⚠ Error al generar el PDF', true);
    return false;
  }
}

// ── Convierte un dataURL (base64) a Blob de forma SÍNCRONA. Se usa para
// que la descarga de imágenes use blob: URLs igual que los PDFs (que sí
// funcionan) en vez de data: URLs, que Chrome bloquea con más frecuencia.
...
// ── Descarga un <canvas> como PNG. toDataURL es SÍNCRONO (a diferencia de
// toBlob, que entrega el resultado en un callback asíncrono y para
// entonces el navegador ya no reconoce la descarga como resultado directo
// del toque del usuario). Convertimos ese dataURL a Blob también de forma
// síncrona y lo descargamos como blob: URL — el mismo mecanismo que ya
// funciona de forma confiable en los PDFs de la app.
function _descargarCanvasPNG(canvas, filename, cb) {
  try {
    const dataURL = canvas.toDataURL('image/png');
    const blob = _dataURLaBlob(dataURL);
    const ok = _descargarBlob(blob, filename);
    if (!ok) toast('⚠ No se pudo descargar la imagen', true);
    if (typeof cb === 'function') cb(ok);
  } catch (e) {
    console.error('[_descargarCanvasPNG]', e);
    toast('⚠ Error al generar la imagen', true);
...
function _pdfHeader(doc, titulo, subtitulo, ancho) {
  ...
  doc.text((_getNombreTienda() || 'Despensa Económica').toUpperCase(), 12, 10);
  doc.setFontSize(10);
...
function generarPDFInventarioActual() {
  ...
  for (const cat of Object.keys(porCat).sort()) {
    ...
    y = _pdfAutoTable(doc,
      [[{ content: cat, colSpan: 6, styles: { fillColor:[234,88,12], textColor:[255,255,255], fontStyle:'bold', fontSize:9 }}],
  ...
  if (_descargarPDF(doc, `Inventario_${now.toLocaleDateString('es-SV').replace(/\//g,'-')}.pdf`)) {
    toast('✓ PDF de inventario descargado');
  }
}

function _pdfBuildReporte(doc, titulo, subtitulo, acumData, catFiltro, opts = {}, provFiltro = 'todos') {
  ...
  Object.values(acumData || {}).forEach(v => {
    ...
    if (provFiltro && provFiltro !== 'todos') {
      const prodChk = v.id ? productos.find(p => String(p.id) === String(v.id)) : null;
      if (!prodChk || (prodChk.proveedor || '') !== provFiltro) return;
    }
    ...
    const nomConProveedor = (prod && prod.proveedor) ? `${v.nom || '—'} — ${prod.proveedor}` : (v.nom || '—');
  ...
  for (const cat of Object.keys(cats).sort()) {
    ...
    y = _pdfAutoTable(doc,
      [[{ content: cat, colSpan: colSpanN, styles:{fillColor:[234,88,12],textColor:[255,255,255],fontStyle:'bold',fontSize:9}}],
       headerRow],
      rows, y, {
...
function generarPDF(tipo) {
  ...
  const provFiltro = document.getElementById('selProv')?.value || 'todos';
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF({ unit: 'mm', format: 'a4' });
  const now = new Date();
  const subtitulo = `${tipo.charAt(0).toUpperCase()+tipo.slice(1)} · ${now.toLocaleDateString('es-SV')} ${now.toLocaleTimeString('es-SV')}${provFiltro!=='todos' ? ' · Proveedor: '+provFiltro : ''}`;
  ...
  if (tipo === 'mensual' || tipo === 'semanal') {
    ...
    _pdfBuildReporte(doc, `REPORTE ${tipo.toUpperCase()}`, subtitulo, acum, filtro, { conGanancia: true }, provFiltro);
    if (_descargarPDF(doc, `Despensa_${tipo}_${now.toLocaleDateString('es-SV').replace(/\//g,'-')}.pdf`)) {
  ...
  _pdfBuildReporte(doc, `REPORTE ${tipo.toUpperCase()}`, subtitulo, acum, filtro, {}, provFiltro);
  if (_descargarPDF(doc, `Despensa_${tipo}_${now.toLocaleDateString('es-SV').replace(/\//g,'-')}.pdf`)) {
    toast(`✓ PDF ${tipo} descargado`);
  }
}
...
function generarPDFRango() {
  ...
  const provFiltro = document.getElementById('pdfProveedor')?.value || 'todos';
  ...
  const catLabel = (catFiltro === 'todas' ? 'Todas las categorías' : catFiltro) +
    (provFiltro !== 'todos' ? ' · Proveedor: ' + provFiltro : '');
  ...
  const { totalInvertidoGlobal, totalGananciaGlobal } =
    _pdfBuildReporte(doc, 'REPORTE POR PERÍODO', subtitulo, acum, catFiltro, { conGanancia: true, descripcion }, provFiltro);
  ...
  if (_descargarPDF(doc, `Reporte_${desdeVal}_${hastaVal}${catFiltro!=='todas'?'_'+catFiltro:''}${provFiltro!=='todos'?'_'+provFiltro:''}.pdf`)) {
...
function reiniciarDia() {
  _requierePin(() => _reiniciarDiaReal());
}
function _reiniciarDiaReal() {
...
function abrirModalReiniciarSemana() {
  _requierePin(() => _abrirModalReiniciarSemanaReal());
}
function _abrirModalReiniciarSemanaReal() {
  if (typeof _puedeHacer === 'function' && !_puedeHacer('reiniciar')) { toast('Solo el Admin puede reiniciar', true); return; }
  const catsEnSem = {};
...
function reiniciarHistorial() {
  _requierePin(() => _reiniciarHistorialReal());
}
function _reiniciarHistorialReal() {
...
function guardarEdicionCobro() {
  _requierePin(() => _guardarEdicionCobroReal());
}
function _guardarEdicionCobroReal() {
  const v = historial[_editCobroIdx];
  ...
  if (!activos.length) {
    ...
    // SYNC FIX: antes esto solo se aplicaba en memoria/broadcast — si el otro
    // teléfono no estaba conectado en ese instante, la venta devuelta nunca
    // se borraba de Supabase y volvía a aparecer en su próxima sincronización.
    // Ahora se borra también en la nube para que cualquier teléfono (conectado
    // o no en el momento) quede correcto al refrescar.
    if (_cobroId && typeof _sbDeleteFiltro === 'function') {
      _sbDeleteFiltro('ventas', { id: 'eq.' + _cobroId }).catch(() => {});
    }
  } else {
  ...
  console.log(`[Diag-STOCK] guardarEdicionCobro EJECUTADO — cobro id=${v.id}, items a devolver=${devolver.length}`);
  ...
  if (fechaDev && montoDevuelto > 0) {
    ...
    if (vdIdx >= 0) {
      ...
      ventasDiarias[vdIdx] = { ...ventasDiarias[vdIdx], monto: nuevoMonto, _auto: true, _ts: Date.now() };
  ...
  if (typeof syncAhora === 'function') syncAhora('productos');
  // SYNC FIX: subir el historial actualizado (venta reducida o eliminada) a
  // Supabase. Antes solo se enviaba por broadcast en vivo, así que un
  // teléfono desconectado en ese momento nunca se enteraba de la devolución.
...
function actualizarSubtituloBackup() {
  ...
  if (alert) alert.style.display = 'none'; // aviso ocultado permanentemente por decisión del dueño de la tienda
  if (!ultimo) { sub.textContent = 'Nunca has exportado un backup'; return; }
...
function confirmarFusion() {
  ...
  (ext.ventasDiarias || []).forEach(vExt => {
    ...
    if (idx >= 0) {
      ...
      // FIX SYNC: estampar _ts = Date.now() aquí también. Antes esta línea
      // cambiaba `monto` sin tocar `_ts`, así que el dato fusionado se subía
      // a Supabase con una marca de tiempo vieja (heredada de una sync
      // anterior) y el merge "última escritura gana" de sync2.js lo
      // descartaba en el otro teléfono creyendo que era un dato viejo.
      ...
      ventasDiarias[idx]._ts   = Date.now();
    } else {
...
function abrirModalReiniciarMes() {
  _requierePin(() => _abrirModalReiniciarMesReal());
}
function _abrirModalReiniciarMesReal() {
...
function generarEstadoResultadosPDF() {
  ...
  doc.setFontSize(10); doc.setFont('helvetica','normal');
  doc.text((_getNombreTienda() || 'Despensa Económica') + ' · ' + (mesNombre.charAt(0).toUpperCase()+mesNombre.slice(1)) + '   |   Generado: ' + now.toLocaleString('es-SV'), w/2, 18, {align:'center'});
  ...
  if (ventasDiariasMes.length) {
    ...
    doc.autoTable({
      ...
      footStyles: { fillColor: [220,252,231], textColor: [63,98,18], fontStyle: 'bold' }
    });
    y = doc.lastAutoTable.finalY + 10;
  } else {
  ...
  doc.text((_getNombreTienda() || 'Despensa Económica') + ' — Estado de Resultados ' + mesClave, w/2, y, {align:'center'});

  if (_descargarPDF(doc, `Estado_Resultados_${mesClave}.pdf`)) {
...
function generarImagenEstadoResultados() {
  ...
  ctx.fillText(`${_getNombreTienda() || 'Despensa Económica'} · ${mesCap}`, W/2, 140);
  ...
  // 1. Ingresos y Costo de Ventas
  sectionHeader('INGRESOS Y COSTO DE VENTAS', '#c2410c');
  row('Ventas totales del mes', '$'+ingresoRealMes.toFixed(2), '#c2410c');
  row('Menos costo de ventas (COGS)', '-$'+cogsMes.toFixed(2), '#dc2626');
  rowTotal('UTILIDAD BRUTA: $'+utilidadBruta.toFixed(2), '#c2410c');
  ...
  row('Utilidad bruta', '$'+utilidadBruta.toFixed(2), '#c2410c');
  row('Menos gastos operativos', '-$'+totalGastosOperativos.toFixed(2), '#dc2626');
  rowTotal((utilidadNetaMes>=0?'UTILIDAD NETA: $':'PÉRDIDA NETA: $')+Math.abs(utilidadNetaMes).toFixed(2), utilidadNetaMes>=0?'#c2410c':'#dc2626');
  ...
  row('Ganancia ya realizada (ventas)', '$'+gananciaEfectivoMes.toFixed(2), '#c2410c');
  row('Meta mínima establecida', '$'+objetivoMeta.toFixed(2), '#7c3aed');
  rowTotal((metaCumplida?'META CUMPLIDA: $':'META NO CUMPLIDA: $')+gananciaGeneralMes.toFixed(2), metaCumplida?'#c2410c':'#dc2626');

  // 5. Lo Que Tengo en Mi Tienda
  sectionHeader('LO QUE TENGO EN MI TIENDA', '#0891b2');
  row('Efectivo inicial del mes', '$'+efectivoInicialReal.toFixed(2), '#0891b2');
  row('Mas ventas cobradas', '+$'+ingresoRealMes.toFixed(2), '#c2410c');
  ...
  if (rankingProductosMes.length) {
    rankingProductosMes.slice(0, 5).forEach((p, i) => {
      row(`${i+1}. ${p.nom}`, '$'+p.ganancia.toFixed(2), p.ganancia >= 0 ? '#c2410c' : '#dc2626');
    });
  } else {
  ...
  if (rankingGananciaUnidad.length) {
    ctx.font = 'bold 24px Arial'; ctx.fillStyle = '#c2410c'; ctx.textAlign = 'left';
    ctx.fillText('Mejor margen:', PAD + 8, y); y += 40;
    rankingGananciaUnidad.slice(0, 3).forEach(p => row(p.nom, '$'+p.ganancia.toFixed(2), '#c2410c'));
  ...
  // 8. Ventas por Día
  sectionHeader('VENTAS POR DÍA DEL MES', '#c2410c');
  if (ventasDiariasMes.length) {
    const totalVD = ventasDiariasMes.reduce((s, v) => s + Number(v.monto || 0), 0);
    ventasDiariasMes.slice(-6).forEach(v => row(v.fecha, '$'+Number(v.monto||0).toFixed(2), '#c2410c'));
    rowTotal('TOTAL VENTAS POR DÍA: $'+totalVD.toFixed(2), '#c2410c');
  } else {
    ctx.font = '24px Arial'; ctx.fillStyle = '#9ca3af'; ctx.textAlign = 'center';
  ...
  ctx.fillText(`${_getNombreTienda() || 'Despensa Económica'} — Estado de Resultados ${mesClave}`, W/2, H - 40);

  _descargarCanvasPNG(canvas, `Estado_Resultados_${mesClave}.png`, (ok) => {
    if (ok) toast('🖼️ Imagen del Estado de Resultados descargada');
  });
}

function ejecutarReiniciarMes(remoto) {
  // SYNC FIX: parámetro "remoto" = true cuando esta función se ejecuta porque
  // llegó el evento Realtime 'reinicio_mes' desde OTRO teléfono (ver
  // supabase_sync.js). En ese caso no se debe pedir confirmación, ni volver a
  // generar/descargar el PNG y el JSON (ya se generaron en el teléfono que
  // presionó el botón), ni volver a emitir broadcasts (evita bucles).
  // Cuando se llama normalmente desde el botón (onclick="ejecutarReiniciarMes()"),
  // remoto es undefined y todo el comportamiento es EXACTAMENTE el mismo de siempre.
  if (!remoto) {
  ...
  if (!remoto) {
  try { generarImagenEstadoResultados(); } catch (e) { console.error('Error generando imagen del Estado de Resultados:', e); toast('⚠ No se pudo generar la imagen resumen', true); }
  }
  ...
  const _tsReinicio = Date.now();
  _fechasVDBorradas.forEach(f => {
    ventasDiariasEliminadas = (ventasDiariasEliminadas || []).filter(x => (typeof x === 'string' ? x : x.f) !== f);
    ventasDiariasEliminadas.push({ f, ts: _tsReinicio });
  });
  ...
  salvar();
  // SYNC FIX (red de respaldo): guardamos el timestamp de este reinicio.
  // Se usa para que, si el broadcast en vivo se pierde (canal caído en ese
  // instante), el sondeo periódico normal (AutoCarga) detecte que hay un
  // reinicio más reciente en Supabase y lo aplique aunque el broadcast
  // nunca haya llegado. Ver bloque "reinicioMesTs" en supabase_sync.js.
  const _reinicioMesTs = new Date().toISOString();
  localStorage.setItem('vpos_reinicioMesTs', _reinicioMesTs);
  if (!remoto) { cerrarModal('modalReiniciarMes'); toast('Mes reiniciado — inventario intacto', false, true); }
  else { toast('🗓 Mes reiniciado desde otro teléfono', false, true); }
  ...
  // SYNC FIX: los pasos 1️⃣, 2️⃣ y 3️⃣ de abajo son los que propagan el reinicio
  // a otros teléfonos. Si esta ejecución YA vino de un evento remoto (remoto
  // === true), NO se debe repetir ninguno de estos pasos: evita el bucle de
  // broadcasts infinito (teléfono A → B → A → B...) y evita subidas duplicadas
  // a Supabase. El teléfono que presionó el botón (remoto falsy) es el único
  // que emite estos broadcasts/uploads.
  if (!remoto) {
  ...
  // 4️⃣ SYNC FIX: guardar el marcador reinicioMesTs en la tabla "config".
  // Esta tabla YA se recarga sola en cada AutoCarga periódica (independiente
  // del canal de broadcast), así que si el paso 1️⃣ se pierde por una caída
  // de conexión, el otro teléfono igual detecta el reinicio en su próximo
  // sondeo automático y lo aplica (ver bloque en supabase_sync.js).
  if (typeof _sbPost === 'function' && typeof _getTiendaId === 'function') {
    const _tid = _getTiendaId();
    if (_tid) {
      _sbPost('config', { clave: 'reinicioMesTs', tienda_id: _tid, valor: _reinicioMesTs, updated_at: _reinicioMesTs }, true).catch(() => {});
    }
  }
  }
}
...
function guardarVentaDiaria() {
  ...
  if (idx >= 0) {
    ...
    ventasDiarias[idx] = { fecha, monto, nota, _ts: Date.now() };
  } else {
    ventasDiarias.push({ fecha, monto, nota, _ts: Date.now() });
  }
  ventasDiarias.sort((a, b) => a.fecha.localeCompare(b.fecha));
  salvar(false);
  idbSetMany([['vpos_ventasDiarias', ventasDiarias]]).catch(console.error);
...
function eliminarVentaDiaria(fecha) {
  _requierePin(() => _eliminarVentaDiariaReal(fecha));
}
function _eliminarVentaDiariaReal(fecha) {
  ...
  // BUGFIX SYNC: guardar fecha en tombstone (con hora) para que _fusionarDos
  // no la restaure desde snapshots viejos, pero SIN bloquear ventas nuevas
  // legítimas hechas después de este momento.
  const _tsBorrado = Date.now();
  ventasDiariasEliminadas = (ventasDiariasEliminadas || []).filter(x => (typeof x === 'string' ? x : x.f) !== fecha);
  ventasDiariasEliminadas.push({ f: fecha, ts: _tsBorrado });
  salvar(false);
  idbSetMany([['vpos_ventasDiariasElim', ventasDiariasEliminadas]]).catch(() => {});
  if (typeof _broadcast === 'function') _broadcast('ventas_dia_eliminada', { fecha, ts: _tsBorrado, ventasDiariasEliminadas });
...
function renderVentasDiarias() {
  ...
  tbody.innerHTML = listaMostrar.map(v => {
    ...
    return `<tr${esHoyFlag ? ' style="background:#fffbeb;"' : ''}>
...
function actualizarTodo() {
  ...
  if (pgId === 'pgEntradasHoy' && typeof renderEntradasHoy === 'function') renderEntradasHoy();
...
function renderPagina(pgId) {
  ...
  if (pgId === 'pgEntradasHoy') { if (typeof renderEntradasHoy === 'function') renderEntradasHoy(); }
...
// =====================================================================
//  📋 PEDIDOS ONLINE — Panel en el POS (conectado a Supabase)
// =====================================================================
let _ordersPolling = null;
let _pedidosOnlineCache = [];
...
async function _checkPedidosNuevos() {
  try {
    ...
    const tid = _getTiendaId();
    if (!tid) return;
    const pedidos = await _sbGet('pedidos_online', {
      select: '*', tienda_id: 'eq.' + tid, order: 'fecha_iso.desc', limit: '100'
    });
    _pedidosOnlineCache = pedidos || [];
    const fab = document.getElementById('ordersFab');
    if (!fab) return;
    const pendientes = _pedidosOnlineCache.filter(p => p.estado === 'nuevo').length;
    if (pendientes > 0) {
  ...
  } catch(e) { console.warn('[_checkPedidosNuevos]', e.message); }
}
...
async function renderPedidosAdmin() {
  ...
  w.innerHTML = '<div style="padding:22px;text-align:center;color:var(--text-muted);font-size:13px;font-weight:700;">Cargando…</div>';
  try {
    const tid = _getTiendaId();
    const pedidos = await _sbGet('pedidos_online', {
      select: '*', tienda_id: 'eq.' + tid, order: 'fecha_iso.desc', limit: '100'
    ...
    w.innerHTML = _pedidosOnlineCache.map((p) => {
      const stLbl = { nuevo: '🕐 Nuevo', aceptado: '✅ Aceptado', rechazado: '❌ Rechazado' }[p.estado] || 'Nuevo';
      const items = (p.items || []).map(i => `${i.cant}× ${i.nom}`).join(', ');
      const fechaStr = new Date(p.fecha_iso).toLocaleString('es-SV');
      const acc = p.estado === 'nuevo' ? `<div class="op-actions">
        <button class="op-edit" onclick="abrirEditarPedido('${p.id}')">✏️ Editar productos</button>
        <div style="display:flex;gap:8px;">
          <button class="op-acc" onclick="responderPedidoPOS('${p.id}','aceptado')">✅ Aceptar</button>
          <button class="op-rej" onclick="responderPedidoPOS('${p.id}','rechazado')">❌ Rechazar</button>
        </div>
      </div>` : '';
      return `<div class="op-card">
        ...
        <div class="op-body">
          <b>${p.nombre}</b>${p.tel ? ' · 📞 ' + p.tel : ''}<br>
          🏪 Retiro en tienda<br>
          💰 $${Number(p.total).toFixed(2)}<br>
    ...
    w.innerHTML = '<div style="padding:22px;text-align:center;color:var(--text-muted);">Error al cargar pedidos: ' + e.message + '</div>';
  }
}

// ── Editar productos de un pedido (agregar/quitar) antes de aceptarlo ──
let _editPedido = null;      // el pedido que se está editando (copia de trabajo)
let _editPedidoItems = [];   // items en edición (array de {id,nom,cant,precio,cat})

function abrirEditarPedido(pedidoId) {
  const pedido = _pedidosOnlineCache.find(p => String(p.id) === String(pedidoId));
  if (!pedido) { toast('Pedido no encontrado', true); return; }
  _editPedido = pedido;
  _editPedidoItems = (pedido.items || []).map(i => ({ ...i, id: String(i.id) }));
  _renderModalEditarPedido();
}

function _renderModalEditarPedido() {
  let modal = document.getElementById('modalEditarPedido');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'modalEditarPedido';
    modal.style.cssText = 'position:fixed;inset:0;z-index:10085;background:rgba(5,46,22,0.75);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;padding:16px;';
    modal.addEventListener('click', e => { if (e.target === modal) cerrarEditarPedido(); });
    document.body.appendChild(modal);
  }

  const subtotal = _editPedidoItems.reduce((s, i) => s + i.precio * i.cant, 0);
  const filasItems = _editPedidoItems.length ? _editPedidoItems.map(i => `
    <div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid #f3f4f6;">
      <div style="flex:1;min-width:0;">
        <div style="font-size:13px;font-weight:800;color:#292524;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${i.nom}</div>
        <div style="font-size:11px;color:#78716c;font-weight:700;">$${Number(i.precio).toFixed(2)} c/u</div>
      </div>
      <button onclick="_editPedidoCambiarCant('${i.id}',-1)" style="width:28px;height:28px;border:1.5px solid #e5e7eb;background:#f9fafb;border-radius:8px;font-weight:900;font-size:15px;cursor:pointer;">−</button>
      <span style="min-width:22px;text-align:center;font-weight:900;font-size:14px;">${i.cant}</span>
      <button onclick="_editPedidoCambiarCant('${i.id}',1)" style="width:28px;height:28px;border:1.5px solid #e5e7eb;background:#f9fafb;border-radius:8px;font-weight:900;font-size:15px;cursor:pointer;">+</button>
      <button onclick="_editPedidoQuitar('${i.id}')" style="width:28px;height:28px;border:none;background:#fee2e2;color:#dc2626;border-radius:8px;font-weight:900;font-size:13px;cursor:pointer;">✕</button>
    </div>`).join('') : '<div style="padding:16px 0;text-align:center;color:#a8a29e;font-size:13px;font-weight:700;">Sin productos — agrega alguno abajo</div>';

  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px;width:100%;max-width:420px;max-height:88vh;display:flex;flex-direction:column;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
      <div style="background:linear-gradient(135deg,#92400e,#b45309);padding:16px 18px;display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
        <div>
          <div style="color:#fff;font-size:15px;font-weight:900;font-family:'Nunito',sans-serif;">✏️ Editar Pedido</div>
          <div style="color:rgba(255,255,255,0.85);font-size:11px;font-weight:700;">${_editPedido.id} · ${_editPedido.nombre}</div>
        </div>
        <button onclick="cerrarEditarPedido()" style="background:rgba(255,255,255,0.2);border:none;color:#fff;width:30px;height:30px;border-radius:8px;font-size:15px;cursor:pointer;">✕</button>
      </div>
      <div style="padding:14px 18px;overflow-y:auto;flex:1;">
        <div style="font-size:11px;font-weight:900;color:#92400e;text-transform:uppercase;margin-bottom:4px;">Productos del pedido</div>
        <div id="editPedidoItems">${filasItems}</div>

        <div style="font-size:11px;font-weight:900;color:#92400e;text-transform:uppercase;margin:16px 0 6px;">Agregar producto</div>
        <input id="editPedidoBuscar" type="text" placeholder="Buscar producto…" oninput="_editPedidoBuscarProductos(this.value)"
          style="width:100%;box-sizing:border-box;padding:10px 12px;border:1.5px solid #e5e7eb;border-radius:10px;font-size:13px;font-family:'Nunito',sans-serif;">
        <div id="editPedidoResultados" style="max-height:160px;overflow-y:auto;"></div>
      </div>
      <div style="padding:14px 18px;border-top:1px solid #f3f4f6;flex-shrink:0;">
        <div style="display:flex;justify-content:space-between;font-size:14px;font-weight:900;color:#292524;margin-bottom:12px;">
          <span>Nuevo total</span><span id="editPedidoTotal">$${subtotal.toFixed(2)}</span>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          <button onclick="cerrarEditarPedido()" style="padding:13px;background:#f9fafb;border:1.5px solid #e5e7eb;border-radius:12px;font-size:13px;font-weight:900;cursor:pointer;color:#374151;">Cancelar</button>
          <button id="btnGuardarEdicionPedido" onclick="_editPedidoGuardar()" style="padding:13px;background:linear-gradient(135deg,#ea580c,#c2410c);border:none;border-radius:12px;font-size:13px;font-weight:900;cursor:pointer;color:#fff;">💾 Guardar cambios</button>
        </div>
      </div>
    </div>`;
  modal.style.display = 'flex';
...
function cerrarEditarPedido() {
  ...
  _editPedido = null; _editPedidoItems = [];
}

function _editPedidoCambiarCant(id, delta) {
  const it = _editPedidoItems.find(i => i.id === id);
  if (!it) return;
  it.cant += delta;
  if (it.cant <= 0) _editPedidoItems = _editPedidoItems.filter(i => i.id !== id);
  _renderModalEditarPedido();
}
function _editPedidoQuitar(id) {
  _editPedidoItems = _editPedidoItems.filter(i => i.id !== id);
  _renderModalEditarPedido();
}

function _editPedidoBuscarProductos(texto) {
  const w = document.getElementById('editPedidoResultados');
  const q = (texto || '').trim().toLowerCase();
  if (!q) { w.innerHTML = ''; return; }
  const yaEnPedido = new Set(_editPedidoItems.map(i => i.id));
  const resultados = (productos || [])
    .filter(p => (p.nom || '').toLowerCase().includes(q) && (p.stock || 0) > 0)
    .slice(0, 8);
  if (!resultados.length) { w.innerHTML = '<div style="padding:10px 0;color:#a8a29e;font-size:12px;font-weight:700;">Sin resultados</div>'; return; }
  w.innerHTML = resultados.map(p => `
    <div onclick="_editPedidoAgregarProducto('${p.id}')" style="display:flex;justify-content:space-between;align-items:center;padding:9px 4px;border-bottom:1px solid #f3f4f6;cursor:pointer;">
      <div>
        <div style="font-size:13px;font-weight:800;color:#292524;">${p.nom}${yaEnPedido.has(String(p.id)) ? ' ✓' : ''}</div>
        <div style="font-size:11px;color:#78716c;font-weight:700;">$${Number(p.venta).toFixed(2)} · stock ${p.stock}</div>
      </div>
      <div style="color:#ea580c;font-weight:900;font-size:18px;">+</div>
    </div>`).join('');
}

function _editPedidoAgregarProducto(prodId) {
  const p = (productos || []).find(x => String(x.id) === String(prodId));
  if (!p) return;
  const existente = _editPedidoItems.find(i => i.id === String(p.id));
  if (existente) { existente.cant += 1; }
  else { _editPedidoItems.push({ id: String(p.id), nom: p.nom, cant: 1, precio: p.venta, cat: p.cat || '' }); }
  document.getElementById('editPedidoBuscar').value = '';
  document.getElementById('editPedidoResultados').innerHTML = '';
  _renderModalEditarPedido();
}

async function _editPedidoGuardar() {
  if (!_editPedido) return;
  const btn = document.getElementById('btnGuardarEdicionPedido');
  btn.disabled = true; btn.textContent = 'Guardando…';
  try {
    await _sbRpc('editar_pedido_online', {
      p_pedido_id: _editPedido.id,
      p_nuevos_items: _editPedidoItems.map(i => ({ id: i.id, nom: i.nom, cant: i.cant, precio: i.precio, cat: i.cat || '' }))
    });
    toast('✏️ Pedido actualizado');
    cerrarEditarPedido();
    await renderPedidosAdmin();
    if (typeof _autoCargarDesdeSupa === 'function') await _autoCargarDesdeSupa();
    if (typeof renderInv === 'function') renderInv();
  } catch (e) {
    btn.disabled = false; btn.textContent = '💾 Guardar cambios';
    const msg = e.message.includes('SIN_STOCK') ? e.message.replace('SIN_STOCK: ', '⚠️ ') : ('Error: ' + e.message);
    toast(msg, true);
  }
}

async function responderPedidoPOS(pedidoId, decision) {
  try {
    const pedido = _pedidosOnlineCache.find(p => String(p.id) === String(pedidoId));
    if (!pedido) { toast('Pedido no encontrado', true); return; }

    if (decision === 'rechazado') {
      // La función en el servidor devuelve el stock reservado y marca el
      // pedido como rechazado — todo de forma atómica y segura.
      await _sbRpc('rechazar_pedido_online', { p_pedido_id: pedido.id });
      toast('❌ Pedido rechazado — stock devuelto');
    } else {
      // Aceptar: el stock YA se descontó cuando el cliente hizo el pedido
      // (no se vuelve a tocar aquí). Registramos la venta EXACTAMENTE
      // igual que finalizarVenta() lo hace para una venta normal del POS,
      // usando las mismas variables globales (ventasDia/Sem/Mes,
      // historial) para que Reportes, Venta Hoy y Estado de Caja la
      // reflejen de inmediato.
      if (typeof _validarFechaReportes === 'function') _validarFechaReportes();

      const venta = {
        id: pedido.id, ts: Date.now(), fechaISO: new Date().toISOString(),
        fechaStr: new Date(pedido.fecha_iso).toLocaleString('es-SV'),
        origen: 'tienda_online',
        cliente: pedido.nombre, telefono: pedido.tel || '',
        direccion: 'Retiro en tienda', delivery: 'retiro', nota: pedido.nota || '',
        pago: pedido.pago, envio: pedido.envio || 0,
      ...
      // Registra la venta directo en Supabase de inmediato (no depender
      // solo del sync periódico) — igual que el resto de la contabilidad.
      try {
        await _sbPost('ventas', {
          id: String(venta.id),
          fecha_iso: venta.fechaISO,
          total: parseFloat(venta.total) || 0,
          pago: parseFloat(venta.pago_monto) || 0,
          vuelto: 0,
          tienda_id: _getTiendaId(),
          items: venta.items.map(x => x.cant + 'x ' + x.nom).join(' | '),
          items_json: JSON.stringify(venta.items)
        }, true);
      } catch (e) { console.warn('[responderPedidoPOS] venta a Supabase falló, quedará en el sync general:', e.message); }

      // FIX (igual que finalizarVenta): registrar la venta diaria ANTES
      // de salvar(), para que Estado de Caja quede correcto de inmediato.
      if (typeof autoRegistrarVentaDiaria === 'function') autoRegistrarVentaDiaria();
      salvar();

      await _sbPatch('pedidos_online', { id: 'eq.' + pedido.id }, { estado: 'aceptado' });
      if (typeof _broadcast === 'function') _broadcast('venta', { ...venta, items_json: JSON.stringify(venta.items || []) });
      toast('✅ Pedido aceptado — venta registrada');
    }

    await renderPedidosAdmin();
    await _checkPedidosNuevos();
    // Refresca el stock mostrado en pantalla con el valor real del servidor
    if (typeof _autoCargarDesdeSupa === 'function') await _autoCargarDesdeSupa();
    if (typeof renderInv === 'function') renderInv();
...
// Revisa el cambio de día mientras la app queda abierta (por si nadie
// hace ninguna venta justo después de medianoche) — así "Entradas de
// Hoy" se limpia sola aunque el negocio esté cerrado en ese momento.
setInterval(() => { if (typeof _validarFechaReportes === 'function') _validarFechaReportes(); }, 5 * 60 * 1000);
...
window.cerrarEditarPedido       = cerrarEditarPedido;
window._editPedidoCambiarCant   = _editPedidoCambiarCant;
window._editPedidoQuitar        = _editPedidoQuitar;
window._editPedidoBuscarProductos = _editPedidoBuscarProductos;
window._editPedidoAgregarProducto = _editPedidoAgregarProducto;
window._editPedidoGuardar       = _editPedidoGuardar;
...
function _rp_poblarCatFiltro(valorPrevio) {
  ...
  const selProv = document.getElementById('rp_provFiltro');
  if (selProv) {
    const provs = [...new Set(productos.map(p => p.proveedor).filter(Boolean))].sort();
    const valorPrevioProv = selProv.value;
    selProv.innerHTML = '<option value="todos">🏭 Todos los proveedores</option>';
    provs.forEach(pr => selProv.innerHTML += `<option value="${pr}">${pr}</option>`);
    const preferidoProv = valorPrevioProv || document.getElementById('pdfProveedor')?.value || 'todos';
    selProv.value = provs.includes(preferidoProv) ? preferidoProv : 'todos';
  }
  return sel.value;
}

function _rp_provFiltroActual() { return document.getElementById('rp_provFiltro')?.value || 'todos'; }
function _rp_agruparActual()    { return document.getElementById('rp_agrupar')?.value || 'cat'; }
function _rp_grupoDe(p, agruparPor) { return agruparPor === 'proveedor' ? (p.proveedor || 'SIN PROVEEDOR') : (p.cat || 'SIN CATEGORÍA'); }
...
function _rp_render() {
  ...
  // Poblar/conservar los filtros propios del modal
  const catFiltro  = _rp_poblarCatFiltro(document.getElementById('rp_catFiltro')?.value);
  const provFiltro = _rp_provFiltroActual();
  const agruparPor = _rp_agruparActual();
  ...
  let listaConVentas = (productos || []).filter(p => {
    ...
    if (provFiltro !== 'todos' && (p.proveedor || '') !== provFiltro) return false;
    return acum[String(p.id)] && acum[String(p.id)].cant > 0;
  });
  listaConVentas = listaConVentas.slice().sort((a,b) => (_rp_grupoDe(a,agruparPor)||'').localeCompare(_rp_grupoDe(b,agruparPor)||'') || (a.nom||'').localeCompare(b.nom||''));
  ...
  if (tbody1) {
    ...
    if (!listaConVentas.length) {
      ...
      listaConVentas.forEach(p => {
        const cat = _rp_grupoDe(p, agruparPor);
        if (cat !== lastCat) {
          rows += `<tr><td colspan="6" style="background:#ffedd5;color:#9a3412;font-size:11px;font-weight:900;padding:6px 10px;border-bottom:1px solid #fdba74;">${agruparPor==='proveedor'?'🏭':'📦'} ${cat}</td></tr>`;
          lastCat = cat;
        }
        ...
        rows += `<tr style="border-bottom:1px solid #fffbeb;${stockBajo?'background:#fef2f2;':''}">
          <td style="padding:7px 10px;font-weight:700;color:#1e293b;">${p.nom || '—'}<br><span style="font-size:10px;color:#6b7280;">${p.cat||'SIN CATEGORÍA'}${p.proveedor?' · 🏭 '+p.proveedor:''}</span></td>
          <td style="padding:7px 10px;text-align:center;font-weight:900;color:${stockBajo?'#dc2626':'#ea580c'};">${p.stock || 0}</td>
          ...
          <td style="padding:7px 10px;text-align:right;font-weight:700;color:#ea580c;">$${ventaTotal.toFixed(2)}</td>
          <td style="padding:7px 10px;text-align:right;font-weight:900;color:#c2410c;">$${gananciaFila.toFixed(2)}</td>
        </tr>`;
        snapFilas.push({ id: String(p.id), nom: p.nom||'—', cat: p.cat||'SIN CATEGORÍA', proveedor: p.proveedor||'', stock: p.stock||0, vendido, costoTotal, ventaTotal, gananciaFila, costoUd, ventaUd });
      });
      const gananciaTotalS1 = totalVentaS1 - totalCostoS1;
      rows += `<tr style="background:#fffbeb;border-top:2px solid #ea580c;">
        <td colspan="3" style="padding:9px 10px;font-weight:900;font-size:13px;color:#3f6212;">TOTAL GENERAL</td>
        <td style="padding:9px 10px;text-align:right;font-weight:900;font-size:14px;color:#7c3aed;">$${totalCostoS1.toFixed(2)}</td>
        <td style="padding:9px 10px;text-align:right;font-weight:900;font-size:14px;color:#ea580c;">$${totalVentaS1.toFixed(2)}</td>
        <td style="padding:9px 10px;text-align:right;font-weight:900;font-size:14px;color:#c2410c;">$${gananciaTotalS1.toFixed(2)}</td>
      </tr>`;
    }
  ...
  if (tbody2) {
    ...
    listaTodos.forEach(p => {
      const cat = _rp_grupoDe(p, agruparPor);
      if (cat !== lastCat2) {
        rows += `<tr><td colspan="4" style="background:#dbeafe;color:#1e3a8a;font-size:11px;font-weight:900;padding:6px 10px;border-bottom:1px solid #93c5fd;">${agruparPor==='proveedor'?'🏭':'📦'} ${cat}</td></tr>`;
        lastCat2 = cat;
      }
      const pid = String(p.id);
      const costoUd = _compraUdReal(p);
      ...
      rows += `<tr style="border-bottom:1px solid #eff6ff;" id="rp_row_${pid}">
        <td style="padding:7px 10px;font-weight:700;color:#1e293b;vertical-align:middle;">${p.nom || '—'}<br><span style="font-size:10px;color:#6b7280;">${p.cat||'SIN CATEGORÍA'}${p.proveedor?' · 🏭 '+p.proveedor:''}</span></td>
...
function _rpGenerarImagen(filas, numImg, totalImgs, totalCostoPedido, totalVentaPedido, gananciaPedido, conDatos) {
  ...
  grad.addColorStop(0, '#365314'); grad.addColorStop(0.5, '#3f6212'); grad.addColorStop(1, '#4d7c0f');
  ctx.fillStyle = grad; ctx.fillRect(0, 0, ANCHO, H_BAND);

  // Círculo decorativo
  ctx.save(); ctx.globalAlpha = 0.12; ctx.fillStyle = '#bef264';
  ...
  ctx.fillStyle = '#bef264'; ctx.font = `${11*SCALE}px Arial`; ctx.textAlign = 'left';
  ctx.fillText(`${_getNombreTienda() || 'Despensa Económica'}  ·  Fecha: ${hoy}`, 20*SCALE, H_BAND + 19*SCALE);
  ctx.textAlign = 'right'; ctx.fillStyle = '#d9f99d';
  ctx.fillText(`Imagen ${numImg} / ${totalImgs}`, ANCHO - 20*SCALE, H_BAND + 19*SCALE);

  // Cabecera columnas
  const yCH = H_BAND + H_SUB;
  ctx.fillStyle = '#65a30d'; ctx.fillRect(0, yCH, ANCHO, H_HEAD);
  ctx.fillStyle = '#84cc16'; ctx.fillRect(0, yCH + H_HEAD - 3*SCALE, ANCHO, 3*SCALE);
  ...
  ctx.strokeStyle = '#a3e635'; ctx.lineWidth = 2*SCALE;
  ...
  for (let i = 0; i < POR_IMG; i++) {
    ...
    if (fila) {
      ...
      cg.addColorStop(0, '#65a30d'); cg.addColorStop(1, '#4d7c0f');
      ctx.fillStyle = cg;
  ...
  fg.addColorStop(0, '#365314'); fg.addColorStop(1, '#4d7c0f');
  ctx.fillStyle = fg; ctx.fillRect(0, yFoot, ANCHO, H_FOOT);
  ctx.fillStyle = '#bef264'; ctx.font = `${11*SCALE}px Arial`; ctx.textAlign = 'center';
  ...
  if (conDatos) {
    ...
    ctx.fillStyle = '#fffbeb'; ctx.fillRect(0, yRes, ANCHO, H_RESUMEN);
    ctx.strokeStyle = '#d9f99d'; ctx.lineWidth = 2*SCALE;
    ...
    statChip('TOTAL VENTA PEDIDO', '$'+totalVentaPedido.toFixed(2), ANCHO*0.72, cy1, '#ea580c', '#fdba74');

    const yGan = yRes + H_RESUMEN;
    ctx.fillStyle = '#ecfdf5'; ctx.fillRect(0, yGan, ANCHO, H_GANANCIA);
    ctx.strokeStyle = '#ea580c'; ctx.lineWidth = 1.5*SCALE;
    ...
    ctx.fillStyle = '#3f6212'; ctx.font = `bold ${12*SCALE}px Arial`; ctx.textAlign = 'center';
  ...
  ctx.strokeStyle = '#65a30d'; ctx.lineWidth = 3*SCALE;
  ctx.strokeRect(1.5*SCALE, 1.5*SCALE, ANCHO - 3*SCALE, ALTO - 3*SCALE);

  const nombreImg = conDatos ? `pedido_${numImg}_datos.png` : `pedido_${numImg}_proveedor.png`;
  _descargarCanvasPNG(canvas, nombreImg, (ok) => {
    if (ok) toast(`✓ Imagen ${numImg}/${totalImgs} (${conDatos ? 'con datos' : 'proveedor'}) descargada`);
  });
}
...
function rp_descargarPDFS1() {
  ...
  bloques.forEach((bloque, imgIdx) => {
    ...
    ctx.fillText(`${_getNombreTienda() || 'Despensa Económica'}  ·  ${snap.rangoStr}  ·  ${snap.catLabel}  ·  ${hoy}`, 20*SCALE, H_BAND + 19*SCALE);
    ...
    for (let i = 0; i < POR_IMG; i++) {
      ...
      if (f) {
        ...
        chip(f.stockAct, cx[1], '#c2410c', '#fffbeb');
        ...
        chip(f.ventaUd.toFixed(2), cx[4], '#3f6212', '#ecfdf5', '$');
      }
    }
    ...
    statChip('TOTAL VENTA (VENDIDO)', '$'+snap.totalVenta.toFixed(2), xB, cy1, '#ea580c', '#fff', '#fdba74');
    statChip('DISPONIBLE P/RECOMPRAR', '$'+snap.totalCosto.toFixed(2), xC2, cy1, '#b45309', '#fff', '#fde68a');
    statChip('GANANCIA DE LO VENDIDO', '$'+snap.ganancia.toFixed(2), xD, cy1, '#c2410c', '#fff', '#fdba74');

    // Fórmula
    ctx.fillStyle = '#ecfdf5'; ctx.strokeStyle = '#ea580c'; ctx.lineWidth = 1.5*SCALE;
    const fRw = ANCHO - 40*SCALE, fRx = 20*SCALE, fRy = yRes + 58*SCALE, fRh = 28*SCALE;
    ctx.beginPath(); ctx.roundRect(fRx, fRy, fRw, fRh, 8*SCALE); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#3f6212'; ctx.font = `bold ${11*SCALE}px Arial`; ctx.textAlign = 'center';
    ...
    _descargarCanvasPNG(canvas, `ventas_${imgIdx+1}.png`, (ok) => {
      if (ok) toast(`✓ Imagen ${imgIdx+1}/${totalImgs} descargada`);
    });
  });
}
...
function generarPDFRangoSimple() {
  ...
  let lista = (productos || []).filter(p =>
    (catFiltro === 'todas' || (p.cat || 'SIN CATEGORÍA') === catFiltro) &&
    (provFiltro === 'todos' || (p.proveedor || '') === provFiltro)
  );
  ...
  lista.forEach(p => {
    ...
    rows.push([p.nom || '—', cat, p.proveedor || '—', String(p.stock || 0), String(vendido), '$' + costoTotal.toFixed(2), '$' + ventaTotal.toFixed(2)]);
  });

  if (window.jspdf && doc.autoTable) {
    doc.autoTable({
      startY: y,
      head: [['Producto', 'Categoría', 'Proveedor', 'Stock', 'Vendido', 'Costo Inv.', 'Venta Inv.']],
      body: rows,
      styles: { fontSize: 8, cellPadding: 3 },
      ...
      columnStyles: {
        0: { fontStyle: 'bold' },
        3: { halign: 'center' },
        4: { halign: 'center', textColor: [29, 78, 216] },
        5: { halign: 'right', textColor: [124, 58, 237] },
  ...
  if (_descargarPDF(doc, `Inventario_actual_${desdeVal}_${hastaVal}${catFiltro!=='todas'?'_'+catFiltro:''}${provFiltro!=='todos'?'_'+provFiltro:''}.pdf`)) {
...
function piRender() {
  ...
  if (!Object.keys(snap).length) {
    tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:20px;color:#9ca3af;">Presiona "Recalcular inventario inicial" para registrar el stock actual de cada producto.</td></tr>';
    return;
  }
  ...
  keys.forEach((pid, i) => {
    ...
    if (cat !== lastCat) {
      html += `<tr><td colspan="4" style="background:#fff1f2;color:#be123c;font-size:10px;font-weight:900;padding:5px 10px;border-bottom:1px solid #fda4af;">📦 ${cat}</td></tr>`;
      lastCat = cat;
    }
    html += `<tr style="border-bottom:1px solid #fff1f2;${i%2===0?'background:#fff':'background:#fff9f9'}">
      ...
      <td style="padding:7px 10px;text-align:center;font-size:11px;color:#6b7280;font-weight:600;">${entry.proveedor || '—'}</td>
...
function piGuardarManual() {
  _requierePin(() => _piGuardarManualReal());
}
async function _piGuardarManualReal() {
