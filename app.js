...
function generarImagenEstadoResultados() {
  ...
  try {
    canvas.toBlob((blob) => {
      if (!blob) { toast('⚠ No se pudo generar la imagen del Estado de Resultados', true); return; }
      const url = URL.createObjectURL(blob);
    ...
    console.error('[generarImagenEstadoResultados]', e);
    toast('⚠ Error al descargar la imagen del Estado de Resultados', true);
  }
}
