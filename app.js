...
function pausarVentaActual() {
  ...
  try { localStorage.setItem('vpos_carritoEnEspera', JSON.stringify(carritoEnEspera)); } catch (e) {}
  carrito = [];
  renderCarrito();
  _actualizarBotonEspera();
...
function reanudarVentaEnEspera() {
  ...
  if (carrito.length) {
    if (!confirm('Tienes productos sin cobrar en el carrito actual. ¿Descartarlos para reanudar la venta en espera?')) return;
  ...
  localStorage.removeItem('vpos_carritoEnEspera');
  renderCarrito();
  _actualizarBotonEspera();
...
function _actualizarBotonEspera() {
  ...
  if (carritoEnEspera && carritoEnEspera.length) {
    const total = carritoEnEspera.reduce((s, i) => s + (Number(i.precio) || 0) * (i.cant || 0), 0);
