...
function guardarEdicionCobro() {
  ...
  if (!activos.length) {
    ...
    historial[_editCobroIdx] = { ...v, items: activos.map(i => { const {devuelto,...rest}=i; return rest; }), total: nuevoTotal.toFixed(2), _editTs: Date.now() };
  }
...
function ejecutarReiniciarMes() {
  ...
  // BUGFIX SYNC: guardar tombstones ANTES de tocar red/otros teléfonos.
  // _fusionarDos() y _autoCargarDesdeSupa() usan estos timestamps para saber que
  // el historial/pagos/restock de ANTES de este momento fueron borrados a propósito
  // y no deben "resucitar" si se fusionan con un teléfono que aún no recibió el
  // reinicio (mismo patrón que ya usa reiniciarHistorial() para vpos_historialWipeTs).
  const _tsReinicioMes = new Date().toISOString();
  localStorage.setItem('vpos_historialWipeTs', _tsReinicioMes); // protege historial (cobros)
  localStorage.setItem('vpos_mesWipeTs', _tsReinicioMes);       // protege pagos/gastos y restock log
  ...
  if (typeof _broadcast === 'function') {
    _broadcast('reinicio_mes', {
      ...
      efectivoInicial: 0,
      inventarioInicial: 0,
      // BUGFIX SYNC: el otro teléfono guarda estos tombstones al recibirlos para
      // que TAMBIÉN quede protegido en una fusión futura, no solo en este instante.
      historialWipeTs: _tsReinicioMes,
      mesWipeTs: _tsReinicioMes
    });
  }
  ...
  // BUGFIX SYNC: 2000ms no siempre alcanza — los _sbDeleteFiltro(...) de arriba (ventas,
  // pagos, ventas_diarias, finanzas_mes, cierre_diario, acciones_log) son async y no se
  // esperan (fire-and-forget). Si snapshot_push llega antes de que Supabase termine de
  // borrar, el otro teléfono puede recargar datos que todavía no se terminaron de borrar.
  // reiniciarHistorial() ya usa 5000ms para un borrado más chico; aquí hay más tablas
  // de por medio, así que usamos el mismo margen como mínimo razonable.
  setTimeout(() => {
    if (typeof _autoEnviarSnapshot === 'function') _autoEnviarSnapshot();
    if (typeof _broadcast === 'function') {
