...
// ── Descarga un <canvas> como PNG usando toBlob (más confiable que
// toDataURL en imágenes grandes) + el mismo mecanismo robusto de arriba.
...
function rp_descargarImagenesPedido() {
  ...
  // Chrome bloquea descargas automáticas múltiples si se disparan todas de
  // golpe. Se espacian ~450ms entre cada una para que cada descarga se
  // complete antes de disparar la siguiente.
  let _rpDelay = 0;
  bloques.forEach((bloque, idx) => {
    // Versión CON datos (costo/venta/ganancia) — para ti
    setTimeout(() => _rpGenerarImagen(bloque, idx + 1, totalImgs, totalCostoPedido, totalVentaPedido, gananciaPedido, true), _rpDelay);
    _rpDelay += 450;
    // Versión SIN datos financieros — para enviar al proveedor
    setTimeout(() => _rpGenerarImagen(bloque, idx + 1, totalImgs, totalCostoPedido, totalVentaPedido, gananciaPedido, false), _rpDelay);
    _rpDelay += 450;
  });
}
...
function rp_descargarPDFS1() {
  ...
  bloques.forEach((bloque, imgIdx) => {
    ...
    // Chrome bloquea descargas automáticas múltiples si se disparan todas
    // de golpe, así que si hay más de una imagen se espacian ~450ms.
    setTimeout(() => {
