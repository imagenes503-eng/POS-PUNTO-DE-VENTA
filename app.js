...
/* =====================================================================
   🔒 PIN DE SEGURIDAD — protege acciones sensibles (devoluciones,
   reinicios, borrados, edición de inventario y de los datos base del
   mes). Un solo PIN compartido, configurable por el dueño desde el
   botón "🔒 PIN" en la barra de respaldo. Si todavía no se configuró
   ningún PIN, las acciones se dejan pasar sin pedir nada (para no
   bloquear a nadie antes de que el dueño decida activarlo).
   ===================================================================== */
...
// Pide el PIN con un diálogo propio de la app (nunca prompt() nativo) y
// ejecuta `callback` solo si coincide. Si no hay PIN configurado aún,
// ejecuta `callback` directamente sin pedir nada.
function _requierePin(callback) {
  if (!_pinConfigurado()) { callback(); return; }
  _mostrarModalPin((pinIngresado) => {
    if (pinIngresado !== null && pinIngresado === _pinGuardado()) {
      callback();
    ...
    modal.innerHTML = `
      <div style="background:#fff;border-radius:20px;width:100%;max-width:320px;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
        ...
        <div style="padding:20px 22px;">
          ...
          <input id="inputPinSeguridad" type="password" inputmode="numeric" pattern="[0-9]*" maxlength="12" placeholder="••••"
            style="width:100%;box-sizing:border-box;text-align:center;letter-spacing:6px;font-size:22px;font-weight:900;padding:12px;border:1.5px solid #e5e7eb;border-radius:12px;margin-bottom:16px;font-family:'Nunito',sans-serif;">
...
// ── Configurar / cambiar / quitar el PIN de seguridad ──
function abrirConfigPin() {
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px;width:100%;max-width:340px;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
      <div style="background:linear-gradient(135deg,#7c2d12,#c2410c);padding:20px 22px 16px;text-align:center;">
        <div style="font-size:30px;margin-bottom:6px;">🔒</div>
        <div style="color:#fff;font-size:15px;font-weight:900;font-family:'Nunito',sans-serif;">PIN de Seguridad</div>
        <div style="color:rgba(255,255,255,0.85);font-size:11px;font-weight:700;font-family:'Nunito',sans-serif;margin-top:4px;">
          ${yaConfigurado ? 'Protege devoluciones, reinicios, borrados y edición de datos sensibles.' : 'Aún no tienes un PIN configurado — actívalo para proteger las acciones sensibles.'}
        </div>
      </div>
      <div style="padding:20px 22px;">
        ${yaConfigurado ? `
        <label style="font-size:11px;font-weight:900;color:#c2410c;">PIN actual</label>
        <input id="cpPinActual" type="password" inputmode="numeric" placeholder="PIN actual" style="width:100%;box-sizing:border-box;padding:11px;border:1.5px solid #e5e7eb;border-radius:10px;margin:6px 0 14px;font-size:15px;font-family:'Nunito',sans-serif;">
        ` : ''}
        <label style="font-size:11px;font-weight:900;color:#c2410c;">${yaConfigurado ? 'Nuevo PIN' : 'Crea un PIN'}</label>
        <input id="cpPinNuevo" type="password" inputmode="numeric" placeholder="Ej: 1234" style="width:100%;box-sizing:border-box;padding:11px;border:1.5px solid #e5e7eb;border-radius:10px;margin:6px 0 14px;font-size:15px;font-family:'Nunito',sans-serif;">
        <label style="font-size:11px;font-weight:900;color:#c2410c;">Confirmar PIN</label>
        <input id="cpPinConfirmar" type="password" inputmode="numeric" placeholder="Repite el PIN" style="width:100%;box-sizing:border-box;padding:11px;border:1.5px solid #e5e7eb;border-radius:10px;margin:6px 0 18px;font-size:15px;font-family:'Nunito',sans-serif;">
        ...
        ${yaConfigurado ? `<button id="cpBtnQuitar" type="button" style="width:100%;padding:12px;background:#fef2f2;border:1.5px solid #fecaca;border-radius:12px;font-size:13px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#dc2626;margin-bottom:10px;">🗑 Quitar protección por PIN</button>` : ''}
  ...
  document.getElementById('cpBtnGuardar').onclick = () => {
    if (yaConfigurado) {
      ...
      if (actual !== _pinGuardado()) { toast('❌ El PIN actual no es correcto', true); return; }
    }
    const nuevo   = document.getElementById('cpPinNuevo').value.trim();
    const confirmar = document.getElementById('cpPinConfirmar').value.trim();
    if (!nuevo || nuevo.length < 4) { toast('El PIN debe tener al menos 4 dígitos', true); return; }
    if (nuevo !== confirmar) { toast('Los PIN no coinciden', true); return; }
    localStorage.setItem(_PIN_STORAGE_KEY, nuevo);
    modal.style.display = 'none';
    toast('🔒 PIN de seguridad guardado');
  };
  ...
  if (btnQuitar) {
    btnQuitar.onclick = () => {
      const actual = document.getElementById('cpPinActual').value;
      if (actual !== _pinGuardado()) { toast('❌ El PIN actual no es correcto', true); return; }
      if (!confirm('¿Quitar la protección por PIN? Las acciones sensibles dejarán de pedir PIN.')) return;
      localStorage.removeItem(_PIN_STORAGE_KEY);
      modal.style.display = 'none';
      toast('PIN de seguridad desactivado');
    };
  }
}
