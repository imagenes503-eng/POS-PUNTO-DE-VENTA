...
// ── Atender otro cliente en una pestaña nueva, sin tocar la venta en curso ──
// Cada pestaña del navegador tiene su propio carrito en memoria (no se
// guarda hasta cobrar), así que abrir una pestaña nueva de la misma página
// ya te da una venta totalmente aparte, sin arriesgar la venta grande.
function abrirVentaNuevaPestana() {
  const url = location.origin + location.pathname + '#ventaRapida';
  const nueva = window.open(url, '_blank');
  if (!nueva) {
    toast('⚠ El navegador bloqueó la pestaña nueva — permite ventanas emergentes para este sitio', true);
    return;
  }
  toast('🗔 Pestaña nueva abierta — esta venta sigue guardada aquí');
}

// Si esta pestaña se abrió desde otra (botón de arriba), muestra un botón
// flotante para volver a esa pestaña original con un toque.
function _initVentaRapidaSiAplica() {
  if (location.hash !== '#ventaRapida' || !window.opener) return;
  const btn = document.createElement('button');
  btn.textContent = '← Volver a la venta anterior';
  btn.style.cssText = 'position:fixed;left:12px;right:12px;bottom:14px;z-index:99998;background:linear-gradient(135deg,#0369a1,#075985);color:#fff;border:none;border-radius:12px;padding:13px;font-weight:900;font-family:Nunito,sans-serif;font-size:14px;box-shadow:0 6px 20px rgba(3,105,161,.4);cursor:pointer;';
  btn.onclick = () => {
    try { window.opener.focus(); } catch (e) {}
    toast('👉 Cambia a la otra pestaña arriba de tu navegador');
  };
  document.body.appendChild(btn);
  // Abre directo la pantalla de Registrar Venta para despachar rápido
  setTimeout(() => { if (typeof abrirModalVenta === 'function') abrirModalVenta(); }, 800);
}
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', _initVentaRapidaSiAplica);
} else {
  _initVentaRapidaSiAplica();
}
