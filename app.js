...
function abrirConfigPin() {
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px;width:100%;max-width:340px;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
      ...
      <div style="padding:20px 22px;">
        ...
        <input id="cpPinActual" type="password" autocomplete="off" autocapitalize="off" placeholder="Contraseña actual" style="width:100%;box-sizing:border-box;padding:11px;border:1.5px solid #e5e7eb;border-radius:10px;margin:6px 0 4px;font-size:15px;font-family:'Nunito',sans-serif;">
        <div style="text-align:right;margin-bottom:10px;">
          <span id="cpOlvide" style="font-size:11px;font-weight:800;color:#c2410c;text-decoration:underline;cursor:pointer;">¿Olvidaste tu contraseña? Verificar por correo</span>
        </div>
        ` : ''}
  ...
  const linkOlvide = document.getElementById('cpOlvide');
  if (linkOlvide) {
    linkOlvide.onclick = () => { modal.style.display = 'none'; _enviarCodigoRecuperacionPin(); };
  }
}

// =====================================================================
//  Restablecer contraseña de seguridad con verificación por correo
//  (código de 6 dígitos) — así nadie que solo tenga el teléfono en la
//  mano puede quitar la protección; hace falta también el acceso a tu
//  correo de cuenta. Usa el mismo sistema de correo de Supabase Auth
//  que tu POS ya usa para "Recuperar contraseña de cuenta".
// =====================================================================
async function _enviarCodigoRecuperacionPin() {
  const email = _usuarioActual && _usuarioActual.email;
  if (!email) { toast('No se encontró tu correo de cuenta — vuelve a iniciar sesión e intenta de nuevo', true); return; }
  toast('📧 Enviando código a ' + email + '…');
  try {
    const resp = await fetch(_sbUrl() + '/auth/v1/otp', {
      method: 'POST',
    ...
    if (!resp.ok) {
      ...
      throw new Error(data.msg || data.error_description || 'No se pudo enviar el código');
    }
    _mostrarModalCodigoPin(email);
  } catch (e) {
    toast('⚠ ' + e.message, true);
  }
}

function _mostrarModalCodigoPin(email) {
  let modal = document.getElementById('modalCodigoPin');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'modalCodigoPin';
    modal.style.cssText = 'position:fixed;inset:0;z-index:10091;background:rgba(5,46,22,0.75);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;padding:16px;';
    modal.addEventListener('click', e => { if (e.target === modal) modal.style.display = 'none'; });
    document.body.appendChild(modal);
  }
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px;width:100%;max-width:320px;box-shadow:0 24px 60px rgba(0,0,0,0.4);overflow:hidden;">
      <div style="background:linear-gradient(135deg,#7c2d12,#c2410c);padding:20px 22px 16px;text-align:center;">
        <div style="font-size:30px;margin-bottom:6px;">📧</div>
        <div style="color:#fff;font-size:15px;font-weight:900;font-family:'Nunito',sans-serif;">Verifica tu correo</div>
        <div style="color:rgba(255,255,255,0.85);font-size:11px;font-weight:700;font-family:'Nunito',sans-serif;margin-top:4px;">
          Enviamos un código a ${email}
        </div>
      </div>
      <div style="padding:20px 22px;">
        <div style="font-size:12px;font-weight:700;color:#374151;font-family:'Nunito',sans-serif;text-align:center;margin-bottom:14px;">
          Escribe el código de 6 dígitos que recibiste (revisa spam si no aparece)
        </div>
        <input id="inputCodigoPin" type="text" inputmode="numeric" autocomplete="off" maxlength="6" placeholder="••••••"
          style="width:100%;box-sizing:border-box;text-align:center;letter-spacing:8px;font-size:22px;font-weight:900;padding:12px;border:1.5px solid #e5e7eb;border-radius:12px;margin-bottom:6px;font-family:'Nunito',sans-serif;">
        <div id="codigoPinMsg" style="display:none;font-size:11px;font-weight:800;text-align:center;margin-bottom:10px;"></div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px;">
          <button id="btnCancelarCodigoPin" type="button" style="padding:13px;background:#f9fafb;border:1.5px solid #e5e7eb;border-radius:12px;font-size:14px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#374151;">✕ Cancelar</button>
          <button id="btnVerificarCodigoPin" type="button" style="padding:13px;background:linear-gradient(135deg,#ea580c,#c2410c);border:none;border-radius:12px;font-size:14px;font-weight:900;font-family:'Nunito',sans-serif;cursor:pointer;color:#fff;box-shadow:0 4px 14px rgba(234,88,12,0.3);">Verificar</button>
        </div>
        <div style="text-align:center;margin-top:12px;">
          <span id="btnReenviarCodigoPin" style="font-size:11px;font-weight:800;color:#c2410c;text-decoration:underline;cursor:pointer;">Reenviar código</span>
        </div>
      </div>
    </div>`;
  modal.style.display = 'flex';
  const input = document.getElementById('inputCodigoPin');
  input.value = '';
  setTimeout(() => input.focus(), 60);

  const verificar = async () => {
    const codigo = input.value.trim();
    const msg = document.getElementById('codigoPinMsg');
    if (!codigo || codigo.length < 6) {
      msg.style.display = 'block'; msg.style.color = '#dc2626'; msg.textContent = 'Ingresa los 6 dígitos del código';
      return;
    }
    const btn = document.getElementById('btnVerificarCodigoPin');
    btn.disabled = true; btn.textContent = 'Verificando…';
    try {
      const resp = await fetch(_sbUrl() + '/auth/v1/verify', {
        method: 'POST',
        headers: { 'apikey': _sbKey(), 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'email', email, token: codigo })
      });
      const data = await resp.json().catch(() => ({}));
      if (resp.ok && data.access_token) {
        // Código correcto — NO usamos ni guardamos este token (no
        // queremos alterar tu sesión actual del POS), solo confirma
        // que sí tienes acceso a ese correo.
        modal.style.display = 'none';
        localStorage.removeItem(_PIN_STORAGE_KEY);
        toast('✅ Verificado — contraseña anterior borrada, crea una nueva');
        abrirConfigPin();
      } else {
  ...
  document.getElementById('btnCancelarCodigoPin').onclick = () => { modal.style.display = 'none'; };
  document.getElementById('btnVerificarCodigoPin').onclick = verificar;
  input.onkeydown = (e) => { if (e.key === 'Enter') verificar(); };
  document.getElementById('btnReenviarCodigoPin').onclick = () => { modal.style.display = 'none'; _enviarCodigoRecuperacionPin(); };
}
...
function generarPDFRango() {
  ...
  if (_descargarPDF(doc, `Reporte_${desdeVal}_${hastaVal}${catFiltro!=='todas'?'_'+catFiltro:''}.pdf`)) {
  ...
  if (tbody1) {
    ...
    if (!listaConVentas.length) {
      ...
      listaConVentas.forEach(p => {
        const cat = p.cat || 'SIN CATEGORÍA';
        if (cat !== lastCat) {
          rows += `<tr><td colspan="6" style="background:#ffedd5;color:#9a3412;font-size:11px;font-weight:900;padding:6px 10px;border-bottom:1px solid #fdba74;">📦 ${cat}</td></tr>`;
          lastCat = cat;
        }
...
function generarPDFRangoSimple() {
  ...
  if (_descargarPDF(doc, `Inventario_actual_${desdeVal}_${hastaVal}${catFiltro!=='todas'?'_'+catFiltro:''}.pdf`)) {
