...
(function _estilosCierre() {
  ...
  s.textContent = `
    ...
    :root{
      ...
      --cd-green:#65a30d;--cd-amber:#d97706;--cd-red:#dc2626;--cd-purple:#7c3aed;
    ...
    .cd-hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 80% 60% at 70% -10%,rgba(99,179,237,0.18),transparent),radial-gradient(ellipse 50% 80% at 10% 110%,rgba(132,204,22,0.12),transparent);pointer-events:none;}
    ...
    .cd-total-row.green{background:#ecfdf5;} .cd-total-row.green span{color:#65a30d!important;}
    ...
    .cd-btn-update.green{background:rgba(101,163,13,0.08);border-color:rgba(101,163,13,0.25);color:var(--cd-green);}
    .cd-btn-update.green:hover{background:rgba(101,163,13,0.14);}
    ...
    .cd-item-denom.inv{background:#ffedd5;border-color:#fdba74;color:#c2410c;}
    /* ── Cambios grid ── */
    ...
    /* Util colors */
    .val-pos{color:#65a30d;} .val-neg{color:#dc2626;} .val-warn{color:#d97706;}
    .val-blue{color:#1e40af;} .val-purple{color:#7c3aed;}
    /* ── Action buttons ── */
    .btn-cd-captura{width:100%;padding:15px;background:linear-gradient(135deg,#65a30d,#84cc16);color:#fff;border:none;border-radius:14px;font-size:14px;font-weight:800;font-family:var(--cd-ff);cursor:pointer;box-shadow:0 4px 16px rgba(101,163,13,0.3);transition:all .15s;display:flex;align-items:center;justify-content:center;gap:8px;letter-spacing:-0.2px;}
    .btn-cd-captura:hover{transform:translateY(-1px);box-shadow:0 6px 20px rgba(101,163,13,0.35);}
...
function _cdFlashBtnGuardar(){
  ...
  if(btn){
    btn.style.background = 'linear-gradient(135deg,#65a30d,#84cc16)';
    btn.innerHTML = '✅ Guardado';
    clearTimeout(_cdFlashTimer);
    _cdFlashTimer = setTimeout(()=>{
      btn.style.background = 'linear-gradient(135deg,#c2410c,#ea580c)';
...
async function renderCierreDia(pgId){
  ...
  pg.innerHTML=`
    ...
    <div class="cd-body">
      ...
      <div class="cd-panel">
        ...
        <div class="cd-panel-body">
          ...
          <div style="background:#fef2f2;border:1.5px solid #fca5a5;border-radius:12px;padding:12px;margin-top:4px;">
            ...
            <div style="margin-top:12px;padding:10px 12px;background:#fffbeb;border:1.5px solid #fdba74;border-radius:10px;">
              <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:12px;font-weight:900;color:#c2410c;font-family:Nunito,sans-serif;margin-bottom:8px;">
                <input type="checkbox" id="cdGastoEsInventario" onchange="_cdToggleInvFields()" style="width:16px;height:16px;accent-color:#ea580c;"> 📦 Este pago es para inventario (tiene ganancia)
              </label>
              <div id="cdGastoInvFields" style="display:none;grid-template-columns:1fr 1fr;gap:8px;">
                ...
                <div class="cd-field" style="grid-column:span 2;"><label>Ganancia estimada</label><div id="cdGastoGananciaLbl" style="padding:8px 12px;background:#ffedd5;border-radius:8px;font-size:14px;font-weight:900;color:#c2410c;font-family:Nunito,sans-serif;">$0.00</div></div>
              </div>
            </div>
      ...
      <!-- SALDO EN CAJA -->
      <div class="cd-panel" style="border:2px solid #c2410c;">
        <div class="cd-panel-header" style="background:linear-gradient(135deg,#c2410c,#ea580c);">
        ...
        <div class="cd-panel-body">
          <div style="font-size:11px;color:#c2410c;font-weight:700;background:#fffbeb;border:1px solid #fed7aa;border-radius:8px;padding:8px 12px;margin-bottom:12px;font-family:Nunito,sans-serif;">
          ...
          <button id="cdBtnGuardarSaldo"
            ...
            style="margin-top:14px;width:100%;padding:13px;background:linear-gradient(135deg,#c2410c,#ea580c);color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:800;font-family:var(--cd-ff);cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 3px 10px rgba(21,128,61,0.3);transition:all .2s;">
            💾 Guardar saldo en caja
          </button>
          <div id="cdSaldoGuardadoMsg" style="display:none;text-align:center;font-size:11px;font-weight:700;color:#c2410c;margin-top:6px;font-family:var(--cd-ff);">✓ Saldo guardado correctamente</div>
        </div>
      </div>
      <!-- REGISTRO MENSUAL -->
      <div class="cd-panel">
        ...
        <div class="cd-panel-body">
            ...
            <table class="cd-mes-tabla" id="cdMesTablaVentas">
              ...
              <tbody>
                ${_cdMesData.ventas.length ? _cdMesData.ventas.slice().reverse().map(v=>`
                  <tr>
                    ...
                    <td style="color:#c2410c;font-weight:900;">$${(v.total-(v.alquiler||0)).toFixed(2)}</td>
              ...
              <tfoot>
                <tr style="background:#fffbeb;">
                  <td style="font-weight:900;color:#c2410c;">Total</td>
                  ...
                  <td style="font-weight:900;color:#c2410c;">$${(totalVentasMes-totalAlquilerMes).toFixed(2)}</td>
                  <td></td>
                </tr>
              </tfoot>
            </table>
            ...
            <table class="cd-mes-tabla" id="cdMesTablaGastos">
              ...
              <tbody>
                ${_cdMesData.gastos.length ? _cdMesData.gastos.slice().reverse().map(g=>`
                  <tr>
                    ...
                    <td style="color:#c2410c;">${g.tipoInv?`Costo: $${g.costoInv.toFixed(2)} → Venta: $${(g.costoInv+g.gananciaInv).toFixed(2)} (+$${g.gananciaInv.toFixed(2)})`:'-'}</td>
              ...
              <tfoot>
                <tr style="background:#fef2f2;">
                  ...
                  <td style="font-weight:900;color:#c2410c;">Total ganancia inv: $${totalInvGanancia.toFixed(2)}</td>
                  <td></td>
                </tr>
              </tfoot>
            </table>
          ...
          <!-- Resumen mensual -->
          <div style="background:#fffbeb;border:1.5px solid #fdba74;border-radius:12px;padding:14px 16px;margin-top:8px;">
            <div style="font-size:13px;font-weight:900;color:#c2410c;margin-bottom:10px;font-family:Nunito,sans-serif;">📊 Resumen del mes</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:12px;font-family:Nunito,sans-serif;">
              ...
              <div style="color:var(--text-muted);font-weight:700;">+ Ventas a caja:</div><div style="font-weight:900;color:#c2410c;">+$${ventasACajaMes.toFixed(2)}</div>
              <div style="color:var(--text-muted);font-weight:700;border-top:1px solid #fed7aa;padding-top:6px;font-weight:900;">💵 Debería haber en CAJA:</div>
              <div style="font-weight:900;color:#0369a1;font-size:14px;border-top:1px solid #fed7aa;padding-top:6px;">$${cajaFinal.toFixed(2)}</div>
              ...
              <div style="color:var(--text-muted);font-weight:700;">+ Ganancia de recompras:</div><div style="font-weight:900;color:#c2410c;">+$${totalInvGanancia.toFixed(2)}</div>
              <div style="color:var(--text-muted);font-weight:700;border-top:1px solid #fed7aa;padding-top:6px;">📦 Debería haber en INVENTARIO:</div>
              <div style="font-weight:900;color:#7c3aed;font-size:14px;border-top:1px solid #fed7aa;padding-top:6px;">$${invFinal.toFixed(2)}</div>
            </div>
          </div>
      ...
      <div class="cd-panel" style="border:2px solid #7c3aed;margin-top:4px;">
        ...
        <div class="cd-panel-body">
          ...
          <div style="background:#fffbeb;border:1.5px solid #fdba74;border-radius:12px;padding:12px 14px;margin-bottom:10px;">
            <div style="display:flex;align-items:center;gap:6px;font-size:11px;font-weight:900;color:#c2410c;text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px;">
              💵 B) Ganancia Ya Realizada <span style="font-weight:700;text-transform:none;opacity:.75;">(ventas de este mes)</span>
            </div>
            <div style="font-size:22px;font-weight:900;color:#c2410c;">$${gananciaEfectivoMes.toFixed(2)}</div>
            <div style="font-size:10px;color:#ea580c;font-weight:700;margin-top:3px;">
          ...
          <div style="margin-bottom:10px;">
            ...
            <div style="background:#ede9fe;border-radius:99px;height:14px;overflow:hidden;">
              <div style="height:100%;width:${Math.min(100, progresoPct)}%;background:linear-gradient(90deg,#7c3aed,${metaCumplida ? '#ea580c' : '#a78bfa'});border-radius:99px;transition:width .3s;"></div>
            </div>
          </div>

          <!-- Total general -->
          <div style="background:${metaCumplida ? '#fffbeb' : '#fef2f2'};border:1.5px solid ${metaCumplida ? '#fdba74' : '#fca5a5'};border-radius:12px;padding:14px 16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;padding-bottom:8px;border-bottom:1px dashed ${metaCumplida ? '#fed7aa' : '#fecaca'};">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;padding-bottom:8px;border-bottom:1px dashed ${metaCumplida ? '#fed7aa' : '#fecaca'};">
              <span style="font-size:11px;font-weight:800;color:#c2410c;">💵 Ganancia ya en efectivo</span>
              <span style="font-size:13px;font-weight:900;color:#c2410c;">$${gananciaEfectivoMes.toFixed(2)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
              <span style="font-size:12px;font-weight:900;color:var(--text-muted);">💰 TOTAL GENERAL DE GANANCIA</span>
              <span style="font-size:19px;font-weight:900;color:${metaCumplida ? '#c2410c' : '#b45309'};">$${gananciaGeneralMes.toFixed(2)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;border-top:1px solid ${metaCumplida ? '#fed7aa' : '#fecaca'};padding-top:8px;">
              <span style="font-size:12px;font-weight:900;color:${metaCumplida ? '#c2410c' : '#dc2626'};">
                ${metaCumplida ? '✅ ¡Meta cumplida o superada!' : '📉 Aún falta para la meta'}
              </span>
              <span style="font-size:14px;font-weight:900;color:${metaCumplida ? '#c2410c' : '#dc2626'};">
        ...
        <div class="cd-panel-body">
          ...
          <div style="background:#ecfeff;border:1.5px solid #a5f3fc;border-radius:12px;padding:14px 16px;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2px;">
              ...
              <span style="font-size:13px;font-weight:900;color:#c2410c;">+$${ingresoRealMes.toFixed(2)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;padding-bottom:8px;border-bottom:1px dashed #a5f3fc;">
...
function _cdEliminarVentaMes(id){
  ...
  if(tbl){
    ...
    tbl.innerHTML=_cdMesData.ventas.length?_cdMesData.ventas.slice().reverse().map(v=>`<tr><td>${_cdFmtFecha(v.fecha)}</td><td style="color:#c2410c;font-weight:900;">$${v.total.toFixed(2)}</td><td style="color:#b45309;">$${(v.alquiler||0).toFixed(2)}</td><td style="color:#0369a1;font-weight:900;">$${(v.total-(v.alquiler||0)).toFixed(2)}</td><td><button style="background:none;border:none;cursor:pointer;color:#dc2626;font-size:14px;padding:2px 6px;" onclick="_cdEliminarVentaMes('${v.id}')">✕</button></td></tr>`).join(''):'<tr><td colspan="5" style="text-align:center;color:#94a3b8;">Sin ventas registradas</td></tr>';
...
function _cdEliminarGastoMes(id){
  ...
  if(tbl){
    ...
    tbl.innerHTML=_cdMesData.gastos.length?_cdMesData.gastos.slice().reverse().map(g=>`<tr><td>${_cdFmtFecha(g.fecha)}</td><td>${g.desc}</td><td style="color:#dc2626;font-weight:900;">$${g.total.toFixed(2)}</td><td style="color:#c2410c;">${g.tipoInv?`Costo: $${g.costoInv.toFixed(2)} → Venta: $${(g.costoInv+g.gananciaInv).toFixed(2)} (+$${g.gananciaInv.toFixed(2)})`:'-'}</td><td><button style="background:none;border:none;cursor:pointer;color:#dc2626;font-size:14px;padding:2px 6px;" onclick="_cdEliminarGastoMes('${g.id}')">✕</button></td></tr>`).join(''):'<tr><td colspan="5" style="text-align:center;color:#94a3b8;">Sin gastos registrados</td></tr>';
...
function _cdActualizarStats(){
  ...
  if(cRes) cRes.innerHTML=movsPendientes.length
    ?movsPendientes.map(m=>`<div style="display:flex;align-items:center;gap:8px;padding:5px 10px;background:var(--surface);border:1px solid var(--border);border-radius:7px;margin-bottom:4px;font-size:12px;font-family:Nunito,sans-serif;"><span style="font-weight:900;color:#dc2626;">${m.de} −$${m.monto.toFixed(2)}</span><span>→</span><span style="font-weight:900;color:#c2410c;">${m.hacia} +$${m.monto.toFixed(2)}</span></div>`).join(''):'';
...
async function _cdTomarCaptura(){
  ...
  try{
    ...
    if (typeof _descargarCanvasPNG === 'function') {
      _descargarCanvasPNG(outCanvas, `Cierre_${_cdFecha}.png`, (ok) => {
        if (ok && typeof toast==='function') toast('📸 Imagen 9:16 descargada ('+OUT_W+'×'+OUT_H+')');
      });
    } else {
...
async function _cdGenerarPDFMensual(){
  ...
  doc.text((typeof _getNombreTienda === 'function' && _getNombreTienda() || 'Despensa Económica') + ' — Estado de Resultados '+mes,w/2,y,{align:'center'});

  if (typeof _descargarPDF === 'function' && _descargarPDF(doc, `Estado_Resultados_${mes}.pdf`)) {
    if(typeof toast==='function')toast('📄 Estado de Resultados descargado');
  } else if (typeof _descargarPDF !== 'function') {
