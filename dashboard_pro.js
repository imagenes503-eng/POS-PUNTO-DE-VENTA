...
function renderDashboardPro() {
  ...
  // ── Hora pico histórica (todos los días, no solo hoy) ──
  const flujoPorHora = {}; // hora (0-23) -> número de ventas
  (historial || []).forEach(v => {
    const h = new Date(v.fechaISO || v.ts || 0).getHours();
    flujoPorHora[h] = (flujoPorHora[h] || 0) + 1;
  });
  let horaPico = null, horaPicoCant = 0;
  Object.entries(flujoPorHora).forEach(([h, cant]) => {
    if (cant > horaPicoCant) { horaPicoCant = cant; horaPico = Number(h); }
  });
  const _fmtHora12 = (h) => {
  ...
  const horaPicoTxt = horaPico === null ? null : `${_fmtHora12(horaPico)} – ${_fmtHora12((horaPico+1)%24)}`;
  ...
  cont.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
      ...
      <div style="background:#fff;border:1.5px solid #e5e7eb;border-radius:14px;overflow:hidden;">
        ...
        ${horaPicoTxt ? `
        <div style="margin:10px 12px 0;background:linear-gradient(135deg,#fff7ed,#ffedd5);border:1px solid #fdba74;border-radius:10px;padding:9px 10px;text-align:center;">
          <div style="font-size:9px;font-weight:900;color:#9a3412;text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;">🔥 Hora pico</div>
          <div style="font-size:12.5px;font-weight:900;color:#7c2d12;line-height:1.3;">${horaPicoTxt}</div>
          <div style="font-size:9px;font-weight:700;color:#c2410c;margin-top:2px;">${horaPicoCant} venta${horaPicoCant!==1?'s':''} · todos los días</div>
        </div>` : ''}
            ...
            : `<div style="display:flex;align-items:flex-end;gap:3px;height:80px;">
                ${Array.from({length:14},(_,i)=>i+7).map(h => {
                  ...
                  return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;" title="$${val.toFixed(0)}">
                    <div style="width:100%;background:${val>0?'#f97316':'#f3f4f6'};border-radius:3px 3px 0 0;height:${pct}px;transition:height .4s;min-height:4px;"></div>
      ...
      <div style="background:#fff;border:1.5px solid ${prodsBajos.length>0?'#fca5a5':'#e5e7eb'};border-radius:14px;overflow:hidden;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid #f3f4f6;">
          ...
          <span style="background:${prodsBajos.length>0?'rgba(220,38,38,.1)':'#fffbeb'};color:${prodsBajos.length>0?'#dc2626':'#c2410c'};border-radius:100px;padding:2px 8px;font-size:11px;font-weight:900;">${prodsBajos.length}</span>
        </div>
          ...
          ${ultimasVentas.length===0
            ...
            :ultimasVentas.map(v=>{
              ...
              return `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #f9fafb;">
                ...
                <div style="font-family:'Space Mono',monospace;font-size:12px;font-weight:900;color:#c2410c;">$${parseFloat(v.total).toFixed(2)}</div>
              </div>`;
            }).join('')}
        </div>
      </div>
...
function generarReportePDF() {
  ...
  doc.text((typeof _getNombreTienda === 'function' && _getNombreTienda()) || 'Despensa Económica', 14, 14);
  ...
  if (typeof _descargarPDF === 'function') {
    if (_descargarPDF(doc, `Reporte_${tiendaId}_${_hoyStr()}.pdf`)) _backupMsg('✅ Reporte PDF generado');
  } else {
